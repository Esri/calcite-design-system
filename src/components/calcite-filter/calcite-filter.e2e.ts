...
describe("calcite-filter", () => {
  ...
  describe("strings", () => {
    it("should update the filter placeholder when a string is provided", async () => {
      ...
      await page.setContent(
        `<calcite-filter placeholder="${placeholderText}"></calcite-filter>`
      );
  ...
  describe("filter behavior", () => {
    ...
    it("emits an event with filtered data after a search query is typed into the input", async () => {
      ...
      expect(
        event.detail.find((element) => element.value === "harry")
      ).toBeDefined();
      expect(
        event.detail.find((element) => element.value === "matt")
      ).toBeDefined();
      expect(
        event.detail.find((element) => element.value === "franco")
      ).toBeDefined();
      expect(
        event.detail.find((element) => element.value === "jon")
      ).toBeDefined();
      expect(
        event.detail.find((element) => element.value === "katy")
      ).toBeUndefined();
    });
