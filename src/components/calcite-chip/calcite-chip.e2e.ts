...
describe("calcite-chip", () => {
  it("renders", async () =>
    renders("<calcite-chip active>doritos</calcite-chip>"));
  it("honors hidden attribute", async () => hidden("calcite-chip"));

  it("is accessible", async () =>
    accessible(`<calcite-chip active>doritos</calcite-chip>`));

  it("should be hidden after the close button is clicked", async () => {
    const page = await newE2EPage();
    await page.setContent(`<calcite-chip active>cheetos</calcite-chip>`);
