  ...
  describe("text directionality", () => {
    ...
    describe("initial render", () => {
      it("should have default LTR direction, but no `dir` attribute", async () => {
        ...
        const tileSelectGroupEl = await page.find("calcite-tile-select-group");
        expect(await (await tileSelectGroupEl.getComputedStyle()).getPropertyValue("direction")).toEqual("ltr");
        expect(tileSelectGroupEl.getAttribute("dir")).toBeNull();
      ...
      it("should honor ancestor's `dir` attribute, and not have its own `dir` attribute", async () => {
        const page = await newE2EPage({
          ...
          <html lang="he" dir="rtl">
            <body>
              <calcite-tile-select-group>${groupContent}</calcite-tile-select-group>
            </body>
          </html>
        `
        });
        const tileSelectGroupEl = await page.find("calcite-tile-select-group");
        expect(await (await tileSelectGroupEl.getComputedStyle()).getPropertyValue("direction")).toEqual("rtl");
        expect(tileSelectGroupEl.getAttribute("dir")).toBeNull();
      });
    });
    ...
    describe("when it has its own `dir` attribute", () => {
      it("should render with text direction based on `dir` value", async () => {
        ...
        const tileSelectGroupEl = await page.find("calcite-tile-select-group");
        expect(await (await tileSelectGroupEl.getComputedStyle()).getPropertyValue("direction")).toEqual("rtl");
        expect(tileSelectGroupEl.getAttribute("dir")).toEqual("rtl");
