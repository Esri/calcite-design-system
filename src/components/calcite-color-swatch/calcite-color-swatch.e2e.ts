...
describe("calcite-color-swatch", () => {
  ...
  it("reflects", () => reflects("calcite-color-swatch", [
    {
      propertyName: "isActive",
      value: true
    }
  ...
  describe("has accepts CSS color strings", () => {
    it("supports rgb", async () => {
      ...
      await page.setContent("<calcite-color-swatch color='rgb(255, 255, 255)'></calcite-color-swatch>");
    ...
    it("supports keywords", async () => {
      const page = await newE2EPage();
      await page.setContent("<calcite-color-swatch color='chartreuse'></calcite-color-swatch>");
      const swatch = await page.find(`calcite-color-swatch >>> .${CSS.swatch}`);
      const style = await swatch.getComputedStyle();
    ...
    it("supports hsl", async () => {
      const page = await newE2EPage();
      await page.setContent("<calcite-color-swatch color='hsl(120, 100%, 97%)'></calcite-color-swatch>");
      const swatch = await page.find(`calcite-color-swatch >>> .${CSS.swatch}`);
      const style = await swatch.getComputedStyle();
    ...
    it("supports hex", async () => {
      const page = await newE2EPage();
      await page.setContent("<calcite-color-swatch color='#FF8200'></calcite-color-swatch>");
      const swatch = await page.find(`calcite-color-swatch >>> .${CSS.swatch}`);
  ...
  it("has an active state", async () => {
    const page = await newE2EPage();
    await page.setContent("<calcite-color-swatch is-active></calcite-colo r-swatch>");
    const swatch = await page.find(`calcite-color-swatch >>> .${CSS.swatchActive}`);
