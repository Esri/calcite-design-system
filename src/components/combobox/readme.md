...
| `calciteComboboxChipDismiss`  | Fires when a selected item in the component is dismissed via its `calcite-chip`. **Note:**: The event payload is deprecated, please use the `value` property on the component to determine the removed value instead. | `CustomEvent<any>`                                                               |
...
| `calciteLookupChange`         | <span style="color:red">**[DEPRECATED]**</span> use `calciteComboboxChange` instead.<br/><br/>Fires when the selected items set changes.                                                                              | `CustomEvent<HTMLCalciteComboboxItemElement[]>`                                  |
