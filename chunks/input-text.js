import { a as x, L as k, d as r, h as p, s as a, x as l, E as i, C as y, c as I } from "./iframe.js";
import { l as w } from "./live.js";
import { e as h, n as d } from "./ref.js";
import { a as E, u as $ } from "./useT9n.js";
import { m as f, g as z } from "./dom.js";
import { c as T, i as b, d as V, s as C, H } from "./form.js";
import { u as F, I as L } from "./interactive.js";
import { c as M, d as O, g as W } from "./label.js";
import { c as P, g as m } from "./component.js";
import { V as q } from "./Validation.js";
import { s as D } from "./input.js";
...
class U extends k {
  ...
  async setFocus() {
    await P(this), this.childEl?.focus();
  }
  connectedCallback() {
    super.connectedCallback(), this.inlineEditableEl = this.el.closest("calcite-inline-editable"), this.inlineEditableEl && (this.editingEnabled = this.inlineEditableEl.editingEnabled || !1), M(this), T(this), this.el.addEventListener(b, this.onHiddenFormInputInput);
  }
  async load() {
  ...
  disconnectedCallback() {
    super.disconnectedCallback(), O(this), V(this), this.el.removeEventListener(b, this.onHiddenFormInputInput);
  }
  get isClearable() {
  ...
  keyDownHandler(e) {
    this.readOnly || this.disabled || e.defaultPrevented || (this.isClearable && e.key === "Escape" && (this.clearInputTextValue(e), e.preventDefault()), e.key === "Enter" && C(this) && e.preventDefault());
  }
  onLabelClick() {
    this.setFocus();
  }
  ...
  setValue({ committing: e = !1, nativeEvent: t, origin: o, previousValue: c, value: s }) {
    this.setPreviousValue(c ?? this.value), this.previousValueOrigin = o, this.userChangedValue = o === "user" && s !== this.value, this.value = s, o === "direct" && (this.setInputValue(s), this.setPreviousEmittedValue(s)), t && (this.calciteInputTextInput.emit().defaultPrevented ? this.value = this.previousValue : e && this.emitChangeIfUserModified());
  }
  render() {
    const e = z(this.el), t = l`<div class=${a(n.loader)}><calcite-progress .label=${this.messages.loading} type=indeterminate></calcite-progress></div>`, o = l`<button .ariaLabel=${this.messages.clear} class=${a(n.clearButton)} .disabled=${this.disabled || this.readOnly} @click=${this.clearInputTextValue} tabindex=-1 type=button><calcite-icon icon=x .scale=${m(this.scale)}></calcite-icon></button>`, c = l`<calcite-icon class=${a(n.inputIcon)} .flipRtl=${this.iconFlipRtl} .icon=${this.requestedIcon} .scale=${m(this.scale)}></calcite-icon>`, s = l`<div class=${a(n.prefix)}>${this.prefixText}</div>`, u = l`<div class=${a(n.suffix)}>${this.suffixText}</div>`, g = l`<input aria-errormessage=${v.validationMessage} .ariaInvalid=${this.status === "invalid"} .ariaLabel=${W(this)} autocomplete=${this.autocomplete ?? i} .autofocus=${this.el.autofocus} class=${a({
    ...
    return L({ disabled: this.disabled, children: l`<div class=${a({ [n.inputWrapper]: !0, [y.rtl]: e === "rtl" })} ${d(this.inputWrapperEl)}>${this.prefixText ? s : null}<div class=${a(n.wrapper)}>${g}${this.isClearable ? o : null}${this.requestedIcon ? c : null}${this.loading ? t : null}</div><div class=${a(n.actionWrapper)} ${d(this.actionWrapperEl)}><slot name=${S.action}></slot></div>${this.suffixText ? u : null}${H({ component: this })}</div>${this.validationMessage && this.status === "invalid" ? q({ icon: this.validationIcon, id: v.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
