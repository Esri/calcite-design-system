import { a as C, L as D, d as v, h as z, s as o, x as c, E as i, C as F, c as P } from "./iframe.js";
import { l as I } from "./live.js";
import { i as L } from "./keyed.js";
import { e as V, n as y } from "./ref.js";
import { i as E, u as U } from "./static.js";
import { a as M, u as A } from "./useT9n.js";
import { f as K, m as N, e as W, g as j } from "./dom.js";
import { c as _, i as H, d as q, s as R, H as G } from "./form.js";
import { u as Y, I as Z } from "./interactive.js";
import { n as J } from "./key.js";
import { c as Q, d as X, g as S } from "./label.js";
import { c as tt, g as x } from "./component.js";
import { i as b, B as et, n as u, p as it, s as rt, h as at } from "./locale.js";
...
class ct extends D {
  ...
  async setFocus() {
    await tt(this), K(this.type === "number" ? this.childNumberEl : this.childEl);
  }
  connectedCallback() {
    super.connectedCallback(), this.inlineEditableEl = this.el.closest("calcite-inline-editable"), this.inlineEditableEl && (this.editingEnabled = this.inlineEditableEl.editingEnabled || !1), Q(this), _(this), this.el.addEventListener(H, this.onHiddenFormInputInput);
  }
  async load() {
  ...
  disconnectedCallback() {
    super.disconnectedCallback(), X(this), q(this), this.el.removeEventListener(H, this.onHiddenFormInputInput);
  }
  get isClearable() {
  ...
  incrementOrDecrementNumberValue(t, e, r, d) {
    ...
    if (a === "Infinity" || a === "-Infinity")
      return;
    const s = t === "up" ? 1 : -1, p = this.step === "any" ? 1 : Math.abs(this.step || 1), l = new et(a !== "" ? a : "0").add(`${p * s}`), h = () => typeof r == "number" && !isNaN(r) && l.subtract(`${r}`).isNegative, f = () => typeof e == "number" && !isNaN(e) && !l.subtract(`${e}`).isNegative, g = h() ? `${r}` : f() ? `${e}` : l.toString();
    this.setValue({
      committing: !0,
      nativeEvent: d,
      origin: "user",
  ...
  nudgeNumberValue(t, e) {
    if (e instanceof KeyboardEvent && e.repeat || this.type !== "number")
      return;
    const r = this.maxString ? parseFloat(this.maxString) : null, d = this.minString ? parseFloat(this.minString) : null, a = 150;
  ...
  setValue({ committing: t = !1, nativeEvent: e, origin: r, previousValue: d, value: a }) {
    if (this.setPreviousValue(d ?? this.value), this.previousValueOrigin = r, this.type === "number") {
      ...
      const s = this.previousValue?.length > a.length || this.value?.length > a.length, p = a.charAt(a.length - 1) === ".", m = p && s ? a : rt(a), l = a && !m ? b(this.previousValue) ? this.previousValue : "" : m;
      let h = u.localize(l);
      r !== "connected" && !p && (h = at(h, l, u)), this.displayedValue = p && s ? `${h}${u.decimal}` : h, this.userChangedValue = r === "user" && this.value !== l, this.value = ["-", "."].includes(l) ? "" : l;
    } else
      this.userChangedValue = r === "user" && this.value !== a, this.value = a;
    r === "direct" && (this.setInputValue(a), this.previousEmittedValue = a), e && (this.calciteInputInput.emit().defaultPrevented ? (this.value = this.previousValue, this.displayedValue = this.type === "number" ? u.localize(this.previousValue) : this.previousValue) : t && this.emitChangeIfUserModified());
  }
  inputKeyUpHandler() {
    window.clearInterval(this.nudgeNumberValueIntervalId);
  }
  ...
  render() {
    const t = j(this.el), e = c`<div class=${o(n.loader)}><calcite-progress .label=${this.messages.loading} type=indeterminate></calcite-progress></div>`, r = c`<button .ariaLabel=${this.messages.clear} class=${o(n.clearButton)} .disabled=${this.disabled || this.readOnly} @click=${this.clearInputValue} tabindex=-1 type=button><calcite-icon icon=x .scale=${x(this.scale)}></calcite-icon></button>`, d = c`<calcite-icon class=${o(n.inputIcon)} .flipRtl=${this.iconFlipRtl} .icon=${this.requestedIcon} .scale=${x(this.scale)}></calcite-icon>`, a = this.numberButtonType === "horizontal", s = c`<button aria-hidden=true class=${o({
    ...
    return Z({ disabled: this.disabled, children: c`<div class=${o({
      [n.inputWrapper]: !0,
      [F.rtl]: t === "rtl",
      [n.hasSuffix]: this.suffixText,
      [n.hasPrefix]: this.prefixText
    })} ${y(this.inputWrapperEl)}>${this.type === "number" && this.numberButtonType === "horizontal" && !this.readOnly ? p : null}${this.prefixText ? l : null}<div class=${o(n.wrapper)}>${B}${O}${this.isClearable ? r : null}${this.requestedIcon ? d : null}${this.loading ? e : null}</div><div class=${o(n.actionWrapper)} ${y(this.actionWrapperEl)}><slot name=${st.action}></slot></div>${this.type === "number" && this.numberButtonType === "vertical" && !this.readOnly ? m : null}${this.suffixText ? h : null}${this.type === "number" && this.numberButtonType === "horizontal" && !this.readOnly ? s : null}${G({ component: this })}</div>${this.validationMessage && this.status === "invalid" ? nt({ icon: this.validationIcon, id: k.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
P("calcite-input", ct);
export {
