...
import { i as b, B as et, n as u, p as it, s as rt, h as nt } from "./locale.js";
import { V as at } from "./Validation.js";
...
class ct extends D {
  ...
  incrementOrDecrementNumberValue(t, e, r, d) {
    ...
    if (n === "Infinity" || n === "-Infinity")
      return;
    const s = t === "up" ? 1 : -1, p = this.step === "any" ? 1 : Math.abs(this.step || 1), l = new et(n !== "" ? n : "0").add(`${p * s}`), h = () => typeof r == "number" && !isNaN(r) && l.subtract(`${r}`).isNegative, f = () => typeof e == "number" && !isNaN(e) && !l.subtract(`${e}`).isNegative, g = h() ? `${r}` : f() ? `${e}` : l.toString();
    this.setValue({
      committing: !0,
      nativeEvent: d,
      origin: "user",
  ...
  setValue({ committing: t = !1, nativeEvent: e, origin: r, previousValue: d, value: n }) {
    if (this.setPreviousValue(d ?? this.value), this.previousValueOrigin = r, this.type === "number") {
      ...
      const s = this.previousValue?.length > n.length || this.value?.length > n.length, p = n.charAt(n.length - 1) === ".", m = p && s ? n : rt(n), l = n && !m ? b(this.previousValue) ? this.previousValue : "" : m;
      let h = u.localize(l);
      r !== "connected" && !p && (h = nt(h, l, u)), this.displayedValue = p && s ? `${h}${u.decimal}` : h, this.userChangedValue = r === "user" && this.value !== l, this.value = ["-", "."].includes(l) ? "" : l;
    } else
  ...
  render() {
    ...
    })} ${y(this.inputWrapperEl)}>${this.type === "number" && this.numberButtonType === "horizontal" && !this.readOnly ? p : null}${this.prefixText ? l : null}<div class=${o(a.wrapper)}>${B}${O}${this.isClearable ? r : null}${this.requestedIcon ? d : null}${this.loading ? e : null}</div><div class=${o(a.actionWrapper)} ${y(this.actionWrapperEl)}><slot name=${st.action}></slot></div>${this.type === "number" && this.numberButtonType === "vertical" && !this.readOnly ? m : null}${this.suffixText ? h : null}${this.type === "number" && this.numberButtonType === "horizontal" && !this.readOnly ? s : null}${G({ component: this })}</div>${this.validationMessage && this.status === "invalid" ? at({ icon: this.validationIcon, id: k.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
P("calcite-input", ct);
export {
