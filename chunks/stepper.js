import { s as r, b as f, x as l, a as v, L as S, d as p, E as m, c as I } from "./iframe.js";
import { n as y } from "./ref.js";
import { b as c, d as A } from "./dom.js";
...
class w extends S {
  ...
  determineActiveStepper() {
    ...
    this.multipleViewMode = i !== "horizontal-single", e.forEach((s, a) => {
      s.itemHidden = i === "horizontal-single" && a !== (t || 0);
    });
  }
  getEnabledStepIndex(e, t = "next") {
    const { items: i, currentActivePosition: s } = this;
    let a = e;
    for (; i[a]?.disabled && this.layout !== "horizontal-single"; )
      a = a + (t === "previous" ? -1 : 1);
    return a !== s && a < i.length && a >= 0 ? a : null;
  }
  updateStep(e) {
  ...
  render() {
    return this.el.ariaLabel = this.messages.label, this.el.role = "region", l`<div class=${r({ container: !0, [o.singleView]: this.layout === "horizontal-single" })} ${y(this.setContainerEl)}>${this.layout === "horizontal-single" && l`<div class=${r({ [o.stepBarContainer]: !0 })}>${this.items.map((e, t) => E({ active: t === this.currentActivePosition, complete: e.complete && t !== this.currentActivePosition && !e.error, disabled: e.disabled && t !== this.currentActivePosition, error: e.error && t !== this.currentActivePosition }))}</div>` || ""}${this.layout === "horizontal-single" && l`<div class=${r({ [o.actionContainer]: !0 })}>${this.renderAction("start")}${this.renderAction("end")}</div>` || ""}<slot @slotchange=${this.handleDefaultSlotChange}></slot></div>`;
  }
  renderAction(e) {
    const t = e === "start", i = t ? "chevron-left" : "chevron-right", { currentActivePosition: s, multipleViewMode: a, layout: u } = this, b = this.items.length, g = `${this.guid}-${t ? "start" : "end"}`;
    return u === "horizontal-single" && !a ? l`<calcite-action alignment=center appearance=transparent class=${r({
      [o.actionIcon]: !0
