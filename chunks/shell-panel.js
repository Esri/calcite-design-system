...
import { g as k, k as b, d as V, s as M } from "./dom.js";
...
class K extends H {
  ...
  handleKeyDown(e) {
    const { key: l, defaultPrevented: r, shiftKey: a } = e, { position: i, layout: t, resizable: n, contentEl: s, el: x, resizeValues: { maxBlockSize: m, maxInlineSize: f, minBlockSize: p, minInlineSize: g } } = this, y = [...t === "horizontal" ? ["ArrowUp", "ArrowDown"] : ["ArrowLeft", "ArrowRight"], "Home", "End"];
    if (!n || !s || r || !y.includes(l))
      return;
    const h = this.getContentElDOMRect(), w = k(x) === "rtl" ? -1 : 1, c = a ? D : R;
    switch (l) {
      case "ArrowUp":
        this.updateSize({
  ...
  async setupInteractions() {
    ...
    const { inlineSize: s, minInlineSize: x, blockSize: m, minBlockSize: f, maxInlineSize: p, maxBlockSize: g } = window.getComputedStyle(l), z = {
      inlineSize: b(s),
      blockSize: b(m),
      minInlineSize: b(x),
      minBlockSize: b(f),
  ...
  render() {
    ...
    })} .hidden=${e} ${I(this.storeContentEl)}>${this.renderHeader()}<div class=${d(o.contentBody)}><slot></slot></div>${s}</div>`)}</div>`, p = [S("action-bar-container", v`<div class=${d(o.actionBarContainer)}><slot name=${E.actionBar} @slotchange=${this.handleActionBarSlotChange}></slot></div>`), m];
