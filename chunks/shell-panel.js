import { a as A, L as H, d as B, s as d, x as v, C as u, n as D, K as R, c as L } from "./iframe.js";
import { i as k } from "./keyed.js";
import { n as I } from "./ref.js";
import { i as $ } from "./interact.min.js";
import { g as S, k as b, d as V, s as M } from "./dom.js";
...
class K extends H {
  constructor() {
    super(...arguments), this.actionBars = [], this.messages = O(), this.resizeValues = {
      ...
      maxInlineSize: null,
      maxBlockSize: null
    }, this.hasHeader = !1, this.collapsed = !1, this.displayMode = "dock", this.layout = "vertical", this.position = "start", this.resizable = !1, this.widthScale = "m", this.calciteInternalShellPanelResizeEnd = B({ cancelable: !1 }), this.calciteInternalShellPanelResizeStart = B({ cancelable: !1 });
  }
  static {
  ...
  handleKeyDown(e) {
    const { key: l, defaultPrevented: r, shiftKey: a } = e, { position: i, layout: t, resizable: n, contentEl: s, el: x, resizeValues: { maxBlockSize: m, maxInlineSize: f, minBlockSize: p, minInlineSize: y } } = this, g = [...t === "horizontal" ? ["ArrowUp", "ArrowDown"] : ["ArrowLeft", "ArrowRight"], "Home", "End"];
    if (!n || !s || r || !g.includes(l))
      return;
    const h = this.getContentElDOMRect(), w = S(x) === "rtl" ? -1 : 1, c = a ? D : R;
    switch (l) {
      case "ArrowUp":
        this.updateSize({
      ...
      case "ArrowLeft":
        this.updateSize({
          size: h.width + (t === "vertical" && i === "end" ? c : -c) * w,
          type: "inlineSize"
        }), e.preventDefault();
        break;
      case "ArrowRight":
      ...
      case "Home":
        this.updateSize({
          size: t === "horizontal" ? p : y,
          type: t === "horizontal" ? "blockSize" : "inlineSize"
        }), e.preventDefault();
        break;
      case "End":
        this.updateSize({
          size: t === "horizontal" ? m : f,
          type: t === "horizontal" ? "blockSize" : "inlineSize"
        }), e.preventDefault();
        break;
    }
  ...
  updateSize({ type: e, size: l }) {
    const { contentEl: r, resizeValues: a } = this;
    if (!r)
      return;
  ...
  async setupInteractions() {
    ...
    const { el: e, contentEl: l, resizable: r, position: a, collapsed: i, resizeHandleEl: t, layout: n } = this;
    if (!l || i || !r || !t)
      return;
    await this.el.componentOnReady();
    const { inlineSize: s, minInlineSize: x, blockSize: m, minBlockSize: f, maxInlineSize: p, maxBlockSize: y } = window.getComputedStyle(l), z = {
      inlineSize: b(s),
      blockSize: b(m),
      minInlineSize: b(x),
      minBlockSize: b(f),
      maxInlineSize: b(p) || window.innerWidth,
      maxBlockSize: b(y) || window.innerHeight
    };
    this.resizeValues = z;
    const g = S(e) === "rtl";
    this.interaction = $(l, { context: e.ownerDocument }).resizable({
      edges: {
        top: a === "end" && n === "horizontal" ? t : !1,
        right: a === (g ? "end" : "start") && n === "vertical" ? t : !1,
        bottom: a === "start" && n === "horizontal" ? t : !1,
        left: a === (g ? "start" : "end") && n === "vertical" ? t : !1
      },
      modifiers: [
      ...
      listeners: {
        ...
        move: ({ rect: h }) => {
          const w = n === "horizontal";
          this.updateSize({
            size: w ? h.height : h.width,
            type: w ? "blockSize" : "inlineSize"
          });
        }
      }
    });
  ...
  handleActionBarSlotChange(e) {
    const l = V(e).filter((r) => r?.matches("calcite-action-bar"));
  ...
  render() {
    ...
    })} .hidden=${e} ${I(this.storeContentEl)}>${this.renderHeader()}<div class=${d(o.contentBody)}><slot></slot></div>${s}</div>`)}</div>`, p = [k("action-bar-container", v`<div class=${d(o.actionBarContainer)}><slot name=${E.actionBar} @slotchange=${this.handleActionBarSlotChange}></slot></div>`), m];
