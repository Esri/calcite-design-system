import { h as T, L as O, k as I, n as E, i as z, s as o, E as p, x as m, j as R } from "./iframe.js";
import { n as u } from "./ref.js";
import { l as S, d as l, b as D, a as c, m as B, o as $, i as L } from "./date.js";
import { f as N } from "./dom.js";
import { d as W, r as H, c as y, b as M, f as U, h as Y, F as A } from "./floating-ui.js";
import { c as _, d as q, s as j, H as G } from "./form.js";
import { u as K, I as Z } from "./interactive.js";
import { n as F } from "./key.js";
import { c as J, d as Q } from "./label.js";
import { c as X, g as C } from "./component.js";
import { n as b, a as tt, j as et } from "./locale.js";
import { o as at } from "./openCloseComponent.js";
import { g as x, a as it } from "./utils2.js";
import { d as w, b as k, e as st } from "./focusTrapComponent.js";
import { g as V } from "./guid.js";
import { V as nt } from "./Validation.js";
import { s as rt } from "./input.js";
import { u as ot } from "./useT9n.js";
...
class dt extends O {
  ...
  // #region Lifecycle
  constructor() {
    super(), this.commonDateSeparators = [".", "-", "/"], this.dialogId = `date-picker-dialog--${V()}`, this.focusOnOpen = !1, this.focusTrapDeactivates = () => {
      this.open = !1;
    }, this.transitionProp = "opacity", this.placeholderTextId = `calcite-input-date-picker-placeholder-${V()}`, this.rangeStartValueChangedByUser = !1, this.userChangedValue = !1, this._value = "", this.valueAsDateChangedExternally = !1, this.focusedInput = "start", this.disabled = !1, this.focusTrapDisabled = !1, this.layout = "horizontal", this.messages = ot({ blocking: !0 }), this.monthStyle = "wide", this.open = !1, this.overlayPositioning = "absolute", this.placement = W, this.proximitySelectionDisabled = !1, this.range = !1, this.readOnly = !1, this.required = !1, this.scale = "m", this.status = "idle", this.validity = {
      valid: !1,
      badInput: !1,
      customError: !1,
      patternMismatch: !1,
  ...
  connectedCallback() {
    ...
    J(this), _(this), this.setFilteredPlacements(), b.numberFormatOptions = {
  ...
  disconnectedCallback() {
    super.disconnectedCallback(), w(this), Q(this), q(this), M(this);
  }
  // #endregion
  // #region Private Methods
  handleFocusTrapDisabled(t) {
    this.open && (t ? w(this) : k(this));
  }
  ...
  openHandler() {
    at(this), !(this.disabled || this.readOnly) && this.reposition(!0);
  }
  calciteInternalInputInputHandler(t) {
    const e = t.target, a = e.value, i = this.parseNumerals(a), s = this.formatNumerals(i);
    e.value = s;
    const { year: r } = B(a, this.localeData);
    if (r && r.length < 4)
      return;
    const h = $(a, this.localeData);
    L(h, this.min, this.max) && (this.datePickerActiveDate = h);
  }
  ...
  handleDateTimeFormatChange() {
    ...
    this.dateTimeFormat = new Intl.DateTimeFormat(et(this.messages._lang), t);
  }
  setReferenceEl() {
  ...
  setFilteredPlacements() {
    ...
    this.filteredFlipPlacements = e ? U(e, t) : null;
  }
  setTransitionEl(t) {
  ...
  onClose() {
    this.calciteInputDatePickerClose.emit(), Y(this), w(this), this.focusOnOpen = !1, this.datePickerEl?.reset();
  }
  ...
  commitValue() {
    const { focusedInput: t, value: e } = this, a = `${t}Input`, i = this[a].value, s = $(i, this.localeData), r = c(s), h = Array.isArray(e);
    if (this.range) {
      const d = t === "start" ? 0 : 1;
      if (h) {
        if (r === e[d])
  ...
  keyDownHandler(t) {
    ...
    a === "Enter" ? (t.preventDefault(), this.commitValue(), this.shouldFocusRangeEnd() ? this.endInput?.setFocus() : this.shouldFocusRangeStart() && this.startInput?.setFocus(), j(this) && this.restoreInputFocus(!0)) : (a === "ArrowDown" || a === "ArrowUp") && !i ? (this.open = !0, this.focusOnOpen = !0, t.preventDefault()) : this.open && a === "Escape" && (this.open = !1, t.preventDefault(), this.restoreInputFocus(!0));
  }
  startInputFocus() {
  ...
  async loadLocaleData() {
    z() && (b.numberFormatOptions = {
      numberingSystem: this.numberingSystem,
      locale: this.messages._lang,
      useGrouping: !1
    }, this.localeData = await it(this.messages._lang), this.localizeInputValues());
  }
  /**
  ...
  formatNumerals(t) {
    return t ? t.split("").map((e) => this.commonDateSeparators?.includes(e) ? this.localeData?.separator : F?.includes(e) ? b?.numberFormatter?.format(Number(e)) : e).join("") : "";
  }
  parseNumerals(t) {
    return t ? t.split("").map((e) => F.includes(e) ? b.delocalize(e) : e).join("") : "";
  }
  getNormalizedDate(t) {
    if (!t)
      return "";
    if (!ct(t))
      return t;
    const { day: e, month: a, year: i } = S(t);
    return `${ht(Number(i))}-${a}-${e}`;
  }
  focusInput() {
  ...
  render() {
    ...
    return b.numberFormatOptions = {
      ...
      locale: e,
      useGrouping: !1
    }, Z({ disabled: this.disabled, children: m`${this.localeData && m`<div class=${o(n.container)}><div class=${o(n.inputContainer)}><div class=${o(n.inputWrapper)} data-position=start @click=${this.onInputWrapperClick} @pointerdown=${this.onInputWrapperPointerDown} ${u(this.setStartWrapper)}><calcite-input-text aria-controls=${this.dialogId ?? p} aria-describedby=${this.placeholderTextId ?? p} aria-errormessage=${P.validationMessage} aria-autocomplete=none .ariaExpanded=${this.open} aria-haspopup=dialog .ariaInvalid=${this.status === "invalid"} class=${o({
      [n.input]: !0,
      [n.inputNoBottomBorder]: this.layout === "vertical" && this.range,
      [n.inputNoRightBorder]: this.range
    })} .disabled=${t} icon=calendar @calciteInputTextInput=${this.calciteInternalInputInputHandler} @calciteInternalInputTextBlur=${this.calciteInternalInputBlurHandler} @calciteInternalInputTextFocus=${this.startInputFocus} .placeholder=${this.localeData?.placeholder} .readOnly=${s} role=combobox .scale=${this.scale} .status=${this.status} ${u(this.setStartInput)}></calcite-input-text>${!this.readOnly && !this.range && this.renderToggleIcon(this.open && this.focusedInput === "start") || ""}<span aria-hidden=true class=${o(n.assistiveText)} id=${this.placeholderTextId ?? p}>${a.dateFormat.replace("{format}", this.localeData?.placeholder)}</span></div><div .ariaHidden=${!this.open} .ariaLabel=${a.chooseDate} aria-live=polite aria-modal=true class=${o(n.menu)} id=${this.dialogId ?? p} role=dialog ${u(this.setFloatingEl)}><div class=${o({
      [n.calendarWrapper]: !0,
      [A.animation]: !0,
      [A.animationActive]: this.open
    })} ${u(this.setTransitionEl)}><calcite-date-picker .activeDate=${this.datePickerActiveDate} .activeRange=${this.focusedInput} .headingLevel=${this.headingLevel} .layout=${this.layout} .max=${this.max} .maxAsDate=${this.maxAsDate} .messageOverrides=${this.messageOverrides} .min=${this.min} .minAsDate=${this.minAsDate} .monthStyle=${this.monthStyle} .numberingSystem=${i} @calciteDatePickerChange=${this.handleDateChange} @calciteDatePickerRangeChange=${this.handleDateRangeChange} .proximitySelectionDisabled=${this.proximitySelectionDisabled} .range=${this.range} .scale=${this.scale} tabindex=${(this.open ? void 0 : -1) ?? p} .valueAsDate=${this.valueAsDate} ${u(this.setDatePickerRef)}></calcite-date-picker></div></div>${this.range && m`<div class=${o(n.dividerContainer)}><div class=${o(n.divider)}></div></div>` || ""}${this.range && m`<div class=${o(n.inputWrapper)} data-position=end @click=${this.onInputWrapperClick} @pointerdown=${this.onInputWrapperPointerDown} ${u(this.setEndWrapper)}><calcite-input-text aria-controls=${this.dialogId ?? p} aria-autocomplete=none .ariaExpanded=${this.open} aria-haspopup=dialog class=${o({
      [n.input]: !0,
...
R("calcite-input-date-picker", dt);
export {
  dt as InputDatePicker
};
