import { a as V, L as S, d as I, h as O, s as o, E as p, x as m, c as T } from "./iframe.js";
...
class ct extends S {
  ...
  render() {
    ...
    })} .disabled=${t} icon=calendar .label=${this.label} @calciteInputTextInput=${this.calciteInternalInputInputHandler} @calciteInternalInputTextBlur=${this.calciteInternalInputBlurHandler} @calciteInternalInputTextFocus=${this.startInputFocus} .placeholder=${this.localeData?.placeholder} .readOnly=${s} role=combobox .scale=${this.scale} .status=${this.status} ${d(this.setStartInput)}></calcite-input-text>${!this.readOnly && !this.range && this.renderToggleIcon(this.open && this.focusedInput === "start") || ""}<span aria-hidden=true class=${o(n.assistiveText)} id=${this.placeholderTextId ?? p}>${a.dateFormat.replace("{format}", this.localeData?.placeholder)}</span></div><div .ariaHidden=${!this.open} .ariaLabel=${a.chooseDate} aria-live=polite aria-modal=true class=${o(n.menu)} id=${this.dialogId ?? p} role=dialog ${d(this.setFloatingEl)}><div class=${o({
      [n.calendarWrapper]: !0,
      [$.animation]: !0,
