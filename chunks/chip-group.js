...
class S extends o {
  ...
  updateItems(e) {
    const s = this.slotRefEl.value?.assignedElements({ flatten: !0 }).filter((t) => t?.matches("calcite-chip"));
  ...
  render() {
    ...
    return u({ disabled: s, children: r`<div .ariaLabel=${this.label} class="container" .role=${e}><slot @slotchange=${this.updateItems} ${p(this.slotRefEl)}></slot></div>` });
  }
}
