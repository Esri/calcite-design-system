import { j as h, L as p, w as f, x as o, s as d, q as z, h as u, E as k, k as x } from "./iframe.js";
...
class $ extends p {
  ...
  render() {
    ...
    return this.el.ariaLabel = l, this.el.ariaValueMax = a ? "100" : void 0, this.el.ariaValueMin = a ? "0" : void 0, this.el.ariaValueNow = a ? c.toString() : void 0, f(this.el, "id", s), this.el.role = "progressbar", o`<div class=${d(t.rings)}>${this.renderRing("track")}${this.renderRing("progress")}${!r && a && o`<div class=${d(t.percentage)}>${this.formatValue()}</div>` || ""}</div>${!r && n && o`<div class=${d(t.text)}>${n}</div>` || ""}`;
  }
  renderRing(e) {
    ...
    })} style=${z(a)} viewBox=${`0 0 ${i} ${i}`}>${u`<circle cx=${i / 2} cy=${i / 2} r=${s ?? k} />`}</svg>`;
  }
}
x("calcite-loader", $);
export {
