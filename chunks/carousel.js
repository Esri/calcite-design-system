...
import { a as F, L as M, c as g, A as d, s as r, b as l, d as L } from "./index.js";
import { c as U } from "./repeat.js";
import { e as b, n as y } from "./ref.js";
import { u as B } from "./index2.js";
import { A as W, d as K, b as v } from "./dom.js";
...
class Q extends M {
  ...
  renderRotationControl() {
    ...
    return l`<button .ariaLabel=${e} class=${r({
      [a.paginationItem]: !0,
      [a.autoplayControl]: !0
    })} @click=${this.toggleRotation} title=${e ?? d}><calcite-icon .icon=${this.playing ? p.pause : p.play} scale=s></calcite-icon>${this.playing && l`<calcite-progress class=${r(a.autoplayProgress)} .label=${this.messages.carouselItemProgress} .value=${t}></calcite-progress>` || ""}</button>`;
  }
  ...
  renderPaginationItems() {
    ...
    return l`<div .ariaLabel=${n} class=${r(a.paginationItems)} role=tablist>${U(i, (s) => s.id, (s, o) => {
      const h = i.length, u = o === e, z = o === 0, A = o === h - 1, w = h - t - 1, f = e < t, D = e >= w, $ = f ? 0 : e - Math.floor(t / 2), O = D ? h : $ + t, R = f ? 0 : D ? w : $, C = f ? t + 1 : O, H = !z && !A && !u && (o === R - 1 || o === C), S = u || o <= C && o >= R - 1, P = h - 1 <= t, E = u ? p.active : p.inactive;
      return l`<button aria-controls=${(u ? void 0 : s.id) ?? d} .ariaSelected=${u} class=${r({
        [a.paginationItem]: !0,
      ...
      })} data-index=${o ?? d} @click=${c} role=tab title=${s.label ?? d}><calcite-icon .icon=${E} scale=l></calcite-icon></button>`;
    })}</div>`;
  }
  renderPaginationAriaLive() {
    ...
    if (!e._loading)
      return I.numberFormatOptions = {
        locale: t
      }, l`<div aria-live=off class=${r(a.paginationAriaLive)} role=status>${e.paginationStatus.replace("{current}", I.localize(`${i + 1}`)).replace("{total}", I.localize(`${n.length}`))}</div>`;
  }
  renderArrow(e) {
    ...
    return l`<button aria-controls=${this.containerId ?? d} class=${r({ [a.paginationItem]: !0, [c]: !0 })} data-item-direction=${e ?? d} @click=${this.handleArrowClick} title=${s ?? d}><calcite-icon .flipRtl=${i === "rtl"} .icon=${o} .scale=${n}></calcite-icon></button>`;
  }
  render() {
    ...
    })} id=${this.containerId ?? d} ${y(this.itemContainerRef)}><slot @slotchange=${this.handleSlotChange}></slot></section>`;
