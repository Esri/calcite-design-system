...
class ee extends M {
  constructor() {
    ...
    }, this.containerRef = b(), this.containerId = Q.host(j()), this.direction = W(), this.itemContainerRef = b(), this.resizeHandler = ({ contentRect: { width: e } }) => {
      this.setMaxItemsToBreakpoint(e);
    }, this.resizeObserver = q("resize", (e) => e.forEach(this.resizeHandler)), this.tabListRef = b(), this.timer = () => {
  ...
  async itemDirectionWatcher(e) {
    e === "standby" || !this.itemContainerRef.value || (await K(this.itemContainerRef.value, e === "forward" ? "item-forward" : "item-backward"), this.itemDirection = "standby");
  }
  suspendWatcher() {
  ...
  setSelectedItem(e, t) {
    ...
    this.items.forEach((n, c) => {
      const s = e === c;
      n.selected = s, s && (this.selectedItem = n, this.selectedIndex = c);
  ...
  renderRotationControl() {
    ...
    return l`<button .ariaLabel=${e} class=${r({
      [a.paginationItem]: !0,
      [a.autoplayControl]: !0
    })} @click=${this.toggleRotation} title=${e ?? u}><calcite-icon .icon=${this.playing ? p.pause : p.play} scale=s></calcite-icon>${this.playing && l`<calcite-progress class=${r(a.autoplayProgress)} .label=${this.messages.carouselItemProgress} .value=${t}></calcite-progress>` || ""}</button>`;
  }
  renderPaginationArea() {
    return l`<div class=${r({
      [a.pagination]: !0,
  ...
  renderPaginationItems() {
    ...
    return l`<div .ariaLabel=${n} class=${r(a.paginationItems)} role=tablist>${B(i, (s) => s.id, (s, o) => {
      const h = i.length, d = o === e, z = o === 0, O = o === h - 1, D = h - t - 1, f = e < t, w = e >= D, $ = f ? 0 : e - Math.floor(t / 2), A = w ? h : $ + t, R = f ? 0 : w ? D : $, C = f ? t + 1 : A, H = !z && !O && !d && (o === R - 1 || o === C), S = d || o <= C && o >= R - 1, P = h - 1 <= t, E = d ? p.active : p.inactive;
      return l`<button aria-controls=${(d ? void 0 : s.id) ?? u} .ariaSelected=${d} class=${r({
        [a.paginationItem]: !0,
      ...
      })} data-index=${o ?? u} @click=${c} role=tab title=${s.label ?? u}><calcite-icon .icon=${E} scale=l></calcite-icon></button>`;
    })}</div>`;
  }
  renderPaginationAriaLive() {
    const { messages: e, messages: { _lang: t }, selectedIndex: i, items: n } = this;
    if (!e._loading)
      return k.numberFormatOptions = {
        locale: t
      }, l`<div aria-live=off class=${r(L.screenReaderText)} role=status>${e.paginationStatus.replace("{current}", k.localize(`${i + 1}`)).replace("{total}", k.localize(`${n.length}`))}</div>`;
  }
  renderArrow(e) {
    const t = e === "previous", i = this.direction, n = this.arrowType === "edge" ? "m" : "s", c = t ? a.pagePrevious : a.pageNext, s = t ? this.messages.previous : this.messages.next, o = t ? p.chevronLeft : p.chevronRight;
    return l`<button aria-controls=${this.containerId ?? u} class=${r({ [a.paginationItem]: !0, [c]: !0 })} data-item-direction=${e ?? u} @click=${this.handleArrowClick} title=${s ?? u}><calcite-icon .flipRtl=${i === "rtl"} .icon=${o} .scale=${n}></calcite-icon></button>`;
  }
  render() {
    const { itemDirection: e, paginationPosition: t } = this, i = this.renderPaginationArea(), n = l`<section class=${r({
      [a.itemContainer]: !0,
    ...
    return this.interactiveContainer({ disabled: this.disabled, children: l`<div .ariaLabel=${this.label} .ariaLive=${this.playing ? "off" : "polite"} .ariaRoleDescription=${this.messages.carousel} class=${r({
      [a.container]: !0,
      [a.containerOverlaid]: this.controlOverlay,
