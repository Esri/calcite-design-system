...
import { a as O, L as Y, n as a, s as x, b as G, w as y, d as Z } from "./index.js";
...
function H({ width: t, height: e, min: i, max: r }) {
...
function R({ data: t, min: e, max: i, t: r }) {
  ...
  const p = t.reduce((M, b, u) => {
    if (l = t[u - 2], d = t[u - 1], u > 1) {
      const g = B(l, d, b), f = s === void 0 ? C(l, d, g) : s, L = P(l, d, f, g, r);
      return s = g, `${M} ${L}`;
    }
    return M;
...
const E = O`:host{display:block;block-size:100%}.svg{fill:currentColor;stroke:transparent;margin:0;display:block;block-size:100%;inline-size:100%;padding:0}.svg .graph-path--highlight{fill:var(--calcite-graph-highlight-fill-color, var(--calcite-color-brand));opacity:.5}:host([hidden]){display:none}[hidden]{display:none}`, v = {
  svg: "svg",
...
class N extends Y {
  constructor() {
    super(...arguments), this.graphId = j(), this.resizeObserver = A("resize", () => this.requestUpdate()), this.data = [];
  }
  static {
  ...
  render() {
    ...
    const { min: d, max: p } = I(e);
    let w = d, k = p;
    (c < d[0] || c > d[0]) && (w = [c, 0]), (h > p[0] || h < p[0]) && (k = [h, p[1]]);
    const M = H({ min: w, max: k, width: l, height: s }), b = R({ data: e, min: d, max: p, t: M }), u = i ? `url(#${m.linearGradient(n)})` : void 0;
    if ($ !== void 0 && o !== void 0) {
