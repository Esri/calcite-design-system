import { h as S, L as j, s as b, y as $, x as w, d as p, j as O } from "./iframe.js";
...
function B({ data: e, min: s, max: a, t: n }) {
  ...
  const d = e.reduce((x, M, m) => {
    ...
    return x;
  }, `M ${o},${r} L ${o},${c} L ${h},${c}`), k = e[e.length - 1], f = C(g, k, t, z(g, k, t), n);
  return `${d} ${f} L ${i},${r} Z`;
}
const E = S`:host{display:block;block-size:100%}.svg{fill:currentColor;stroke:transparent;margin:0;display:block;block-size:100%;inline-size:100%;padding:0}.svg .graph-path--highlight{fill:var(--calcite-graph-highlight-fill-color, var(--calcite-color-brand));opacity:.5}:host([hidden]){display:none}[hidden]{display:none}`, y = {
  svg: "svg",
...
class I extends j {
  ...
  render() {
    ...
    if (!s || s.length === 0)
      return w`<svg aria-hidden=true class=${b(y.svg)} height=${t ?? $} preserveAspectRatio=none viewBox=${`0 0 ${l} ${t}`} width=${l ?? $}></svg>`;
    const { min: g, max: d } = A(s);
    let k = g, f = d;
    (o < g[0] || o > g[0]) && (k = [o, 0]), (r > d[0] || r < d[0]) && (f = [r, d[1]]);
    const x = H({ min: k, max: f, width: l, height: t }), [M] = x([c, f[1]]), [m] = x([h, f[1]]), u = B({ data: s, min: g, max: d, t: x }), v = a ? `url(#linear-gradient-${i})` : void 0;
    return w`<svg aria-hidden=true class=${b(y.svg)} height=${t ?? $} preserveAspectRatio=none viewBox=${`0 0 ${l} ${t}`} width=${l ?? $}>${a ? p`<defs><linearGradient .id=${`linear-gradient-${i}`} x1=0 x2=1 y1=0 y2=0>${a.map(({ offset: L, color: P, opacity: X }) => p`<stop offset=${`${L * 100}%`} stop-color=${P ?? $} stop-opacity=${X ?? $} />`)}</linearGradient></defs>` : null}${c !== void 0 ? [
      ...
      p`<mask height=100% .id=${`${i}3`} width=100% x=0% y=0%><path d=${`
                ...
                Z
              `} fill=white /></mask>`,
      p`<path class=${b(y.graphPath)} d=${u ?? $} fill=${v ?? $} mask=${`url(#${i}1)`} />`,
      p`<path class=${b(y.graphPathHighlight)} d=${u ?? $} fill=${v ?? $} mask=${`url(#${i}2)`} />`,
      p`<path class=${b(y.graphPath)} d=${u ?? $} fill=${v ?? $} mask=${`url(#${i}3)`} />`
    ] : p`<path class=${b(y.graphPath)} d=${u ?? $} fill=${v ?? $} />`}</svg>`;
  }
}
O("calcite-graph", I);
export {
