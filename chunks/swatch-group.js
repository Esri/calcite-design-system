...
class w extends l {
  constructor() {
    super(), this.items = [], this.slotRefEl = h(), this.focusSetter = f()(this), this.disabled = !1, this.scale = "m", this.selectedItems = [], this.selectionMode = "none", this.calciteSwatchGroupSelect = o({ cancelable: !1 }), this.listen("calciteInternalSwatchKeyEvent", this.calciteInternalSwatchKeyEventListener), this.listen("calciteSwatchSelect", this.calciteSwatchSelectListener), this.listen("calciteInternalSwatchSelect", this.calciteInternalSwatchSelectListener), this.listen("calciteInternalSyncSelectedSwatches", this.calciteInternalSyncSelectedSwatches);
  }
  static {
  ...
  updateItems(e) {
    const s = this.slotRefEl.value?.assignedElements({ flatten: !0 }).filter((t) => t?.matches("calcite-swatch"));
  ...
  render() {
    ...
    return S({ disabled: s, children: n`<div .ariaLabel=${this.label} class=${r(g.container)} .role=${e}><slot @slotchange=${this.updateItems} ${p(this.slotRefEl)}></slot></div>` });
  }
}
