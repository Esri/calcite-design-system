...
class Y extends P {
  ...
  getPreviousMonthDays(e, a, t) {
    const s = new Date(a, e, 0), i = s.getDate(), n = s.getDay(), c = [];
    if (n === (t + w) % m)
      return c;
    if (n === t)
      return [i];
    for (let o = (m + n - t) % m; o >= 0; o--)
      c.push(i - o);
    return c;
  }
  ...
  isHoverInRange() {
    if (!this.hoverRange || !this.startDate)
      return !1;
    const { start: e, end: a } = this.hoverRange, t = this.isFocusedOnStart(), s = this.startDate && a > this.startDate, i = this.endDate && a < this.endDate, n = this.startDate && e > this.startDate, c = this.endDate && e < this.endDate, o = !t && this.startDate && s && (!this.endDate || i), h = t && this.startDate && n && c;
    return o || h;
  }
  isRangeHover(e) {
    if (!this.hoverRange)
      return !1;
    const { start: a, end: t } = this.hoverRange, s = this.isFocusedOnStart(), i = this.isHoverInRange(), n = e > a && e < this.startDate, c = e < t && e > this.endDate, o = e > t && e < this.endDate, h = e < a && e > this.startDate, g = e < t && e > this.startDate, r = e > a && e < this.endDate, D = this.startDate && this.endDate;
    if (i) {
      if (D)
        return s ? e < this.endDate && (h || n) : o || c;
  ...
  getDays(e, a, t, s = "start") {
    ...
    i = s === "end" ? n : i;
    let c = 0;
    const o = () => c++ % 7, h = this.activeDate.getFullYear();
    return [
      ...e.map((r) => ({
        active: !1,
        day: r,
        dayInWeek: o(),
        date: new Date(h, i - 1, r)
      })),
      ...a.map((r) => {
        const D = new Date(h, i, r), f = l(D, /* @__PURE__ */ new Date());
        return {
  ...
  render() {
    ...
    const f = e + 1, M = this.getPreviousMonthDays(f, a, t), x = this.getCurrentMonthDays(f, a), R = this.getNextMonthDays(f, a, t), A = this.getDays(g, h, r), I = this.getDays(M, x, R, "end");
    return y`<div class=${u({ [d.calendarContainer]: !0 })} role=grid>${this.renderCalendar(o, A)}${this.range && this.renderCalendar(o, I, !0) || ""}</div>`;
  }
  renderDateDay({ active: e, currentMonth: a, currentDay: t, date: s, day: i, dayInWeek: n, ref: c }, o) {
    const h = S(s, this.min, this.max);
    ...
    })} .currentMonth=${a} .dateTimeFormat=${this.dateTimeFormat} .day=${i} .disabled=${!h} .endOfRange=${this.isEndOfRange(s)} .highlighted=${this.betweenSelectedRange(s)} @calciteInternalDayHover=${this.dayHover} @calciteInternalDaySelect=${this.daySelect} .range=${!!this.startDate && !!this.endDate && !l(this.startDate, this.endDate)} .rangeEdge=${n === 0 ? "start" : n === 6 ? "end" : void 0} .rangeHover=${h && this.isRangeHover(s)} .scale=${this.scale} .selected=${this.isSelected(s)} .startOfRange=${this.isStartOfRange(s)} .value=${s} ${E((g) => {
  ...
  renderMonthCalendar(e, a, t = !1) {
    return y`<div class=${u({ [d.month]: !0 })} @keydown=${this.keyDownHandler}><div class=${u({ [d.weekHeaderContainer]: !0 })} role=row>${e.map((i) => y`<span class=${u({ [d.weekHeader]: !0 })} role=columnheader>${i}</span>`)}</div><div class=${u({ [d.weekDays]: !0 })} role=row>${a.map((i, n) => this.renderDateDay(i, t ? 50 + n : n))}</div></div>`;
  }
}
