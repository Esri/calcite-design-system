...
import { a as P, L as H, c as f, s as c, A as F, b as n, B as I, C as E, d as M } from "./index.js";
import { e as p, n as h } from "./ref.js";
import { n as C } from "./locale.js";
import { u as U } from "./useT9n.js";
import { i as L } from "./resources29.js";
...
class j extends H {
  ...
  async load() {
    this.readCellContentsToAT = /safari/i.test(V()), this.listenOn(this.el.shadowRoot, "slotchange", this.handleSlotChange);
  }
  ...
  calciteInternalTableRowFocusEvent(e) {
    const t = e.detail.cellPosition, a = e.detail.rowPosition, o = e.detail.destination, r = e.detail.lastCell, i = this.bodyRows?.filter((b) => !v(b)), s = this.allRows?.filter((b) => !v(b)), w = this.headRows[this.headRows.length - 1]?.positionAll, R = i[0]?.positionAll, S = i[i.length - 1]?.positionAll, y = this.footRows[0]?.positionAll, z = s[s.length - 1]?.positionAll, T = o === "next" && a === w, k = o === "previous" && a === y, x = o === "previous" && a === R, A = o === "next" && a === S;
    let d;
    switch (o) {
      case "first":
        d = 0;
  ...
  getSlottedRows(e) {
    return e ? e.assignedElements({ flatten: !0 }).filter(L) : [];
  }
  ...
  render() {
    ...
    this.selectionMode === "multiple" ? "true" : void 0} .ariaRowCount=${this.allRows?.length} class=${c({ [l.tableFixed]: this.layout === "fixed" })} .role=${this.interactionMode === "interactive" ? "grid" : "table"} ${h((e) => {
      e && (this.tableEl = e, I(n`<caption class=${c(E.screenReaderText)}>${this.caption}</caption>${this.renderTHead()}${this.renderTBody()}${this.renderTFoot()}`, e));
