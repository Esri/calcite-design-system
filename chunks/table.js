import { b as I, L as P, c as g, s, E as M, x as n, I as F, q as O } from "./index.js";
...
class D extends P {
  ...
  calciteInternalTableRowFocusEvent(e) {
    const i = e.detail.cellPosition, o = e.detail.rowPosition, a = e.detail.destination, r = e.detail.lastCell, t = this.bodyRows?.filter((h) => !m(h)), c = this.allRows?.filter((h) => !m(h)), f = this.headRows[this.headRows.length - 1]?.positionAll, w = t[0]?.positionAll, S = t[t.length - 1]?.positionAll, R = this.footRows[0]?.positionAll, x = c[c.length - 1]?.positionAll, z = a === "next" && o === f, T = a === "previous" && o === R, A = a === "previous" && o === w, k = a === "next" && o === S;
    let d;
    switch (a) {
      case "first":
        d = 0;
    ...
    const y = this.allRows?.find((h) => h.positionAll === d)?.cellCount, E = i > y ? y : i;
  ...
  updateRows() {
    const e = this.getSlottedRows(this.tableHeadSlotEl.value) || [], i = this.getSlottedRows(this.tableBodySlotEl.value) || [], o = this.getSlottedRows(this.tableFootSlotEl.value) || [], a = [...e, ...i, ...o];
    e?.forEach((t) => {
      const c = e?.indexOf(t);
  ...
  renderSelectionArea() {
    const e = this._selectedItems?.filter((t) => m(t))?.length, i = this.localizeNumber(e?.toString()), a = `${this.localizeNumber(this.selectedCount?.toString())} ${this.messages.selected}`, r = `${i} ${this.messages.hiddenSelected}`;
    ...
    })} .kind=${this.selectedCount > 0 ? "brand" : "neutral"} .label=${a} .scale=${this.scale} .value=${a}>${a}</calcite-chip>${e > 0 && n`<calcite-chip class=${s(l.selectionOutOfViewChip)} .icon=${C.hideEmpty} .label=${r} .scale=${this.scale} title=${r ?? M} .value=${r}>${i}</calcite-chip>` || ""}${this.selectedCount > 0 && n`<calcite-button class=${s(l.dismissButton)} .iconStart=${C.clear} kind=neutral @click=${this.handleDeselectAllRows} round .scale=${this.scale} .title=${`${this.messages.clear} ${a} ${this.messages.row}`}>${this.messages.clear}</calcite-button>` || ""}<div class=${s(l.selectionActions)}><slot name=${v.selectionActions}></slot></div></div>`;
  }
  renderPaginationArea() {
    return n`<div class=${s(l.paginationArea)}><calcite-pagination .groupSeparator=${this.groupSeparator} .numberingSystem=${this.numberingSystem} @calcitePaginationChange=${this.handlePaginationChange} .pageSize=${this.pageSize} .scale=${this.scale} .startItem=${this.pageStartRow} .totalItems=${this.bodyRows?.length} ${b(this.paginationEl)}></calcite-pagination></div>`;
  }
  renderTHead() {
    return n`<thead><slot name=${v.tableHeader} ${b(this.tableHeadSlotEl)}></slot></thead>`;
  }
  renderTBody() {
    return n`<tbody><slot ${b(this.tableBodySlotEl)}></slot></tbody>`;
  }
  renderTFoot() {
    return n`<tfoot><slot name=${v.tableFooter} ${b(this.tableFootSlotEl)}></slot></tfoot>`;
  }
  render() {
    ...
    this.selectionMode === "multiple" ? "true" : null} .ariaRowCount=${this.allRows?.length} class=${s({ [l.tableFixed]: this.layout === "fixed" })} .role=${this.interactionMode === "interactive" ? "grid" : "table"} ${b((e) => {
      e && F(n`<caption class=${s(l.assistiveText)}>${this.caption}</caption>${this.renderTHead()}${this.renderTBody()}${this.renderTFoot()}`, e);
