import { a as I, L as F, d as g, s as d, E as P, x as l, B as M, c as k } from "./iframe.js";
import { e as u, n as p } from "./ref.js";
import { n as v } from "./locale.js";
...
function L() {
  const b = H();
  return b?.brands ? b.brands.map(({ brand: e, version: i }) => `${e}/${i}`).join(" ") : navigator.userAgent;
}
const n = {
  bordered: "bordered",
  striped: "striped",
...
class B extends F {
  ...
  calciteInternalTableRowFocusEvent(e) {
    const i = e.detail.cellPosition, s = e.detail.rowPosition, a = e.detail.destination, c = e.detail.lastCell, t = this.bodyRows?.filter((h) => !f(h)), o = this.allRows?.filter((h) => !f(h)), S = this.headRows[this.headRows.length - 1]?.positionAll, w = t[0]?.positionAll, R = t[t.length - 1]?.positionAll, y = this.footRows[0]?.positionAll, C = o[o.length - 1]?.positionAll, z = a === "next" && s === S, T = a === "previous" && s === y, x = a === "previous" && s === w, A = a === "next" && s === R;
    let r;
    switch (a) {
      case "first":
        r = 0;
  ...
  localizeNumber(e) {
    return v.numberFormatOptions = {
      locale: this.messages._lang,
      numberingSystem: this.numberingSystem,
      useGrouping: this.groupSeparator
    }, v.localize(e.toString());
  }
  renderSelectionArea() {
    const e = this._selectedItems?.filter((t) => f(t))?.length, i = this.localizeNumber(e?.toString()), a = `${this.localizeNumber(this.selectedCount?.toString())} ${this.messages.selected}`, c = `${i} ${this.messages.hiddenSelected}`;
    return l`<div class=${d(n.selectionArea)}><calcite-chip .kind=${this.selectedCount > 0 ? "brand" : "neutral"} .label=${a} .scale=${this.scale} .value=${a}>${a}</calcite-chip>${e > 0 && l`<calcite-chip icon=hide-empty .label=${c} .scale=${this.scale} title=${c ?? P} .value=${c}>${i}</calcite-chip>` || ""}${this.selectedCount > 0 && l`<calcite-button icon-start=x kind=neutral @click=${this.handleDeselectAllRows} round .scale=${this.scale} .title=${`${this.messages.clear} ${a} ${this.messages.row}`}>${this.messages.clear}</calcite-button>` || ""}<div class=${d(n.selectionActions)}><slot name=${m.selectionActions}></slot></div></div>`;
  }
  renderPaginationArea() {
    return l`<div class=${d(n.paginationArea)}><calcite-pagination .groupSeparator=${this.groupSeparator} .numberingSystem=${this.numberingSystem} @calcitePaginationChange=${this.handlePaginationChange} .pageSize=${this.pageSize} .scale=${this.scale} start-item=1 .totalItems=${this.bodyRows?.length} ${p(this.paginationEl)}></calcite-pagination></div>`;
  }
  renderTHead() {
    return l`<thead><slot name=${m.tableHeader} ${p(this.tableHeadSlotEl)}></slot></thead>`;
  }
  renderTBody() {
    return l`<tbody><slot ${p(this.tableBodySlotEl)}></slot></tbody>`;
  }
  renderTFoot() {
    return l`<tfoot><slot name=${m.tableFooter} ${p(this.tableFootSlotEl)}></slot></tfoot>`;
  }
  render() {
    return l`<div class=${d(n.container)}>${this.selectionMode !== "none" && this.selectionDisplay !== "none" && this.renderSelectionArea() || ""}<div class=${d({
    ...
    this.selectionMode === "multiple" ? "true" : null} .ariaRowCount=${this.allRows?.length} class=${d({ [n.tableFixed]: this.layout === "fixed" })} .role=${this.interactionMode === "interactive" ? "grid" : "table"} ${p((e) => {
      e && M(l`<caption class=${d(n.assistiveText)}>${this.caption}</caption>${this.renderTHead()}${this.renderTBody()}${this.renderTFoot()}`, e);
