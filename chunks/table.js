import { a as I, L as F, d as g, s as d, E as P, x as o, B as M, c as k } from "./iframe.js";
...
class B extends F {
  ...
  renderSelectionArea() {
    ...
    return o`<div class=${d(n.selectionArea)}><calcite-chip .kind=${this.selectedCount > 0 ? "brand" : "neutral"} .label=${a} .scale=${this.scale} .value=${a}>${a}</calcite-chip>${e > 0 && o`<calcite-chip icon=hide-empty .label=${c} .scale=${this.scale} title=${c ?? P} .value=${c}>${i}</calcite-chip>` || ""}${this.selectedCount > 0 && o`<calcite-button icon-start=x kind=neutral @click=${this.handleDeselectAllRows} round .scale=${this.scale} .title=${`${this.messages.clear} ${a} ${this.messages.row}`}>${this.messages.clear}</calcite-button>` || ""}<div class=${d(n.selectionActions)}><slot name=${f.selectionActions}></slot></div></div>`;
  }
