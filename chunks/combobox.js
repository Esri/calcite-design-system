import { a as W, L as _, d as g, h as q, E as u, x as d, s as h, D as j, c as X } from "./iframe.js";
import { i as x } from "./keyed.js";
import { l as J } from "./live.js";
import { c as K } from "./core.js";
import { e as E, n as f } from "./ref.js";
...
class we extends _ {
  constructor() {
    super(), this.closeButtonEl = E(), this.selectAllComboboxItemReferenceEl = E(), this.filterItems = (() => {
      ...
      return $e((t, i = !1, s = !0) => {
        ...
        }), this.filterTextMatchPattern = this.filterText && new RegExp(`(${Se(this.filterText)})`, "i"), this.keyboardNavItems.forEach((n) => {
          n.filterTextMatchPattern = this.filterTextMatchPattern;
        }), i && (this.open = this.filterText.trim().length > 0 && this.keyboardNavItems.length > 0), s && this.calciteComboboxFilterChange.emit();
      }, j.filter);
  ...
  static {
    this.properties = { activeChipIndex: 16, activeDescendant: 16, activeItemIndex: 16, compactSelectionDisplay: 16, selectedHiddenChipsCount: 16, selectedVisibleChipsCount: 16, items: 16, allSelected: 16, indeterminate: 16, keyboardNavItems: 16, allowCustomValues: 7, clearDisabled: 7, disabled: 7, filterText: 3, filterProps: 0, filteredItems: 32, flipPlacements: 0, form: 3, label: 1, maxItems: 11, messageOverrides: 0, name: 3, open: 7, overlayPositioning: 3, placeholder: 1, placeholderIcon: 3, placeholderIconFlipRtl: 7, readOnly: 7, required: 7, scale: 3, selectAllEnabled: 7, selectedItems: 0, selectionDisplay: 3, selectionMode: 3, status: 3, validationIcon: [3, { converter: q }], validationMessage: 1, validity: 0, value: 1 };
  }
  static {
    this.styles = ye;
  }
  ...
  get keyboardNavItems() {
    const { selectAllComboboxItemReferenceEl: e } = this, t = this.filteredItems.filter((i) => !i.disabled);
  ...
  get filteredItems() {
    return this.items.filter((e) => !F(e));
  }
  get selectedItems() {
  ...
  loaded() {
    ae(this, this.getValue()), y(this), this.updateItems(), this.filterItems(this.filterText, !1, !1);
  }
  ...
  get showingInlineIcon() {
    const { placeholderIcon: e, selectionMode: t, selectedItems: i, open: s } = this, l = i[0], a = l?.icon, o = I(t);
  ...
  calciteComboboxItemChangeHandler(e) {
    if (this.ignoreSelectedEventsFlag)
      return;
    const t = e.target, i = e.composedPath().includes(this.selectAllComboboxItemReferenceEl.value);
    ...
    switch (t) {
      ...
      case "Enter":
        if (this.open && this.activeItemIndex > -1) {
          ...
          this.toggleSelection(i, !i.selected), e.preventDefault(), this.selectAllEnabled && this.handleSelectAll(i === this.selectAllComboboxItemReferenceEl.value);
  ...
  async refreshSelectionDisplay() {
    if (this.componentOnReady(), I(this.selectionMode) || !this.textInput.value)
      return;
    const { allSelectedIndicatorChipEl: e, chipContainerEl: t, selectionDisplay: i, placeholder: s, selectedIndicatorChipEl: l, textInput: { value: a } } = this, o = parseInt(getComputedStyle(t).gap.replace("px", "")), n = S(t), { fontSize: r, fontFamily: p } = getComputedStyle(a), m = (Y(s, `${r} ${p}`) || parseInt(K)) + o, k = S(e), D = S(l), G = Math.max(k, D);
    if (this.setCompactSelectionDisplay({
      chipContainerElGap: o,
      chipContainerElWidth: n,
      inputWidth: m,
      largestSelectedIndicatorChipWidth: G
    }), this.allSelected && this.selectAllEnabled && this.selectedItems.forEach((v) => {
      const b = this.referenceEl.querySelector(`#${C}${v.guid}`);
      b && this.hideChip(b);
    }), this.indeterminate && this.selectedItems.forEach((v) => {
      const b = this.referenceEl.querySelector(`#${C}${v.guid}`);
      b && this.showChip(b);
    }), i === "fit") {
      const v = Array.from(this.el.shadowRoot.querySelectorAll("calcite-chip")).filter((N) => N.closable), b = Math.round(n - ((this.selectedHiddenChipsCount > 0 ? D : 0) + o + m + o));
      this.refreshChipDisplay({ availableHorizontalChipElSpace: b, chipContainerElGap: o, chipEls: v }), this.setVisibleAndHiddenChips(v);
    }
  }
  setFloatingEl(e) {
  ...
  getMaxScrollerHeight() {
    const t = [...this.groupItems, ...this.getItems(!0)].filter((a) => !F(a)), { maxItems: i } = this;
    let s = 0, l = 0;
  ...
  calculateScrollerHeight(e) {
    if (!e)
      return 0;
    const t = e.getBoundingClientRect().height, i = `:scope > ${V}, :scope > ${z}`, s = Array.from(e.querySelectorAll(i)).reduce((l, a) => l + a.getBoundingClientRect().height, 0);
    return t - s;
  }
  inputHandler(e) {
  ...
  scrollToActiveOrSelectedItem(e = !1) {
    const t = e && this.selectedItems?.length ? this.selectedItems[0] : this.keyboardNavItems[this.activeItemIndex];
    if (!t)
      return;
    t.scrollIntoView({ block: "nearest" });
    const s = this.selectAllComboboxItemReferenceEl.value?.offsetHeight || 0, l = this.listContainerEl, a = t.getBoundingClientRect(), o = l.getBoundingClientRect();
    a.top < o.top + s && (l.scrollTop -= o.top + s - a.top);
  }
  ...
  renderChips() {
    ...
    return this.selectAllEnabled && this.allSelected ? null : this.selectedItems.map((a, o) => {
      const n = {
        [c.chip]: !0
      }, r = [...P(a)].reverse(), p = w(a), $ = [...r, a].map((k) => w(k)), m = s !== "ancestors" ? p : $.join(" / ");
      return x(p, d`<calcite-chip .appearance=${t ? "outline" : "solid"} class=${h(n)} .closable=${!t} data-test-id=${`chip-${o}`} .icon=${a.icon} .iconFlipRtl=${a.iconFlipRtl} id=${(a.guid ? `${C}${a.guid}` : null) ?? u} .label=${m} .messageOverrides=${{ dismissLabel: l.removeTag }} @focusin=${() => this.activeChipIndex = o} @calciteChipClose=${() => this.calciteChipCloseHandler(a)} .scale=${i} .selected=${a.selected} .tabIndex=${e === o ? 0 : -1} title=${m ?? u} .value=${a.value}>${m}</calcite-chip>`);
    });
  }
  renderAllSelectedIndicatorChip() {
    ...
    return d`<calcite-chip class=${h({
      ...
      [c.allSelected]: !0
    })} data-test-id=all-selected-indicator-chip .label=${l} .scale=${t} title=${l ?? u} value ${f(s)}>${l}</calcite-chip>`;
  }
  renderSelectedIndicatorChip() {
    ...
    } else t === "fit" && (n = !!(this.allSelected && a === 0 || l === 0), r = a > 0 ? `+${l}` : `${l} ${this.messages.selected}`);
    return d`<calcite-chip class=${h({
      chip: !0,
      [c.chipInvisible]: n
    })} .label=${r} .scale=${s} title=${r ?? u} value ${f(o)}>${r}</calcite-chip>`;
  }
  renderSelectedIndicatorChipCompact() {
    ...
    if (e) {
      const n = i().length;
      this.allSelected ? a = !0 : t === "fit" ? (a = !(l > 0), o = `${l || 0}`) : t === "single" && (a = !(n > 0), o = `${n}`);
    } else
      a = !0;
    return d`<calcite-chip class=${h({
      chip: !0,
      [c.chipInvisible]: a
    })} .label=${o} .scale=${s} title=${o ?? u} value>${o}</calcite-chip>`;
  }
  renderInput() {
    const { guid: e, disabled: t, placeholder: i, selectionMode: s, selectedItems: l, open: a } = this, o = I(s), n = l[0], r = !a && o && !!n && !this.filterText;
    return d`<span class=${h({
      ...
      [c.label]: !0,
      [c.labelIcon]: !!n?.icon
    })}>${w(n)}</span>`) || ""}${x("input", d`<input aria-activedescendant=${this.activeDescendant ?? u} aria-controls=${`${A}${e}`} aria-errormessage=${R.validationMessage} aria-owns=${`${A}${e}`} aria-autocomplete=list .ariaExpanded=${a} aria-haspopup=listbox .ariaInvalid=${this.status === "invalid"} .ariaLabel=${me(this)} class=${h({
      [c.input]: !0,
      "input--single": !0,
      [c.inputHidden]: r,
  ...
  renderSelectedOrPlaceholderIcon() {
    ...
    return this.showingInlineIcon && x("selected-placeholder-icon", d`<span class="icon-start"><calcite-icon class=${h({
      [c.selectedIcon]: !o,
      [c.placeholderIcon]: o
    })} .flipRtl=${o ? i : l.iconFlipRtl} .icon=${o ? t : a} .scale=${T(this.scale)}></calcite-icon></span>`) || "";
  }
  renderChevronIcon() {
    const { open: e } = this;
  ...
  render() {
    const { selectionDisplay: e, guid: t, label: i, open: s, readOnly: l } = this, a = I(this.selectionMode), o = e === "all", n = e === "single", r = !a && e === "fit", p = !this.clearDisabled && this.value?.length > 0;
    ...
    })} ${f(this.setChipContainerEl)}>${!a && !n && this.renderChips() || ""}${!a && !n && this.selectAllEnabled && this.renderAllSelectedIndicatorChip() || ""}${!a && !o && [
      this.renderSelectedIndicatorChip(),
      this.renderSelectedIndicatorChipCompact(),
      this.renderAllSelectedIndicatorChip()
    ] || ""}<label class=${h(c.screenReadersOnly)} .htmlFor=${`${U}${t}`} .id=${`${B}${t}`}>${i}</label>${this.renderInput()}</div>`)}${!l && p ? x("close-button", xe({ disabled: this.disabled, label: this.messages.clear, ref: this.closeButtonEl, scale: this.scale })) : null}${!l && this.renderChevronIcon() || ""}</div><ul aria-labelledby=${`${B}${t}`} aria-multiselectable=true class=${h(c.screenReadersOnly)} .id=${`${A}${t}`} role=listbox tabindex=-1>${this.renderListBoxOptions()}</ul>${this.renderFloatingUIContainer()}${ce({ component: this })}${this.validationMessage && this.status === "invalid" ? ve({ icon: this.validationIcon, id: R.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
