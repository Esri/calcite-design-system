...
class Le extends oe {
  constructor() {
    super(), this.direction = me(), this.formSupport = He({
      ...
      return ue((t, i = !1, s = !0) => {
        ...
        }), this.noMatchesFound = this.filteredItems.length === 0 && !!this.filterText, this.filterTextMatchPattern = this.filterText ? new RegExp(`(${U(this.filterText)})`, "i") : void 0, this.keyboardNavItems.forEach((c) => {
      ...
      return this.allItems.filter((e) => e.selected && (this.selectionMode !== "ancestors" || !O(e)));
    }, this.groupData = [], this.groupItems = [], this.guid = xe(), this.ignoreSelectedEventsFlag = !1, this.internalValueChangeFlag = !1, this.previousAllSelected = !1, this.refreshingSelectionDisplay = !1, this.mutationObserver = _("mutation", () => this.updateItems()), this.onLabelClick = () => {
      this.setFocus();
  ...
  get filteredItems() {
    return this.items.filter((e) => !Q(e));
  }
  get selectedItems() {
  ...
  willUpdate(e) {
    e.has("open") && (this.hasUpdated || this.open !== !1) && this.openHandler(), e.has("disabled") && (this.hasUpdated || this.disabled !== !1) && this.handleDisabledChange(this.disabled), e.has("maxItems") && (this.hasUpdated || this.maxItems !== 0) && this.setMaxScrollerHeight(), (e.has("overlayPositioning") && (this.hasUpdated || this.overlayPositioning !== "absolute") || e.has("placement") && (this.hasUpdated || this.placement !== G)) && this.reposition(!0), (e.has("selectionMode") || e.has("scale") || e.has("selectionAppearance")) && this.updateItems(), e.has("flipPlacements") && this.flipPlacementsHandler();
  }
  loaded() {
    E(this), this.updateItems(), this.filterItems(this.filterText, !1, !1);
  }
  disconnectedCallback() {
    super.disconnectedCallback(), this.mutationObserver?.disconnect(), this.resizeObserver?.disconnect(), ve(this);
  }
  emitComboboxChange() {
  ...
  setFilteredPlacements() {
    ...
    this.filteredFlipPlacements = t ? Ie(t, e) : void 0;
  }
  getValue() {
  ...
  onClose() {
    this.calciteComboboxClose.emit(), Ce(this), this.topLayer.hide();
  }
  ...
  async refreshSelectionDisplay(e = !0) {
    ...
    const c = parseInt(getComputedStyle(i).gap, 10), r = y(i), { fontSize: g, fontFamily: h, minInlineSize: f } = getComputedStyle(n.value), p = 0.55, b = parseFloat(f) || parseInt(R, 10), v = q(l, `${g} ${h}`), $ = v > 0 ? v : Math.max(b, Math.round((l?.length || 0) * (parseFloat(g) || parseInt(R, 10)) * p)), M = Math.max(b, $), T = (s === "fit" ? M : $) + c, ee = y(t.value), P = y(a.value), te = Math.max(ee, P);
    if (this.setCompactSelectionDisplay({
      chipContainerElGap: c,
      chipContainerElWidth: r,
      inputWidth: T,
      largestSelectedIndicatorChipWidth: te
    }), s !== "fit" && this.allSelected && this.selectAllEnabled && this.selectedItems.forEach((w) => {
      const x = this.referenceEl?.querySelector(`#${u.chip(w.guid)}`);
      x && this.hideChip(x);
      ...
      const w = Array.from(this.renderRoot.querySelectorAll("calcite-chip")).filter((L) => {
        const B = L.value, ae = B != null && `${B}` != "";
        return L.disabled || ae;
      }), { hiddenChipIndicatorWidth: x, hideSelectedChips: V, reservedPlaceholderInputWidth: ie } = this.getFitCompactDisplayState({
        chipContainerElGap: c,
        chipContainerElWidth: r,
        inputMinWidth: b,
        placeholderWidth: $,
        selectedIndicatorChipElWidth: P
      });
      this.fitUsingCompactCountLabel = V;
      const se = Math.round(r - (x + c + ie + c));
      this.refreshChipDisplay({
        availableHorizontalChipElSpace: se,
        chipContainerElGap: c,
        chipEls: w,
        hideSelectedChips: V
      });
      const le = this.selectedHiddenChipsCount;
      this.syncChipVisibilityCounts(w), le !== this.selectedHiddenChipsCount && e && (this.fitFollowUpRefreshPromise = this.updateComplete.then(() => this.refreshSelectionDisplay(!1)).then(() => {
  ...
  setContainerEl(e) {
    j(this.resizeObserver, this.listContainerEl, e), this.listContainerEl = e, this.transitionEl = e;
  }
  setChipContainerEl(e) {
    j(this.resizeObserver, this.chipContainerEl, e), this.chipContainerEl = e;
  }
  setReferenceEl(e) {
  ...
  getMaxScrollerHeight() {
    const t = this.getItemsAndGroups(!0).filter((a) => !Q(a)), { maxItems: i } = this;
    let s = 0, l = 0;
  ...
  calculateScrollerHeight(e) {
    if (!e)
      return 0;
    const t = e.getBoundingClientRect().height, i = `:scope > ${H}, :scope > ${z}`, s = Array.from(e.querySelectorAll(i)).reduce((l, a) => l + a.getBoundingClientRect().height, 0);
    return t - s;
  }
  inputHandler(e) {
  ...
  getItemsAndGroups(e = !1) {
    return e ? Array.from(this.el.querySelectorAll(`${H}, ${z}`)) : [...this.groupItems, ...this.items];
  }
  toggleSelection(e, t) {
  ...
  getItems(e = !1) {
    return Array.from(this.el.querySelectorAll(H)).filter((i) => e || !i.disabled);
  }
  getGroupItems() {
  ...
  nextChip() {
    ...
    t > e ? (this.activeChipIndex = -1, be(this.textInputRef.value)) : (this.activeChipIndex = t, this.focusChip()), this.updateActiveItemIndex(-1);
  }
  focusChip() {
  ...
  handleSelectionModeWarning() {
    this.selectionMode === "single-persist" && this.clearDisabled && he.warn('clearDisabled is ignored when selection-mode is set to "single-persist"');
  }
  ...
  getChipLabel(e, t) {
    return t ? [...[...Z(e)].reverse(), e].map((s) => F(s)).join(" / ") : F(e);
  }
  ...
  renderChipCount(e, t) {
    ...
    return m`<calcite-chip appearance=solid class=${d({
      [o.chip]: !0
    })} data-testid=selected-chip-count id=selected-chip-count .label=${i} .scale=${t} tabindex=-1 title=${i ?? I} ${C(this.selectedChipCountRef)}>${i}</calcite-chip>`;
  }
  renderChips() {
    ...
    let h = 0, f = 0;
    if (g) {
      const p = this.allItems.filter((v) => v.selected && (!c || !O(v)));
  ...
  renderAllSelectedIndicatorChip() {
    ...
    return m`<calcite-chip class=${d({
      ...
      [o.chipInvisible]: !c,
      [o.allSelected]: !0
    })} data-testid=all-selected-indicator-chip .label=${n} .scale=${i} title=${n ?? I} value ${C(e)}>${n}</calcite-chip>`;
  }
  renderSelectedIndicatorChip() {
    ...
    return e ? l = !0 : t === "single" && (this.allSelected ? l = !0 : n > 0 ? l = !1 : l = !0, a = `${n} ${this.messages.selected}`), m`<calcite-chip class=${d({
      [o.chip]: !0,
      [o.chipInvisible]: l
    })} .label=${a} .scale=${i} title=${a ?? I} value ${C(s)}>${a}</calcite-chip>`;
  }
  ...
  renderFloatingUIContainer() {
    ...
    return m`<div aria-hidden=true class=${d(o.floatingUIContainer)} popover=manual ${C(t)}><div class=${d(a)} ${C(i)}><ul class=${d({ [o.list]: !0, [o.listHide]: !s })}>${this.selectAllEnabled && this.selectionMode !== "single" && this.selectionMode !== "single-persist" && m`<calcite-combobox-item class=${d(o.selectAll)} data-testid=select-all-item .heading=${e.selectAll} .id=${`${this.guid}-select-all-enabled-interactive`} .indeterminate=${this.indeterminate} .label=${e.selectAll} .scale=${l} .selected=${this.allSelected} tabindex=-1 value=select-all ${C(this.selectAllComboboxItemRef)}></calcite-combobox-item>` || ""}<slot></slot>${this.noMatchesFound && (this.allowCustomValues ? m`<li aria-label=${n ?? I} class=${d(o.noMatches)} @click=${this.customChipAddHandler} role=option tabindex=0>${Ee({
      text: n,
      pattern: new RegExp(`(${U(this.filterText)})`, "i")
  ...
  renderSelectedOrPlaceholderIcon() {
    ...
    return this.showingInlineIcon && k("selected-placeholder-icon", m`<span class=${d(o.iconStart)}><calcite-icon class=${d({
      [o.selectedIcon]: !n,
      [o.placeholderIcon]: n
    })} .flipRtl=${n ? i : l.iconFlipRtl} .icon=${n ? t : a} .scale=${Y(this.scale)}></calcite-icon></span>`) || "";
  }
  renderChevronIcon() {
    const { open: e } = this;
    return k("chevron", m`<span class=${d(o.iconEnd)}><calcite-icon class=${d(o.icon)} .icon=${e ? K.chevronUp : K.chevronDown} .scale=${Y(this.scale)}></calcite-icon></span>`);
  }
  render() {
    ...
    return this.interactiveContainer({ disabled: this.disabled, children: m`${this.labelText && ke({ labelText: this.labelText, onClick: this.onLabelClick, required: this.required, tooltipText: this.messages.required }) || ""}<div aria-live=polite class=${d({
      [o.wrapper]: !0,
      [o.wrapperSingle]: a || !this.selectedItems.length,
      [o.wrapperActive]: s
    })} @click=${this.clickHandler} @keydown=${this.keyDownHandler} ${C(this.setReferenceEl)}>${this.renderSelectedOrPlaceholderIcon()}${k("grid", m`<div class=${d({
      [o.gridInput]: !0,
      [o.selectionDisplayFit]: r,
      [o.selectionDisplaySingle]: c
    })} ${C(this.setChipContainerEl)}>${!a && !c && this.renderChips() || ""}${!a && !c && this.selectAllEnabled && n && this.renderAllSelectedIndicatorChip() || ""}${h && this.renderSelectedIndicatorChip() || ""}${h && this.renderSelectedIndicatorChipCompact() || ""}${g && this.renderAllSelectedIndicatorChip() || ""}<label class=${d(W.screenReaderText)} .htmlFor=${`${u.input(t)}`} .id=${`${u.label(t)}`}>${i}</label>${this.renderInput()}</div>`)}${!l && p ? we({ ariaLabel: this.messages.clear, disabled: this.disabled, ref: this.clearButtonRef, scale: this.scale, title: this.messages.clear }) : null}${!l && this.renderChevronIcon() || ""}</div><ul aria-labelledby=${`${u.label(t)}`} aria-multiselectable=true class=${d(W.screenReaderText)} .id=${`${u.listbox(t)}`} role=listbox tabindex=-1>${this.renderListBoxOptions()}</ul>${this.renderFloatingUIContainer()}${this.validationMessage && this.status === "invalid" ? Ae({ icon: this.validationIcon, id: u.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
de("calcite-combobox", Le);
export {
  Le as Combobox
};
