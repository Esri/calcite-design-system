...
import { a as ae, L as ne, D as oe, c as S, F as ce, A as I, b as m, l as re, s as d, d as he } from "./index.js";
...
class Pe extends ne {
  constructor() {
    ...
    }, this.groupData = [], this.groupItems = [], this.guid = Ce(), this.ignoreSelectedEventsFlag = !1, this.internalValueChangeFlag = !1, this.previousAllSelected = !1, this.refreshingSelectionDisplay = !1, this.mutationObserver = N("mutation", () => this.updateItems()), this.onLabelClick = () => {
      this.setFocus();
  ...
  async refreshSelectionDisplay(e = !0) {
    ...
    const c = parseInt(getComputedStyle(i).gap, 10), r = y(i), { fontSize: g, fontFamily: h, minInlineSize: f } = getComputedStyle(n.value), p = 0.55, b = parseFloat(f) || parseInt(R, 10), v = U(l, `${g} ${h}`), $ = v > 0 ? v : Math.max(b, Math.round((l?.length || 0) * (parseFloat(g) || parseInt(R, 10)) * p)), M = Math.max(b, $), H = (s === "fit" ? M : $) + c, Z = y(t.value), P = y(a.value), ee = Math.max(Z, P);
    if (this.setCompactSelectionDisplay({
      ...
      }), { hiddenChipIndicatorWidth: x, hideSelectedChips: V, reservedPlaceholderInputWidth: te } = this.getFitCompactDisplayState({
        chipContainerElGap: c,
        chipContainerElWidth: r,
        inputMinWidth: b,
        placeholderWidth: $,
      ...
      const ie = Math.round(r - (x + c + te + c));
  ...
  calculateScrollerHeight(e) {
    if (!e)
      return 0;
    const t = e.getBoundingClientRect().height, i = `:scope > ${T}, :scope > ${z}`, s = Array.from(e.querySelectorAll(i)).reduce((l, a) => l + a.getBoundingClientRect().height, 0);
    return t - s;
  }
  inputHandler(e) {
  ...
  getItemsAndGroups(e = !1) {
    return e ? Array.from(this.el.querySelectorAll(`${T}, ${z}`)) : [...this.groupItems, ...this.items];
  }
  toggleSelection(e, t) {
  ...
  renderFloatingUIContainer() {
    ...
    return m`<div aria-hidden=true class=${d(o.floatingUIContainer)} popover=manual ${C(t)}><div class=${d(a)} ${C(i)}><ul class=${d({ [o.list]: !0, [o.listHide]: !s })}>${this.selectAllEnabled && this.selectionMode !== "single" && this.selectionMode !== "single-persist" && m`<calcite-combobox-item class=${d(o.selectAll)} data-testid=select-all-item .heading=${e.selectAll} .id=${`${this.guid}-select-all-enabled-interactive`} .indeterminate=${this.indeterminate} .label=${e.selectAll} .scale=${l} .selected=${this.allSelected} tabindex=-1 value=select-all ${C(this.selectAllComboboxItemRef)}></calcite-combobox-item>` || ""}<slot></slot>${this.noMatchesFound && (this.allowCustomValues ? m`<li aria-label=${n ?? I} class=${d(o.noMatches)} @click=${this.customChipAddHandler} role=option tabindex=0>${De({
      text: n,
