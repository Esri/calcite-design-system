import { a as _, L as q, d as g, h as N, s as h, E as u, x as d, D as j, c as X } from "./iframe.js";
import { i as x } from "./keyed.js";
import { l as J } from "./live.js";
import { c as K } from "./core.js";
import { e as D, n as f } from "./ref.js";
import { f as Q } from "./filter.js";
import { h as S, j as Y } from "./dom.js";
import { d as Z, r as ee, c as y, b as te, f as ie, h as se, F as H } from "./floating-ui.js";
import { c as le, a as oe, d as ae, s as ne, H as ce } from "./form.js";
import { g as re } from "./guid.js";
import { u as he, I as de } from "./interactive.js";
import { c as pe, d as ue, g as me } from "./label.js";
import { i as F, c as fe, g as T } from "./component.js";
...
class we extends q {
    ...
    super(), this.closeButtonEl = D(), this.filterItems = (() => {
      ...
      return $e((t, i = !1, s = !0) => {
        ...
        o.forEach((n) => {
          ...
          n.itemHidden = r;
          const [p, $] = n.ancestors;
          (e(p, l) || e($, l)) && (n.itemHidden = !1), r || n.ancestors.forEach((m) => m.itemHidden = !1);
        }), this.filterTextMatchPattern = this.filterText && new RegExp(`(${Se(this.filterText)})`, "i"), this.filteredItems.forEach((n) => {
      ...
      typeMismatch: !1,
      valueMissing: !1
    }, this.calciteComboboxBeforeClose = g({ cancelable: !1 }), this.calciteComboboxBeforeOpen = g({ cancelable: !1 }), this.calciteComboboxChange = g({ cancelable: !1 }), this.calciteComboboxChipClose = g({ cancelable: !1 }), this.calciteComboboxClose = g({ cancelable: !1 }), this.calciteComboboxFilterChange = g({ cancelable: !1 }), this.calciteComboboxOpen = g({ cancelable: !1 }), this.listenOn(document, "click", this.documentClickHandler), this.listen("calciteComboboxItemChange", this.calciteComboboxItemChangeHandler), this.listen("calciteInternalComboboxItemChange", this.calciteInternalComboboxItemChangeHandler), this.listen("click", this.comboboxFocusHandler);
  }
  static {
    this.properties = { activeChipIndex: 16, activeDescendant: 16, activeItemIndex: 16, compactSelectionDisplay: 16, selectedHiddenChipsCount: 16, selectedVisibleChipsCount: 16, selectAllComboboxItemReferenceEl: 16, items: 16, allSelected: 16, indeterminate: 16, keyboardNavItems: 16, allowCustomValues: 7, clearDisabled: 7, disabled: 7, filterText: 3, filterProps: 0, filteredItems: 32, flipPlacements: 0, form: 3, label: 1, maxItems: 11, messageOverrides: 0, name: 3, open: 7, overlayPositioning: 3, placeholder: 1, placeholderIcon: 3, placeholderIconFlipRtl: 7, readOnly: 7, required: 7, scale: 3, selectAllEnabled: 7, selectedItems: 0, selectionDisplay: 3, selectionMode: 3, status: 3, validationIcon: [3, { converter: N }], validationMessage: 1, validity: 0, value: 1 };
  }
  static {
    this.styles = ye;
  }
  ...
  get keyboardNavItems() {
    const { selectAllComboboxItemReferenceEl: e, items: t } = this;
    return e ? [e, ...t.filter((i) => !i.disabled)] : t.filter((i) => !i.disabled);
  }
  get filterText() {
  ...
  get filteredItems() {
    return this.keyboardNavItems.filter((e) => !F(e));
  }
  get selectedItems() {
  ...
  connectedCallback() {
    super.connectedCallback(), pe(this), le(this), this.internalValueChangeFlag = !0, this.value = this.getValue(), this.internalValueChangeFlag = !1, this.mutationObserver?.observe(this.el, { childList: !0, subtree: !0 }), this.setFilteredPlacements(), y(this);
  }
  willUpdate(e) {
  ...
  loaded() {
    oe(this, this.getValue()), y(this), this.updateItems(), this.filterItems(this.filterText, !1, !1);
  }
  disconnectedCallback() {
    super.disconnectedCallback(), this.mutationObserver?.disconnect(), this.resizeObserver?.disconnect(), ue(this), ae(this), te(this);
  }
  emitComboboxChange() {
  ...
  get showingInlineIcon() {
    const { placeholderIcon: e, selectionMode: t, selectedItems: i, open: s } = this, l = i[0], o = l?.icon, a = v(t);
  ...
  handleSelectAll(e) {
    e && this.toggleSelectAll(), this.allSelected && this.selectedItems.forEach((t) => {
      const i = this.referenceEl.querySelector(`#${C}${t.guid}`);
      i && this.hideChip(i);
    });
  }
  calciteComboboxItemChangeHandler(e) {
    if (this.ignoreSelectedEventsFlag)
      return;
    const t = e.target, i = e.composedPath().includes(this.selectAllComboboxItemReferenceEl);
    this.selectAllEnabled && this.handleSelectAll(i);
    const s = this.filteredItems.indexOf(t);
  ...
  toggleSelectAll() {
    ...
    this.selectedItems = this.items.filter((t) => (t.selected = e, e)), this.emitComboboxChange();
  }
  keyDownHandler(e) {
    if (this.readOnly)
      return;
    ...
    switch (t) {
      ...
      case "ArrowRight":
        ...
        break;
      case "ArrowUp":
        this.filteredItems.length && (e.preventDefault(), this.open && this.shiftActiveItemIndex(-1), this.scrollToActiveOrSelectedItem(), this.comboboxInViewport() || this.el.scrollIntoView()), this.scrollToActiveOrSelectedItem();
        break;
      case "ArrowDown":
        this.filteredItems.length && (e.preventDefault(), this.open ? this.shiftActiveItemIndex(1) : (this.open = !0, this.ensureRecentSelectedItemIsActive()), this.scrollToActiveOrSelectedItem(), this.comboboxInViewport() || this.el.scrollIntoView());
        break;
      case " ":
      ...
      case "Enter":
        if (this.open && this.activeItemIndex > -1) {
          ...
          this.toggleSelection(i, !i.selected), e.preventDefault(), this.selectAllEnabled && this.handleSelectAll(i === this.selectAllComboboxItemReferenceEl);
  ...
  async refreshSelectionDisplay() {
    if (this.componentOnReady(), v(this.selectionMode) || !this.textInput.value)
      return;
    const { allSelectedIndicatorChipEl: e, chipContainerEl: t, selectionDisplay: i, placeholder: s, selectedIndicatorChipEl: l, textInput: { value: o } } = this, a = parseInt(getComputedStyle(t).gap.replace("px", "")), n = S(t), { fontSize: r, fontFamily: p } = getComputedStyle(o), m = (Y(s, `${r} ${p}`) || parseInt(K)) + a, E = S(e), A = S(l), G = Math.max(E, A);
    if (this.setCompactSelectionDisplay({
      ...
      const b = this.referenceEl.querySelector(`#${C}${I.guid}`);
      b && this.hideChip(b);
    }), this.indeterminate && this.selectedItems.forEach((I) => {
      const b = this.referenceEl.querySelector(`#${C}${I.guid}`);
      b && this.showChip(b);
    }), i === "fit") {
      const I = Array.from(this.el.shadowRoot.querySelectorAll("calcite-chip")).filter((W) => W.closable), b = Math.round(n - ((this.selectedHiddenChipsCount > 0 ? A : 0) + a + m + a));
      this.refreshChipDisplay({ availableHorizontalChipElSpace: b, chipContainerElGap: a, chipEls: I }), this.setVisibleAndHiddenChips(I);
    }
  }
  setFloatingEl(e) {
  ...
  setReferenceEl(e) {
    this.referenceEl = e, y(this);
  }
  setSelectAllComboboxItemReferenceEl(e) {
    this.selectAllComboboxItemReferenceEl = e;
  }
  ...
  getMaxScrollerHeight() {
    const t = [...this.groupItems, ...this.getItems(!0)].filter((o) => !F(o)), { maxItems: i } = this;
    let s = 0, l = 0;
  ...
  calculateScrollerHeight(e) {
    if (!e)
      return 0;
    const t = e.getBoundingClientRect().height, i = `:scope > ${V}, :scope > ${z}`, s = Array.from(e.querySelectorAll(i)).reduce((l, o) => l + o.getBoundingClientRect().height, 0);
    return t - s;
  }
  inputHandler(e) {
  ...
  handleSingleSelection(e, t) {
    this.ignoreSelectedEventsFlag = !0, this.items.forEach((i) => i.selected = i === e ? t : !1), this.ignoreSelectedEventsFlag = !1, this.selectedItems = this.getSelectedItems(), this.emitComboboxChange(), this.textInput.value && (this.textInput.value.value = w(e)), this.open = !1, this.updateActiveItemIndex(-1), this.resetText(), this.filterItems("");
  }
  updateAncestors(e) {
  ...
  focusChip() {
    ...
    (e ? this.referenceEl.querySelector(`#${C}${e}`) : null)?.setFocus();
  }
  scrollToActiveOrSelectedItem(e = !1) {
    ...
    const s = this.selectAllComboboxItemReferenceEl?.offsetHeight || 0, l = this.listContainerEl, o = t.getBoundingClientRect(), a = l.getBoundingClientRect();
    o.top < a.top + s && (l.scrollTop -= a.top + s - o.top);
  }
  ...
  renderChips() {
    ...
    return this.selectAllEnabled && this.allSelected ? null : this.selectedItems.map((o, a) => {
      const n = {
        [c.chip]: !0
      }, r = [...P(o)].reverse(), p = w(o), $ = [...r, o].map((E) => w(E)), m = s !== "ancestors" ? p : $.join(" / ");
      return x(p, d`<calcite-chip .appearance=${t ? "outline" : "solid"} class=${h(n)} .closable=${!t} data-test-id=${`chip-${a}`} .icon=${o.icon} .iconFlipRtl=${o.iconFlipRtl} id=${(o.guid ? `${C}${o.guid}` : null) ?? u} .label=${m} .messageOverrides=${{ dismissLabel: l.removeTag }} @focusin=${() => this.activeChipIndex = a} @calciteChipClose=${() => this.calciteChipCloseHandler(o)} .scale=${i} .selected=${o.selected} .tabIndex=${e === a ? 0 : -1} title=${m ?? u} .value=${o.value}>${m}</calcite-chip>`);
    });
  }
  renderAllSelectedIndicatorChip() {
    const { compactSelectionDisplay: e, scale: t, selectedVisibleChipsCount: i, setAllSelectedIndicatorChipEl: s } = this, l = e ? this.messages.all : this.messages.allSelected;
    return d`<calcite-chip class=${h({
      chip: !0,
      [c.chipInvisible]: !(this.allSelected && !i),
      [c.allSelected]: !0
    })} .label=${l} .scale=${t} title=${l ?? u} value ${f(s)}>${l}</calcite-chip>`;
  }
  renderSelectedIndicatorChip() {
    ...
    if (e)
      ...
      const p = i().length;
      this.allSelected ? n = !0 : p > 0 ? n = !1 : n = !0, r = `${p} ${this.messages.selected}`;
    } else t === "fit" && (n = !!(this.allSelected && o === 0 || l === 0), r = o > 0 ? `+${l}` : `${l} ${this.messages.selected}`);
    return d`<calcite-chip class=${h({
      chip: !0,
      [c.chipInvisible]: n
    })} .label=${r} .scale=${s} title=${r ?? u} value ${f(a)}>${r}</calcite-chip>`;
  }
  renderSelectedIndicatorChipCompact() {
    ...
    if (e) {
      const n = i().length;
      this.allSelected ? o = !0 : t === "fit" ? (o = !(l > 0), a = `${l || 0}`) : t === "single" && (o = !(n > 0), a = `${n}`);
    } else
      o = !0;
    return d`<calcite-chip class=${h({
      chip: !0,
      [c.chipInvisible]: o
    })} .label=${a} .scale=${s} title=${a ?? u} value>${a}</calcite-chip>`;
  }
  renderInput() {
    const { guid: e, disabled: t, placeholder: i, selectionMode: s, selectedItems: l, open: o } = this, a = v(s), n = l[0], r = !o && a && !!n && !this.filterText;
    return d`<span class=${h({
      "input-wrap": !0,
      "input-wrap--single": a
    })}>${r && x("label", d`<span class=${h({
      [c.label]: !0,
      [c.labelIcon]: !!n?.icon
    })}>${w(n)}</span>`) || ""}${x("input", d`<input aria-activedescendant=${this.activeDescendant ?? u} aria-controls=${`${k}${e}`} aria-errormessage=${R.validationMessage} aria-owns=${`${k}${e}`} aria-autocomplete=list .ariaExpanded=${o} aria-haspopup=listbox .ariaInvalid=${this.status === "invalid"} .ariaLabel=${me(this)} class=${h({
      [c.input]: !0,
      "input--single": !0,
      [c.inputHidden]: r,
      "input--icon": this.showingInlineIcon && !!this.placeholderIcon
    })} data-test-id=input .disabled=${t} .id=${`${U}${e}`} @focus=${this.comboboxFocusHandler} @input=${this.inputHandler} placeholder=${i ?? u} .readOnly=${this.readOnly} role=combobox .tabIndex=${this.activeChipIndex === -1 ? 0 : -1} type=text .value=${J(this.filterText ?? "")} ${f(this.textInput)}>`)}</span>`;
  }
  renderListBoxOptions() {
    const e = this.filteredItems.map((t) => d`<li .ariaLabel=${t.label} .ariaSelected=${t.selected} id=${(t.guid ? `${L}${t.guid}` : null) ?? u} role=option tabindex=-1>${t.heading || t.textLabel}</li>`);
    if (this.selectAllEnabled && this.selectionMode !== "single" && this.selectionMode !== "single-persist") {
      const t = d`<li .ariaLabel=${this.messages.selectAll} .ariaSelected=${this.allSelected} .id=${`${this.guid}-select-all-enabled-screen-reader`} role=option tabindex=-1>${this.messages.selectAll}</li>`;
      t && e.unshift(t);
    }
    return e;
  }
  renderFloatingUIContainer() {
    const { setFloatingEl: e, setContainerEl: t, open: i, scale: s } = this, l = {
      [c.listContainer]: !0,
      [H.animation]: !0,
      [H.animationActive]: i
    };
    return d`<div aria-hidden=true class=${h(c.floatingUIContainer)} ${f(e)}><div class=${h(l)} ${f(t)}><ul class=${h({ list: !0, "list--hide": !i })}>${this.selectAllEnabled && this.selectionMode !== "single" && this.selectionMode !== "single-persist" && d`<calcite-combobox-item class=${h(c.selectAll)} .id=${`${this.guid}-select-all-enabled-interactive`} .indeterminate=${this.indeterminate} .label=${this.messages.selectAll} .scale=${s} .selected=${this.allSelected} tabindex=-1 text-label=${this.messages.selectAll ?? u} value=select-all ${f(this.setSelectAllComboboxItemReferenceEl)}></calcite-combobox-item>` || ""}<slot></slot></ul></div></div>`;
  }
  renderSelectedOrPlaceholderIcon() {
    ...
    return this.showingInlineIcon && x("selected-placeholder-icon", d`<span class="icon-start"><calcite-icon class=${h({
      [c.selectedIcon]: !a,
      [c.placeholderIcon]: a
    })} .flipRtl=${a ? i : l.iconFlipRtl} .icon=${a ? t : o} .scale=${T(this.scale)}></calcite-icon></span>`) || "";
  }
  renderChevronIcon() {
    const { open: e } = this;
    return x("chevron", d`<span class="icon-end"><calcite-icon class=${h(c.icon)} .icon=${e ? "chevron-up" : "chevron-down"} .scale=${T(this.scale)}></calcite-icon></span>`);
  }
  render() {
    const { selectionDisplay: e, guid: t, label: i, open: s, readOnly: l } = this, o = v(this.selectionMode), a = e === "all", n = e === "single", r = !o && e === "fit", p = !this.clearDisabled && this.value?.length > 0;
    return de({ disabled: this.disabled, children: d`<div aria-live=polite class=${h({
      [c.wrapper]: !0,
      [c.wrapperSingle]: o || !this.selectedItems.length,
      [c.wrapperActive]: s
    })} @click=${this.clickHandler} @keydown=${this.keyDownHandler} ${f(this.setReferenceEl)}>${this.renderSelectedOrPlaceholderIcon()}${x("grid", d`<div class=${h({
      "grid-input": !0,
      [c.selectionDisplayFit]: r,
      [c.selectionDisplaySingle]: n
    })} ${f(this.setChipContainerEl)}>${!o && !n && this.renderChips() || ""}${!o && !n && this.selectAllEnabled && this.renderAllSelectedIndicatorChip() || ""}${!o && !a && [
      this.renderSelectedIndicatorChip(),
      this.renderSelectedIndicatorChipCompact(),
      this.renderAllSelectedIndicatorChip()
    ] || ""}<label class=${h(c.screenReadersOnly)} .htmlFor=${`${U}${t}`} .id=${`${B}${t}`}>${i}</label>${this.renderInput()}</div>`)}${!l && p ? x("close-button", xe({ disabled: this.disabled, label: this.messages.clear, ref: this.closeButtonEl, scale: this.scale })) : null}${!l && this.renderChevronIcon() || ""}</div><ul aria-labelledby=${`${B}${t}`} aria-multiselectable=true class=${h(c.screenReadersOnly)} .id=${`${k}${t}`} role=listbox tabindex=-1>${this.renderListBoxOptions()}</ul>${this.renderFloatingUIContainer()}${ce({ component: this })}${this.validationMessage && this.status === "invalid" ? Ie({ icon: this.validationIcon, id: R.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
X("calcite-combobox", we);
export {
  we as Combobox
};
