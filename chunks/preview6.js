...
var { buildQueries: dI, configure: pI, createEvent: fI, fireEvent: mI, findAllByAltText: hI, findAllByDisplayValue: bI, findAllByLabelText: yI, findAllByPlaceholderText: gI, findAllByRole: vI, findAllByTestId: _I, findAllByText: RI, findAllByTitle: wI, findByAltText: EI, findByDisplayValue: CI, findByLabelText: qI, findByPlaceholderText: PI, findByRole: OI, findByTestId: TI, findByText: SI, findByTitle: MI, getAllByAltText: AI, getAllByDisplayValue: xI, getAllByLabelText: jI, getAllByPlaceholderText: NI, getAllByRole: $I, getAllByTestId: II, getAllByText: LI, getAllByTitle: kI, getByAltText: BI, getByDisplayValue: DI, getByLabelText: FI, getByPlaceholderText: UI, getByRole: HI, getByTestId: VI, getByText: zI, getByTitle: GI, getConfig: WI, getDefaultNormalizer: KI, getElementError: YI, getNodeText: JI, getQueriesForElement: XI, getRoles: QI, getSuggestedQuery: ZI, isInaccessible: e2, logDOM: t2, logRoles: r2, prettyDOM: n2, queries: o2, queryAllByAltText: a2, queryAllByAttribute: l2, queryAllByDisplayValue: i2, queryAllByLabelText: s2, queryAllByPlaceholderText: u2, queryAllByRole: c2, queryAllByTestId: d2, queryAllByText: p2, queryAllByTitle: f2, queryByAltText: m2, queryByAttribute: h2, queryByDisplayValue: b2, queryByLabelText: y2, queryByPlaceholderText: g2, queryByRole: v2, queryByTestId: _2, queryByText: R2, queryByTitle: w2, queryHelpers: E2, screen: C2, waitFor: q2, waitForElementToBeRemoved: P2, within: W$, prettyFormat: O2 } = ei, { userEvent: T2 } = mi({ userEvent: V$ }, { intercept: !0 }), { expect: Ad } = mi({ expect: yh }, { getKeys: (e, t) => {
...
var Cn = "storybook/a11y", Z$ = `${Cn}/result`, eI = `${Cn}/request`, tI = `${Cn}/running`, rI = `${Cn}/error`, nI = `${Cn}/manual`, Ya = { RESULT: Z$, REQUEST: eI, RUNNING: tI, ERROR: rI, MANUAL: nI }, oI = "a11y-test", { document: xd } = Q$, Ja = X$.getChannel(), Ey = { config: {}, options: {} }, aI = ["region"], ri = [], ni = !1, Cy = async () => {
  if (ri.length === 0) {
    ni = !1;
    return;
  }
...
var Nd = !1, S2 = async ({ reporting: e, parameters: t, globals: r, tags: n }) => {
  let o = t.a11y, l = r.a11y;
  if (o?.manual !== !0 && o?.disable !== !0 && l?.manual !== !0) {
    if (lI() && !n.includes(oI)) return;
    try {
      let a = await qy(o);
      if (a) {
        let i = (a?.violations.length ?? 0) > 0;
        if (e.addReport({ type: "a11y", version: 1, result: a, status: i ? "failed" : "passed" }), jd() && i) {
          if (!Nd) {
            let { toHaveNoViolations: u } = await import("./matchers-57EZJSQE.js");
            Ad.extend({ toHaveNoViolations: u }), Nd = !0;
          }
          Ad(a).toHaveNoViolations();
        }
      }
    } catch (a) {
      if (e.addReport({ type: "a11y", version: 1, result: { error: a }, status: "failed" }), jd()) throw a;
    }
  }
}, M2 = { a11y: { manual: !1 } };
export {
  S2 as experimental_afterEach,
  M2 as initialGlobals
};
