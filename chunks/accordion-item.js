import { a as x, L as f, d as h, s as t, x as n, C as b, c as $ } from "./iframe.js";
import { i as m } from "./keyed.js";
import { n as k } from "./ref.js";
import { c as v, s as p, g as S } from "./dom.js";
import { c as y, g as l } from "./component.js";
import { H as E } from "./Heading.js";
...
class w extends f {
  ...
  static {
    this.properties = { hasActionsEnd: 16, hasActionsStart: 16, accordionParent: 0, description: 1, expanded: 7, heading: 1, iconEnd: 3, iconFlipRtl: 3, appearance: 1, headingLevel: 11, iconPosition: 1, iconStart: 3, iconType: 1, scale: 3 };
  }
  static {
    this.styles = A;
  }
  async setFocus() {
    await y(this), this.headerEl.focus();
  }
  keyDownHandler(e) {
    if (e.target === this.el)
      switch (e.key) {
  ...
  updateActiveItemOnChange(e) {
    const [i] = e.composedPath(), o = v(this.el, "calcite-accordion");
    i === o && (this.determineActiveItem(o.selectionMode, e.detail.requestedAccordionItem), e.stopPropagation());
  }
  accordionItemSyncHandler(e) {
    const [i] = e.composedPath(), o = this.el;
    if (o.parentElement === i)
      return;
    const a = v(o, "calcite-accordion");
    i === a && (this.appearance = a.appearance, this.iconPosition = a.iconPosition, this.iconType = a.iconType, this.scale = a.scale, e.stopPropagation());
  }
  ...
  renderActionsStart() {
    return n`<div class=${t(c.actionsStart)} .hidden=${!this.hasActionsStart}><slot name=${g.actionsStart} @slotchange=${this.handleActionsStartSlotChange}></slot></div>`;
  }
  renderActionsEnd() {
    return n`<div class=${t(c.actionsEnd)} .hidden=${!this.hasActionsEnd}><slot name=${g.actionsEnd} @slotchange=${this.handleActionsEndSlotChange}></slot></div>`;
  }
  render() {
    const { iconFlipRtl: e, heading: i, headingLevel: o } = this, d = S(this.el), a = this.iconStart ? m("icon-start", n`<calcite-icon class=${t({ [c.icon]: !0, [c.iconStart]: !0 })} .flipRtl=${e === "both" || e === "start"} .icon=${this.iconStart} .scale=${l(this.scale)}></calcite-icon>`) : null, u = this.iconEnd ? m("icon-end", n`<calcite-icon class=${t({ [c.iconEnd]: !0, [c.icon]: !0 })} .flipRtl=${e === "both" || e === "end"} .icon=${this.iconEnd} .scale=${l(this.scale)}></calcite-icon>`) : null, { description: s } = this;
    ...
    })}>${this.renderActionsStart()}<div aria-controls=${r.section} .ariaExpanded=${this.expanded} class=${t(c.headerContent)} id=${r.sectionToggle} @click=${this.itemHeaderClickHandler} role=button tabindex=0 ${k(this.storeHeaderEl)}><div class=${t(c.headerContainer)}>${a}<div class=${t(c.headerText)}>${E({ class: c.heading, level: o, children: i })}${s ? n`<span class=${t(c.description)}>${s}</span>` : null}</div>${u}</div><calcite-icon class=${t(c.expandIcon)} .icon=${this.iconType === "chevron" ? "chevronDown" : this.iconType === "caret" ? "caretDown" : this.expanded ? "minus" : "plus"} .scale=${l(this.scale)}></calcite-icon></div>${this.renderActionsEnd()}</div><section aria-labelledby=${r.sectionToggle} class=${t(c.content)} id=${r.section}><slot></slot></section></div>`;
  }
}
$("calcite-accordion-item", w);
export {
  w as AccordionItem
};
