...
import { a as $, L as C, c as l, s as c, b as o, A as E, F as k, d as A } from "./index.js";
import { i as u } from "./keyed.js";
import { u as y } from "./index2.js";
import { e as w, n as I } from "./ref.js";
import { c as x, s } from "./dom.js";
...
class H extends C {
  ...
  accordionItemSyncHandler(e) {
    ...
    if (a.parentElement === n)
      return;
    const i = x(a, "calcite-accordion");
    n === i && (this.appearance = i.appearance, this.iconPosition = i.iconPosition, this.iconType = i.iconType, this.scale = i.scale, e.stopPropagation());
  }
  handleActionsStartSlotChange(e) {
    this.hasActionsStart = s(e);
  }
  handleActionsEndSlotChange(e) {
    this.hasActionsEnd = s(e);
  }
  handleContentEndSlotChange(e) {
    this.hasContentEnd = s(e);
  }
  handleContentStartSlotChange(e) {
    this.hasContentStart = s(e);
  }
  ...
  renderActionsStart() {
    ...
    return o`<div class=${c({ [t.actionsStart]: !0, [t.hasActions]: e })} .hidden=${!e}><slot name=${d.actionsStart} @slotchange=${this.handleActionsStartSlotChange}></slot></div>`;
  }
  renderActionsEnd() {
    const { hasActionsEnd: e } = this;
    return o`<div class=${c({ [t.actionsEnd]: !0, [t.hasActions]: e })} .hidden=${!e}><slot name=${d.actionsEnd} @slotchange=${this.handleActionsEndSlotChange}></slot></div>`;
  }
  renderContentEnd() {
    return o`<div class=${c(t.slotContentEnd)} .hidden=${!this.hasContentEnd}><slot name=${d.contentEnd} @slotchange=${this.handleContentEndSlotChange}></slot></div>`;
  }
  renderContentStart() {
    return o`<div class=${c(t.slotContentStart)} .hidden=${!this.hasContentStart}><slot name=${d.contentStart} @slotchange=${this.handleContentStartSlotChange}></slot></div>`;
  }
  render() {
    const { iconFlipRtl: e, heading: n, headingLevel: a, messages: p, expanded: i } = this, f = i ? p.collapse : p.expand, b = this.iconStart ? u("icon-start", o`<calcite-icon class=${c({ [t.icon]: !0, [t.iconStart]: !0 })} .flipRtl=${e === "both" || e === "start"} .icon=${this.iconStart} .scale=${v(this.scale)}></calcite-icon>`) : null, S = this.iconEnd ? u("icon-end", o`<calcite-icon class=${c({ [t.iconEnd]: !0, [t.icon]: !0 })} .flipRtl=${e === "both" || e === "end"} .icon=${this.iconEnd} .scale=${v(this.scale)}></calcite-icon>`) : null, { description: g } = this;
    return o`<div class=${c({
      [t.iconPosition(this.iconPosition)]: !0,
      [t.iconType(this.iconType)]: !0
    })}><div class=${c({
      [t.header]: !0,
      [k.rtl]: this.direction === "rtl",
      [t.headerAppearance(this.appearance)]: !0
    })}>${this.renderActionsStart()}<button aria-controls=${h.section} .ariaExpanded=${i} class=${c(t.headerContent)} id=${h.sectionToggle} @click=${this.itemHeaderClickHandler} type=button ${I(this.headerRef)}><div class=${c(t.headerContainer)}>${this.renderContentStart()}${b}<div class=${c(t.headerText)}>${D({ class: t.heading, level: a, children: n })}${g ? o`<span class=${c(t.description)}>${g}</span>` : null}</div>${S}${this.renderContentEnd()}</div><calcite-icon class=${c(t.expandIcon)} .icon=${this.iconType === "chevron" ? m.chevronDown : this.iconType === "caret" ? m.caretDown : i ? m.minus : m.plus} .scale=${v(this.scale)} title=${f ?? E}></calcite-icon></button>${this.renderActionsEnd()}</div><section aria-labelledby=${h.sectionToggle} class=${c(t.content)} id=${h.section}><slot></slot></section></div>`;
  }
}
A("calcite-accordion-item", H);
export {
  H as AccordionItem
};
