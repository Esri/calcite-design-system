import { b as E, L as $, c as m, s as n, C as d, x as a, A as M, E as w, q as A } from "./index.js";
import { i as g } from "./keyed.js";
import { e as T, n as z } from "./ref.js";
import { F as P, d as q, s as L, a as H, t as u } from "./dom.js";
...
class D extends $ {
  constructor() {
    super(), this.actionSlotWrapper = T(), this.userChangedValue = !1, this.hasEndActions = !1, this.updateAfterInitialRender = !1, this.depth = -1, this.disabled = !1, this.expanded = !1, this.indeterminate = !1, this.parentExpanded = !1, this.selected = !1, this.calciteInternalTreeItemSelect = m({ cancelable: !1 }), this.calciteTreeItemCollapse = m({ cancelable: !1 }), this.calciteTreeItemExpand = m({ cancelable: !1 }), this.listen("click", this.onClick), this.listen("keydown", this.keyDownHandler);
  }
  static {
  ...
  render() {
    const e = H(this.el) === "rtl", t = this.selectionMode === "single" || this.selectionMode === "children" || this.selectionMode === "single-persist", o = this.selectionMode === "multiple" || this.selectionMode === "multichildren", s = this.selectionMode === "none" && !this.hasChildren, f = this.hasChildren && this.indeterminate, l = this.hasChildren || this.selectionMode === "ancestors" ? a`<calcite-icon class=${n({
      [i.chevron]: !0,
      [d.rtl]: e
    })} data-test-id=icon .icon=${this.hasChildren ? c.chevronRight : c.blank} @click=${this.iconClickHandler} .scale=${h(this.scale)}></calcite-icon>` : null, b = g("default-slot", a`<slot></slot>`), p = this.selectionMode === "ancestors" ? a`<div class=${n(i.checkboxContainer)}><calcite-icon class=${n(i.checkbox)} .icon=${this.selected ? c.checkSquareF : f ? c.minusSquareF : c.square} .scale=${h(this.scale)}></calcite-icon></div>` : null, r = t ? c.bulletPoint : o ? c.checkmark : s ? c.blank : null, C = r ? a`<calcite-icon class=${n({
      [i.bulletPointIcon]: r === c.bulletPoint,
      [i.checkmarkIcon]: r === c.checkmark,
      [d.rtl]: e
    })} .icon=${r} .scale=${h(this.scale)}></calcite-icon>` : null, k = !(this.parentExpanded || this.depth === 1), v = this.updateAfterInitialRender && this.expanded, { hasEndActions: S } = this, y = g("actionsEndSlot", a`<slot name=${x.actionsEnd} @slotchange=${this.actionsEndSlotChangeHandler}></slot>`), I = a`<calcite-icon class=${n(i.iconStart)} .flipRtl=${this.iconFlipRtl === "start" || this.iconFlipRtl === "both"} .icon=${this.iconStart} .scale=${h(this.scale)}></calcite-icon>`;
    return this.el.ariaChecked = this.selectionMode === "multiple" || this.selectionMode === "multichildren" || this.selectionMode === "ancestors" ? u(this.selected) : void 0, this.el.ariaExpanded = this.hasChildren ? u(v) : void 0, this.el.inert = k, this.el.ariaLive = "polite", this.el.ariaSelected = this.selectionMode === "single" || this.selectionMode === "children" || this.selectionMode === "single-persist" ? u(this.selected) : void 0, this.el.toggleAttribute("calcite-hydrated-hidden", k), this.el.role = "treeitem", M(this.el, "tabIndex", this.disabled ? -1 : 0), F({ disabled: this.disabled, children: a`<div class=${n({ [i.itemExpanded]: v })}><div class=${n(i.nodeAndActionsContainer)}><div class=${n({
      [i.nodeContainer]: !0,
      [d.rtl]: e
    })} data-selection-mode=${this.selectionMode ?? w}>${l}${C}${p || null}${this.iconStart ? I : null}${p ? a`<label class=${n(i.checkboxLabel)}>${b}</label>` : b}</div><div class=${n(i.actionsEnd)} .hidden=${!S} ${z(this.actionSlotWrapper)}>${y}</div></div><div class=${n({
