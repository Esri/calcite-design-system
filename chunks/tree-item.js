...
import { a as I, L as $, c as r, s as i, C as d, b as o, y as E, A as T, d as w } from "./index.js";
import { i as b } from "./keyed.js";
import { e as A, n as M } from "./ref.js";
import { u as z } from "./index2.js";
import { G as L, d as R, s as H } from "./dom.js";
...
import { I as c, C as n, S as v } from "./resources33.js";
...
class D extends $ {
  ...
  updateAncestorTree() {
    ...
    if (!(this.selectionMode !== "ancestors" || !e)) {
      if (this.selected) {
        ...
        if (!t)
          return;
        const a = Array.from(t.children), s = a.filter((l) => l.selected);
        a.length === s.length ? (e.selected = !0, e.indeterminate = !1) : s.length > 0 && (e.indeterminate = !0), Array.from(this.el.querySelectorAll("calcite-tree-item:not([disabled])")).forEach((l) => {
  ...
  getSelectionIcon() {
    ...
    return e === "single" || e === "children" || e === "single-persist" ? c.bulletPoint : e === "multiple" || e === "multichildren" ? c.checkmark : e === "none" && !t ? c.blank : null;
  }
  render() {
    const e = this.direction === "rtl", t = this.getSelectionIcon(), a = this.hasChildren && this.indeterminate, s = this.hasChildren || this.selectionMode === "ancestors" ? o`<calcite-icon class=${i({
      [n.chevron]: !0,
      [d.rtl]: e
    })} .icon=${this.hasChildren ? c.chevronRight : c.blank} @click=${this.iconClickHandler} .scale=${h(this.scale)}></calcite-icon>` : null, p = b("default-slot", o`<slot></slot>`), l = this.selectionMode === "ancestors" ? o`<div class=${i(n.checkboxContainer)}><calcite-icon class=${i(n.checkbox)} .icon=${this.selected ? c.checkSquareF : a ? c.minusSquareF : c.square} .scale=${h(this.scale)}></calcite-icon></div>` : null, x = t ? o`<calcite-icon class=${i({
      [n.selectionIcon]: !0,
      [d.rtl]: e
    })} .icon=${t} .scale=${h(this.scale)}></calcite-icon>` : null, k = !(this.parentExpanded || this.depth === 1), u = this.updateAfterInitialRender && this.expanded, f = this.selectionMode === "multiple" || this.selectionMode === "multichildren" || this.selectionMode === "ancestors", g = this.selectionMode === "single" || this.selectionMode === "children" || this.selectionMode === "single-persist", { hasEndActions: C } = this, S = b("actionsEndSlot", o`<slot name=${v.actionsEnd} @slotchange=${this.actionsEndSlotChangeHandler}></slot>`), y = o`<calcite-icon class=${i(n.iconStart)} .flipRtl=${this.iconFlipRtl === "start" || this.iconFlipRtl === "both"} .icon=${this.iconStart} .scale=${h(this.scale)}></calcite-icon>`;
    return this.el.ariaChecked = m(f && this.selected, f ? "false" : null), this.el.ariaExpanded = m(this.hasChildren && u, this.hasChildren ? "false" : null), this.el.inert = k, this.el.ariaLive = "polite", this.el.ariaSelected = m(g && this.selected, g ? "false" : null), this.el.role = "treeitem", E(this.el, "tabIndex", this.disabled ? -1 : 0), this.interactiveContainer({ disabled: this.disabled, children: o`<div class=${i({ [n.itemExpanded]: u })}><div class=${i(n.nodeAndActionsContainer)}><div class=${i({
      [n.nodeContainer]: !0,
      [d.rtl]: e
    })} data-selection-mode=${this.selectionMode ?? T}>${s}${x}${l || null}${this.iconStart ? y : null}${l ? o`<label class=${i(n.checkboxLabel)}>${p}</label>` : p}</div><div class=${i(n.actionsEnd)} .hidden=${!C} ${M(this.actionSlotWrapperRef)}>${S}</div></div><div class=${i({
      [n.childrenContainer]: !0,
      [d.rtl]: e
    })} @click=${this.childrenClickHandler} .role=${this.hasChildren ? "group" : void 0}><slot name=${v.children} @slotchange=${this.handleChildrenSlotChange}></slot></div></div>` });
  }
}
w("calcite-tree-item", D);
export {
  D as TreeItem
};
