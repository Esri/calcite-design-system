import { j as z, L as C, x as h, s as b, q as u, k as T } from "./iframe.js";
...
class U extends C {
  ...
  renderMeterFill() {
    ...
    return h`<div class=${b({ [s.fill]: !0, [t]: l !== "single" })} style=${u({ width: `${e}%` })}></div>`;
  }
  renderRangeLine(e) {
    const l = { insetInlineStart: `${e}%` };
    return h`<div class=${b(s.stepLine)} style=${u(l)}></div>`;
  }
  renderValueLabel() {
    const { currentPercent: e, valueLabelType: l, unitLabel: t, value: a } = this, i = this.formatLabel(l === "percent" ? e / 100 : a || 0, l);
    return L("low-label-line", h`<div class=${b({ [s.label]: !0, [s.labelValue]: !0 })} ${m(this.valueLabelEl)}>${i}${t && l !== "percent" && h`<span class=${b(s.unitLabel)}>&nbsp;${t}</span>` || ""}</div>`);
  }
  renderMinLabel() {
    const { rangeLabelType: e, min: l, minPercent: t, unitLabel: a } = this, i = { insetInlineStart: `${t}%` }, n = this.formatLabel(e === "percent" ? t : l, e);
    return L("min-label-line", h`<div class=${b({ [s.label]: !0, [s.labelRange]: !0 })} style=${u(i)} ${m(this.minLabelEl)}>${n}${a && e !== "percent" && h`<span class=${b(s.unitLabel)}>&nbsp;${a}</span>` || ""}</div>`);
  }
  renderLowLabel() {
    ...
    return L("low-label-line", h`<div class=${b({ [s.label]: !0, [s.labelRange]: !0 })} style=${u(d)} ${m(this.lowLabelEl)}>${n}</div>`);
  }
  renderHighLabel() {
    ...
    return L("high-label-line", h`<div class=${b({ [s.label]: !0, [s.labelRange]: !0 })} style=${u(c)} ${m(this.highLabelEl)}>${i}</div>`);
  }
  renderMaxLabel() {
    const { rangeLabelType: e, max: l, maxPercent: t } = this, a = { insetInlineEnd: `${100 - t}%` }, i = this.formatLabel(e === "percent" ? t / 100 : l, e);
    return L("max-label-line", h`<div class=${b({ [s.label]: !0, [s.labelRange]: !0 })} style=${u(a)} ${m(this.maxLabelEl)}>${i}</div>`);
  }
  render() {
    ...
    return h`<div .ariaLabel=${i} .ariaValueMax=${f === "percent" ? d : c} .ariaValueMin=${f === "percent" ? o : g} .ariaValueNow=${$ === "percent" ? l : x} .ariaValueText=${$ === "percent" ? E : y ? k : void 0} class=${b({
      [s.container]: !0,
      [s.stepsVisible]: v,
      [s.valueVisible]: w,
      [e]: e !== "outline-fill"
