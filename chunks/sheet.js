...
class j extends E {
  ...
  render() {
    ...
    })} ${g(this.setTransitionEl)}><calcite-scrim class=${h(l.scrim)} @click=${this.handleOutsideClose}></calcite-scrim><div class=${h(l.content)} id=sheet-content ${g(this.setContentEl)}><div class=${h(l.contentContainer)}><slot></slot></div>${e ? H("resize-handle", S`<div .ariaLabel=${this.messages.resizeEnabled} .ariaOrientation=${i ? "vertical" : "horizontal"} .ariaValueMax=${i ? a.maxBlockSize : a.maxInlineSize} .ariaValueMin=${i ? a.minBlockSize : a.minInlineSize} .ariaValueNow=${i ? a.blockSize : a.inlineSize} class=${h(l.resizeHandle)} @keydown=${this.handleKeyDown} role=separator tabindex=0 touch-action=none ${g(this.setResizeHandleEl)}><div class=${h(l.resizeHandleBar)}><calcite-icon .icon=${this.getResizeIcon()} scale=s></calcite-icon></div></div>`) : null}</div></div>`;
  }
}
$("calcite-sheet", j);
export {
