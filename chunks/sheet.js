import { a as D, L as E, d as z, k as O, s as h, C as I, x as S, n as B, K as T, c as $ } from "./iframe.js";
...
class j extends E {
  ...
  handleKeyDown(e) {
    const { key: t, defaultPrevented: s, shiftKey: n } = e, { position: i, resizable: a, contentEl: b, el: c, resizeValues: { maxBlockSize: v, maxInlineSize: x, minBlockSize: f, minInlineSize: k } } = this, u = [...i === "block-end" || i === "block-start" ? ["ArrowUp", "ArrowDown"] : ["ArrowLeft", "ArrowRight"], "Home", "End"];
    if (!a || !b || s || !u.includes(t))
      return;
  ...
  updateSize({ type: e, size: t }) {
    ...
    if (!s)
      return;
    const i = e === "blockSize" ? "minBlockSize" : "minInlineSize", a = e === "blockSize" ? "maxBlockSize" : "maxInlineSize", b = n[i] && n[a] ? L(t, n[i], n[a]) : t, c = Math.round(b);
    this.resizeValues = {
      ...n,
      [e]: c
    }, s.style[e] = t !== null ? `${c}px` : null;
  }
  cleanupInteractions() {
  ...
  async setupInteractions() {
    ...
    this.interaction = y(t, { context: e.ownerDocument }).resizable({
      edges: {
        top: n === "block-end" ? a : !1,
        right: n === (u ? "inline-end" : "inline-start") ? a : !1,
        bottom: n === "block-start" ? a : !1,
        left: n === (u ? "inline-start" : "inline-end") ? a : !1
      },
      modifiers: [
  ...
  render() {
    ...
    })} ${g(this.setTransitionEl)}><calcite-scrim class=${h(l.scrim)} @click=${this.handleOutsideClose}></calcite-scrim><div class=${h(l.content)} id=sheet-content ${g(this.setContentEl)}><div class=${h(l.contentContainer)}><slot></slot></div>${e ? H("resize-handle", S`<div .ariaLabel=${this.messages.resizeEnabled} .ariaOrientation=${i ? "vertical" : "horizontal"} .ariaValueMax=${i ? s.maxBlockSize : s.maxInlineSize} .ariaValueMin=${i ? s.minBlockSize : s.minInlineSize} .ariaValueNow=${i ? s.blockSize : s.inlineSize} class=${h(l.resizeHandle)} @keydown=${this.handleKeyDown} role=separator tabindex=0 touch-action=none ${g(this.setResizeHandleEl)}><div class=${h(l.resizeHandleBar)}><calcite-icon .icon=${this.getResizeIcon()} scale=s></calcite-icon></div></div>`) : null}</div></div>`;
  }
}
$("calcite-sheet", j);
export {
