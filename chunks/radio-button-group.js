...
class v extends i {
  ...
  static {
    this.properties = { radioButtons: 16, disabled: 7, labelText: 1, layout: 3, messageOverrides: 0, name: 3, required: 7, scale: 3, selectedItem: 0, status: 3, validationIcon: [3, { converter: l }], validationMessage: 1 };
  }
  static {
    this.styles = b;
  }
  ...
  loaded() {
    this.passPropsToRadioButtons(), this.layout === "grid" && m.warn('The "grid" value of the layout property is deprecated and will be removed in v4.0. Use "horizontal" instead.');
  }
  ...
  render() {
    return this.el.role = "radiogroup", r`${this.labelText && d({ labelText: this.labelText, required: this.required, tooltipText: this.messages.required }) || ""}<div aria-errormessage=${a.validationMessage} .ariaInvalid=${this.status === "invalid"} .ariaRequired=${this.required} class=${n(g.itemWrapper)}><slot></slot></div>${this.validationMessage && this.status === "invalid" ? h({ icon: this.validationIcon, id: a.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}`;
  }
}
o("calcite-radio-button-group", v);
export {
