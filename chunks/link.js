...
class w extends u {
  ...
  render() {
    const { download: t, el: a } = this, l = v(a), i = this.href ? "a" : "button", c = e`<calcite-icon class="calcite-link--icon icon-start" .flipRtl=${this.iconFlipRtl === "start" || this.iconFlipRtl === "both"} .icon=${this.iconStart} scale=s></calcite-icon>`, r = e`<calcite-icon class="calcite-link--icon icon-end" .flipRtl=${this.iconFlipRtl === "end" || this.iconFlipRtl === "both"} .icon=${this.iconEnd} scale=s></calcite-icon>`, n = i === "button" ? s`button` : s`a`, d = i === "button" ? 0 : null;
    return this.el.role = "presentation", $({ disabled: this.disabled, children: k`<${n} class=${b({ [m.rtl]: l === "rtl" })} download=${(i === "a" ? t === !0 || t === "" ? "" : t || null : null) ?? o} href=${(i === "a" && this.href) ?? o} @click=${this.childElClickHandler} rel=${(i === "a" && this.rel) ?? o} tabindex=${d ?? o} target=${(i === "a" && this.target) ?? o} ${g(this.storeTagRef)}>${e`${this.iconStart ? c : null}<slot></slot>${this.iconEnd ? r : null}`}</${n}>` });
  }
}
f("calcite-link", w);
export {
