import { a as d, L as h, d as p, h as u, s as l, x as o, c as m } from "./iframe.js";
import { n as v } from "./ref.js";
import { a as f } from "./dom.js";
import { c as g, a as b, d as w, H as E } from "./form.js";
...
function s(c) {
  return c.tagName === "CALCITE-OPTION";
}
function $(c) {
  return c.tagName === "CALCITE-OPTION-GROUP";
}
class A extends h {
  constructor() {
  ...
  renderChevron() {
    return o`<div class=${l(n.iconContainer)}><calcite-icon class=${l(n.icon)} icon=chevron-down .scale=${S(this.scale)}></calcite-icon></div>`;
  }
  render() {
    const { disabled: e } = this;
    return C({ disabled: e, children: o`<div class=${l(n.wrapper)}><select aria-errormessage=${r.validationMessage} .ariaInvalid=${this.status === "invalid"} .ariaLabel=${N(this)} class=${l(n.select)} .disabled=${e} @change=${this.handleInternalSelectChange} ${v(this.storeSelectRef)}><slot></slot></select>${this.renderChevron()}${E({ component: this })}</div>${this.validationMessage && this.status === "invalid" ? k({ icon: this.validationIcon, id: r.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
m("calcite-select", A);
export {
