import { b as d, L as h, c as p, u as m, s as c, x as r, q as u } from "./index.js";
import { n as v } from "./ref.js";
import { u as g } from "./useT9n.js";
import { c as f, a as b, d as w, H as x } from "./form.js";
...
class L extends h {
  constructor() {
    super(), this.componentToNativeEl = /* @__PURE__ */ new Map(), this.mutationObserver = N("mutation", () => this.populateInternalSelect()), this.messages = g(), this.focusSetter = O()(this), this.disabled = !1, this.required = !1, this.scale = "m", this.status = "idle", this.validity = {
      valid: !1,
      badInput: !1,
      customError: !1,
      patternMismatch: !1,
  ...
  static {
    this.properties = { disabled: 7, form: 3, label: 1, labelText: 1, name: 3, required: 7, scale: 3, selectedOption: 0, status: 3, validationIcon: [3, { converter: m }], validationMessage: 1, validity: 0, value: 1, width: 3, messageOverrides: 0 };
  }
  static {
    this.styles = q;
  }
  ...
  renderChevron() {
    return r`<div class=${c(n.iconContainer)}><calcite-icon class=${c(n.icon)} icon=chevron-down .scale=${S(this.scale)}></calcite-icon></div>`;
  }
  render() {
    const { disabled: e } = this;
    return C({ disabled: e, children: r`${this.labelText && k({ labelText: this.labelText, onClick: this.onLabelClick, required: this.required, tooltipText: this.messages.required }) || ""}<div class=${c(n.wrapper)}><select aria-errormessage=${o.validationMessage} .ariaInvalid=${this.status === "invalid"} .ariaLabel=${z(this)} class=${c(n.select)} .disabled=${e} @change=${this.handleInternalSelectChange} .required=${this.required} ${v(this.storeSelectRef)}><slot></slot></select>${this.renderChevron()}${x({ component: this })}</div>${this.validationMessage && this.status === "invalid" ? T({ icon: this.validationIcon, id: o.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
u("calcite-select", L);
export {
