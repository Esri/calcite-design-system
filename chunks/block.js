import { a as w, L as O, d as l, x as i, s as a, E as f, c as I } from "./iframe.js";
...
import { t as F } from "./openCloseComponent.js";
import { a as D } from "./floating-ui.js";
import { u as R } from "./useT9n.js";
import { l as v } from "./logger.js";
...
class N extends O {
  ...
  load() {
    !this.heading && !this.label && v.warn(`${this.el.tagName} is missing both heading & label. Please provide a heading or label for the component to be accessible.`);
  }
  willUpdate(e) {
  ...
  renderScrim() {
    const { loading: e } = this, n = i`<slot></slot>`;
    return [e ? i`<calcite-scrim .loading=${e}></calcite-scrim>` : null, n];
  }
  renderLoaderStatusIcon() {
    const { loading: e, messages: n, status: o } = this;
    return e ? s("loader", i`<div class=${a(t.icon)}><calcite-loader inline .label=${n.loading}></calcite-loader></div>`) : o ? s("status-icon", i`<div class=${a(t.icon)}><calcite-icon class=${a({
      [t.statusIcon]: !0,
      [t.valid]: o == "valid",
      [t.invalid]: o == "invalid"
    })} .icon=${u[o]} scale=s></calcite-icon></div>`) : s("icon-slot", i`<div class=${a(t.icon)} .hidden=${!this.hasIcon}>${s("icon-slot", i`<slot name=${h.icon} @slotchange=${this.iconSlotChangeHandler}></slot>`)}</div>`);
  }
  renderActionsEnd() {
  ...
  renderTitle() {
    const { heading: e, headingLevel: n, description: o } = this;
    return e || o ? i`<div class=${a(t.title)}>${L({ class: t.heading, level: n, children: e })}${o ? i`<div class=${a(t.description)}>${o}</div>` : null}</div>` : null;
  }
  renderIcon(e) {
    const { iconFlipRtl: n } = this, o = n === "both" || e === "start" ? n === "start" : n === "end", r = e === "start" ? this.iconStart : this.iconEnd, g = e === "start" ? t.iconStart : t.iconEnd;
    if (r)
      return s(r, i`<calcite-icon class=${a(g)} .flipRtl=${o} .icon=${r} scale=s></calcite-icon>`);
  }
  render() {
    const { collapsible: e, loading: n, expanded: o, label: r, heading: g, messages: p, description: b, menuFlipPlacements: $, menuPlacement: x, moveToItems: k, setPosition: S, setSize: C, dragDisabled: H } = this, y = o ? p.collapse : p.expand, m = i`<header class=${a({ [t.header]: !0, [t.headerHasText]: !!(g || b) })} id=${c.header}>${this.renderIcon("start")}${this.renderContentStart()}${this.renderLoaderStatusIcon()}${this.renderTitle()}</header>`, B = o ? u.expanded : u.collapsed, E = i`<div class=${a(t.headerContainer)}>${this.dragHandle ? i`<calcite-sort-handle .disabled=${H} .label=${g || r} .moveToItems=${k} @calciteSortHandleBeforeClose=${this.handleSortHandleBeforeClose} @calciteSortHandleBeforeOpen=${this.handleSortHandleBeforeOpen} @calciteSortHandleClose=${this.handleSortHandleClose} @calciteSortHandleOpen=${this.handleSortHandleOpen} overlay-positioning=fixed .setPosition=${S} .setSize=${C} ${P(this.setSortHandleEl)}></calcite-sort-handle>` : null}${e ? i`<button aria-controls=${c.content} aria-describedby=${c.header} .ariaExpanded=${e ? o : null} class=${a(t.toggle)} id=${c.toggle} @click=${this.onHeaderClick} title=${y ?? f}>${m}<div class=${a(t.iconEndContainer)}>${this.renderIcon("end")}<calcite-icon class=${a(t.toggleIcon)} .icon=${B} scale=s></calcite-icon></div></button>` : this.iconEnd ? i`<div>${m}<div class=${a(t.iconEndContainer)}>${this.renderIcon("end")}</div></div>` : m}<div aria-labelledby=${c.header} class=${a(t.controlContainer)} .hidden=${!this.hasControl}><slot name=${h.control} @slotchange=${this.controlSlotChangeHandler}></slot></div><calcite-action-menu .flipPlacements=${$ ?? ["top", "bottom"]} .hidden=${!this.hasMenuActions} .label=${p.options} .overlayPositioning=${this.overlayPositioning} .placement=${x}><slot name=${h.headerMenuActions} @slotchange=${this.menuActionsSlotChangeHandler}></slot></calcite-action-menu>${this.renderActionsEnd()}</div>`;
    return T({ disabled: this.disabled, children: i`<article aria-label=${r ?? f} .ariaBusy=${n} class=${a({
      [t.container]: !0
