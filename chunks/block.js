import { b as D, L as M, c, x as i, s as a, E as u, q as L } from "./index.js";
...
import { s as K } from "./_sortable.js";
...
class W extends M {
  ...
  renderLoaderStatusIcon() {
    ...
    return e ? r("loader", i`<div class=${a(t.icon)}><calcite-loader inline .label=${o.loading} .scale=${this.scale}></calcite-loader></div>`) : n ? r("status-icon", i`<div class=${a(t.icon)}><calcite-icon class=${a({
      ...
      [t.valid]: n == "valid",
      [t.invalid]: n == "invalid"
    })} .icon=${f[n]} .scale=${m(this.scale)}></calcite-icon></div>`) : r("icon-slot", i`<div class=${a(t.icon)} .hidden=${!this.hasIcon}>${r("icon-slot", i`<slot name=${h.icon} @slotchange=${this.iconSlotChangeHandler}></slot>`)}</div>`);
  }
  renderActionsEnd() {
  ...
  renderTitle() {
    ...
    return e || n ? i`<div class=${a(t.title)}>${j({ class: t.heading, level: o, children: e })}${n ? i`<div class=${a(t.description)}>${n}</div>` : null}</div>` : null;
  }
  renderIcon(e) {
    const { iconFlipRtl: o } = this, n = o === "both" || e === "start" ? o === "start" : o === "end", s = e === "start" ? this.iconStart : this.iconEnd, g = e === "start" ? t.iconStart : t.iconEnd;
    if (s)
      return r(s, i`<calcite-icon class=${a(g)} .flipRtl=${n} .icon=${s} .scale=${m(this.scale)}></calcite-icon>`);
  }
  render() {
    const { collapsible: e, loading: o, expanded: n, label: s, heading: g, messages: p, description: k, menuFlipPlacements: $, menuPlacement: x, moveToItems: S, addToItems: C, setPosition: H, setSize: y, dragDisabled: B, sortDisabled: E, iconEnd: w, hasContentStart: I, iconStart: O } = this, z = n ? p.collapse : p.expand, P = !!(g || k || I || O || o || status), v = i`<header class=${a({
      [t.header]: !0,
      [t.headerHasContent]: P
    })} id=${l.header}>${this.renderIcon("start")}${this.renderContentStart()}${this.renderLoaderStatusIcon()}${this.renderTitle()}</header>`, A = n ? f.expanded : f.collapsed, T = i`<div class=${a(t.headerContainer)}>${this.dragHandle ? i`<calcite-sort-handle .addToItems=${C} .disabled=${B} .label=${g || s} .moveToItems=${S} @calciteSortHandleBeforeClose=${this.handleSortHandleBeforeClose} @calciteSortHandleBeforeOpen=${this.handleSortHandleBeforeOpen} @calciteSortHandleClose=${this.handleSortHandleClose} @calciteSortHandleOpen=${this.handleSortHandleOpen} overlay-positioning=fixed .scale=${this.scale} .setPosition=${H} .setSize=${y} .sortDisabled=${E} ${F(this.setSortHandleEl)}></calcite-sort-handle>` : null}${e ? i`<button aria-controls=${l.content} aria-describedby=${l.header} .ariaExpanded=${e ? n : null} class=${a(t.toggle)} id=${l.toggle} @click=${this.onHeaderClick} title=${z ?? u}>${v}<div class=${a(t.iconEndContainer)}>${this.renderIcon("end")}<calcite-icon class=${a(t.toggleIcon)} .icon=${A} .scale=${m(this.scale)}></calcite-icon></div></button>` : v}${w && !e ? i`<div class=${a(t.iconEndContainer)}>${this.renderIcon("end")}</div>` : null}<div aria-labelledby=${l.header} class=${a(t.controlContainer)} .hidden=${!this.hasControl}><slot name=${h.control} @slotchange=${this.controlSlotChangeHandler}></slot></div><calcite-action-menu .flipPlacements=${$ ?? ["top", "bottom"]} .hidden=${!this.hasMenuActions} .label=${p.options} .overlayPositioning=${this.overlayPositioning} .placement=${x} .scale=${this.scale}><slot name=${h.headerMenuActions} @slotchange=${this.menuActionsSlotChangeHandler}></slot></calcite-action-menu>${this.renderActionsEnd()}</div>`;
    return R({ disabled: this.disabled, children: i`<article aria-label=${s ?? u} .ariaBusy=${o} class=${a({
      [t.container]: !0
