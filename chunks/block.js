...
import { a as x, L as G, c, l as g, b as a, s as i, A as $, d as U } from "./index.js";
import { i as b } from "./keyed.js";
import { n as F } from "./ref.js";
import { s as r, d as R, x as j } from "./dom.js";
...
class ee extends G {
  ...
  handleDefaultSlotChange(e) {
    this.blockSectionChildren = R(e, "calcite-block-section"), this.hasContent = j(e) || r(e), this.updateBlockSectionScale();
  }
  ...
  renderTitle() {
    ...
    return e || n ? a`<div class=${i(t.title)}>${N({ class: t.heading, level: l, children: e })}${n ? a`<div class=${i(t.description)}>${n}</div>` : null}</div>` : null;
  }
  renderIcon(e) {
  ...
  render() {
    const { expandable: e, loading: l, expanded: n, label: o, heading: h, messages: p, description: C, menuFlipPlacements: S, menuPlacement: y, moveToItems: E, addToItems: H, setPosition: B, setSize: w, dragDisabled: O, sortDisabled: I, iconEnd: f, hasContentEnd: z, hasContentStart: P, iconStart: D, status: A } = this, u = n ? p.collapse : p.expand, T = !!(h || C || z || P || D || l || A), k = a`<header class=${i({
      [t.header]: !0,
      [t.headerHasContent]: T,
      [t.headerDraggable]: this.dragHandle
    })} id=${s.header}>${this.renderIcon("start")}${this.renderContentStart()}${this.renderLoaderStatusIcon()}${this.renderTitle()}</header>`, L = n ? v.expanded : v.collapsed, M = a`<div class=${i(t.headerContainer)}>${this.dragHandle ? a`<calcite-sort-handle .addToItems=${H} .disabled=${O} .label=${h || o} .moveToItems=${E} @calciteSortHandleBeforeClose=${this.handleSortHandleBeforeClose} @calciteSortHandleBeforeOpen=${this.handleSortHandleBeforeOpen} @calciteSortHandleClose=${this.handleSortHandleClose} @calciteSortHandleOpen=${this.handleSortHandleOpen} overlay-positioning=fixed .scale=${this.scale} .setPosition=${B} .setSize=${w} .sortDisabled=${I} .topLayerDisabled=${this.topLayerDisabled} ${F(this.setSortHandleEl)}></calcite-sort-handle>` : null}${e ? a`<button aria-controls=${s.content} aria-describedby=${s.header} .ariaExpanded=${e ? n : void 0} class=${i(t.toggle)} id=${s.toggle} @click=${this.onHeaderClick} title=${u ?? $} type=button>${k}<div class=${i(t.iconEndContainer)}>${this.renderContentEnd()}${this.renderIcon("end")}${this.toggleDisplay === "switch" ? a`<calcite-switch .checked=${n} .disabled=${this.disabled} inert .label=${u} .scale=${this.scale}></calcite-switch>` : a`<calcite-icon class=${i(t.toggleIcon)} .icon=${L} .scale=${m(this.scale)}></calcite-icon>`}</div></button>` : k}${f && !e ? a`<div class=${i(t.iconEndContainer)}>${this.renderContentEnd()}${this.renderIcon("end")}</div>` : !f && !e ? this.renderContentEnd() : null}<calcite-action-menu .flipPlacements=${S ?? ["top", "bottom"]} .hidden=${!this.hasMenuActions} .label=${p.options} .overlayPositioning=${this.overlayPositioning} .placement=${y} .scale=${this.scale} .topLayerDisabled=${this.topLayerDisabled}><slot name=${d.headerMenuActions} @slotchange=${this.menuActionsSlotChangeHandler}></slot></calcite-action-menu>${this.renderActionsEnd()}</div>`;
    return this.interactiveContainer({ disabled: this.disabled, children: a`<article aria-label=${o ?? $} .ariaBusy=${Y(l, void 0)} class=${i({
      [t.container]: !0
    })}>${M}<section aria-labelledby=${s.toggle} class=${i({
      [t.content]: !0,
