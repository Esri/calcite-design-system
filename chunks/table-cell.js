import { j as i, L as c, x as o, s, E as n, C as a, k as r } from "./iframe.js";
...
class f extends c {
  ...
  render() {
    ...
    return b({ disabled: this.disabled, children: o`<td class=${s({
    ...
    })} colSpan=${this.colSpan ?? n} @blur=${this.onContainerBlur} @focus=${this.onContainerFocus} .role=${this.interactionMode === "interactive" ? "gridcell" : "cell"} rowSpan=${this.rowSpan ?? n} .tabIndex=${l ? -1 : 0} ${h(this.containerEl)}>${(this.selectionCell || this.readCellContentsToAT) && o`<span .ariaLive=${this.focused ? "polite" : "off"} class=${s(e.assistiveText)}>${this.selectionCell && this.selectionText || ""}${this.readCellContentsToAT && !this.selectionCell && this.contentsText || ""}</span>` || ""}<slot @slotchange=${this.updateScreenReaderContentsText}></slot></td>` });
  }
}
