import { a as s, L as n, s as o, E as c, C as a, x as i, c as r } from "./iframe.js";
...
class C extends n {
  ...
  render() {
    ...
    return b({ disabled: this.disabled, children: i`<td class=${o({
    ...
    })} colSpan=${this.colSpan ?? c} @blur=${this.onContainerBlur} @focus=${this.onContainerFocus} .role=${this.interactionMode === "interactive" ? "gridcell" : "cell"} rowSpan=${this.rowSpan ?? c} .tabIndex=${l ? -1 : 0} ${h(this.containerEl)}>${(this.selectionCell || this.readCellContentsToAT) && i`<span .ariaLive=${this.focused ? "polite" : "off"} class=${o(e.assistiveText)}>${this.selectionCell && this.selectionText || ""}${this.readCellContentsToAT && !this.selectionCell && this.contentsText || ""}</span>` || ""}<slot @slotchange=${this.updateScreenReaderContentsText}></slot></td>` });
  }
}
r("calcite-table-cell", C);
export {
  C as TableCell
};
