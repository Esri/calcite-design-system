import { j as d, L as h, x as i, s as o, E as n, k as p } from "./iframe.js";
...
class v extends h {
  ...
  render() {
    ...
    })} colSpan=${this.colSpan ?? n} @blur=${this.onContainerBlur} @focus=${this.onContainerFocus} .role=${this.parentRowType === "head" ? "columnheader" : "rowheader"} rowSpan=${this.rowSpan ?? n} scope=${t ?? n} .tabIndex=${this.selectionCell ? 0 : a ? -1 : 0} ${m(this.containerEl)}>${this.heading && i`<div class=${o(e.heading)}>${this.heading}</div>` || ""}${this.description && i`<div class=${o(e.description)}>${this.description}</div>` || ""}${this.selectionCell && this.selectionMode === "multiple" && i`<calcite-icon class=${o({ [e.active]: s || l })} .icon=${r} .scale=${g(this.scale)}></calcite-icon>` || ""}${(this.selectionCell || this.numberCell) && i`<span .ariaLive=${this.focused ? "polite" : "off"} class=${o(e.assistiveText)}>${this.screenReaderText}</span>` || ""}</th>`;
  }
}
