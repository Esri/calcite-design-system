import { a as d, L as h, s as i, E as c, x as o, c as u } from "./iframe.js";
...
class C extends h {
  ...
  render() {
    const t = this.rowSpan ? "rowgroup" : this.colSpan ? "colgroup" : this.parentRowType === "body" ? "row" : "col", l = this.selectedRowCount === this.bodyRowCount, a = this.selectedRowCount > 0, r = l ? n.checked : a ? n.indeterminate : n.unchecked, s = this.interactionMode === "static" && !this.selectionCell;
    ...
    })} colSpan=${this.colSpan ?? c} @blur=${this.onContainerBlur} @focus=${this.onContainerFocus} .role=${this.parentRowType === "head" ? "columnheader" : "rowheader"} rowSpan=${this.rowSpan ?? c} scope=${t ?? c} .tabIndex=${this.selectionCell ? 0 : s ? -1 : 0} ${m(this.containerEl)}>${this.heading && o`<div class=${i(e.heading)}>${this.heading}</div>` || ""}${this.description && o`<div class=${i(e.description)}>${this.description}</div>` || ""}${this.selectionCell && this.selectionMode === "multiple" && o`<calcite-icon class=${i({ [e.active]: a || l })} .icon=${r} .scale=${g(this.scale)}></calcite-icon>` || ""}${(this.selectionCell || this.numberCell) && o`<span .ariaLive=${this.focused ? "polite" : "off"} class=${i(e.assistiveText)}>${this.screenReaderText}</span>` || ""}</th>`;
  }
}
u("calcite-table-header", C);
export {
  C as TableHeader
};
