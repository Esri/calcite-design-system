...
import { b as z, h as u } from "./utils4.js";
...
class L extends $ {
  constructor() {
    super(), this.containerEl = C(), this.focusSetter = M()(this), this.hasImage = !1, this.disabled = !1, this.interactive = !1, this.scale = "m", this.selected = !1, this.selectionMode = "none", this.calciteInternalSwatchKeyEvent = w({ cancelable: !1 }), this.calciteInternalSwatchSelect = w({ cancelable: !1 }), this.calciteInternalSyncSelectedSwatches = w({ cancelable: !1 }), this.calciteSwatchSelect = w({ cancelable: !1 }), this.listen("keydown", this.keyDownHandler);
  }
  static {
  ...
  renderSwatchRect({ clipPath: e, fill: i, height: r, key: a, rx: l, stroke: v, strokeWidth: p, width: f, id: g }) {
    return x(a, h`<rect clip-path=${e ?? c} fill=${i ?? c} height=${r ?? c} id=${(g || d.swatchRect) ?? c} rx=${l ?? c} stroke=${v ?? c} stroke-width=${p ?? c} width=${f ?? c} />`);
  }
  render() {
    ...
    })} @click=${this.handleEmittingEvent} .role=${r} .tabIndex=${i ? -1 : 0} title=${this.label ?? c} ${y(this.containerEl)}>${this.renderSwatchImage()}${!this.internalColor && !this.hasImage && this.renderEmptyDisplay() || ""}${this.disabled && this.renderDisabledDisplay() || ""}<svg class=${s(l)} role=presentation xmlns=http://www.w3.org/2000/svg>${this.renderSwatch()}</svg></div>` });
  }
}
b("calcite-swatch", L);
export {
