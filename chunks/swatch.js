...
class P extends S {
  ...
  keyDownHandler(e) {
    e.target === this.el && E(e.key) && (this.handleEmittingEvent(), e.preventDefault());
  }
  ...
  handleColorChange(e) {
    this.internalColor = e ? y(e) : void 0;
  }
  renderSwatchImage() {
    return o`<div class=${c(t.imageContainer)}><slot name=${M.image} @slotchange=${this.handleSlotImageChange}></slot></div>`;
  }
  renderEmptyDisplay() {
    const e = this.scale === "s" ? "12" : this.scale === "m" ? "16" : "20";
    return o`<div class=${c(t.internalSvgContainer)}><svg fill=none height=${e ?? a} viewBox=${`0 0 ${e} ${e}`} width=${e ?? a} xmlns=http://www.w3.org/2000/svg>${h`<path class=${c(t.internalSvgEmpty)} d=${`M${e} 0L0 ${e}`} />`}</svg></div>`;
  }
  renderDisabledDisplay() {
    const e = o`<svg fill=none height=14 viewBox="0 0 14 14" width=14 xmlns=http://www.w3.org/2000/svg>${h`<path class=${c(t.internalSvgDisabled)} d="M7 0.5C10.5899 0.5 13.5 3.41015 13.5 7C13.5 10.5899 10.5899 13.5 7 13.5C3.41015 13.5 0.5 10.5899 0.5 7C0.5 3.41015 3.41015 0.5 7 0.5ZM4.78906 10.917C5.44221 11.2866 6.19529 11.5 7 11.5C9.48528 11.5 11.5 9.48528 11.5 7C11.5 6.19529 11.2866 5.44221 10.917 4.78906L4.78906 10.917ZM7 2.5C4.51472 2.5 2.5 4.51472 2.5 7C2.5 7.95644 2.79808 8.84235 3.30664 9.57129L9.57129 3.30664C8.84235 2.79808 7.95644 2.5 7 2.5Z" />`}</svg>`, i = o`<svg fill=none height=18 viewBox="0 0 18 18" width=18 xmlns=http://www.w3.org/2000/svg>${h`<path class=${c(t.internalSvgDisabled)} d="M9 0.5C13.6944 0.5 17.5 4.30558 17.5 9C17.5 13.6944 13.6944 17.5 9 17.5C4.30558 17.5 0.5 13.6944 0.5 9C0.5 4.30558 4.30558 0.5 9 0.5ZM5.78125 14.2588C6.71828 14.8337 7.81941 15.167 9 15.167C12.4058 15.167 15.167 12.4058 15.167 9C15.167 7.81941 14.8337 6.71828 14.2588 5.78125L5.78125 14.2588ZM9 2.83301C5.59424 2.83301 2.83301 5.59424 2.83301 9C2.83301 10.3817 3.28731 11.6565 4.05469 12.6846L12.6846 4.05469C11.6565 3.28731 10.3817 2.83301 9 2.83301Z" />`}</svg>`;
    return o`<div class=${c(t.internalSvgContainer)}>${this.scale === "l" ? i : e}</div>`;
  }
  renderSwatch() {
    const { internalColor: e } = this, i = "0", r = !e, s = {
      height: "100%",
      rx: i,
      width: "100%"
    };
    ...
    const l = e.alpha(), w = u(e), v = u(e, l < 1);
    return h`<defs><pattern height=${n.size} id=${d.checker} patternUnits=userSpaceOnUse width=${n.size} x=0 y=0><rect class=${c(t.checker)} height=${n.squareSize} width=${n.squareSize} x=0 y=0 /><rect class=${c(t.checker)} height=${n.squareSize} width=${n.squareSize} x=${n.squareSize} y=${n.squareSize} /></pattern></defs>${this.renderSwatchRect({
      fill: "url(#checker)",
      rx: s.rx,
      height: s.height,
      width: s.width
  ...
  renderSwatchRect({ clipPath: e, fill: i, height: r, key: s, rx: l, stroke: w, strokeWidth: v, width: f, id: g }) {
    return x(s, h`<rect clip-path=${e ?? a} fill=${i ?? a} height=${r ?? a} id=${(g || d.swatchRect) ?? a} rx=${l ?? a} stroke=${w ?? a} stroke-width=${v ?? a} width=${f ?? a} />`);
  }
  render() {
    const { disabled: e } = this, i = e || !e && !this.interactive, r = this.selectionMode === "multiple" && this.interactive ? "checkbox" : this.selectionMode !== "none" && this.interactive ? "radio" : this.interactive ? "button" : "presentation", s = !this.internalColor, l = {
      [t.swatch]: !0,
      [t.noColorSwatch]: s || this.hasImage && !this.internalColor
    };
    return this.interactiveContainer({ disabled: e, children: o`<div .ariaChecked=${this.selectionMode !== "none" && this.interactive ? this.selected : void 0} .ariaLabel=${r !== "presentation" ? this.label : ""} class=${c({
      [t.container]: !0,
      [t.selectable]: this.selectionMode !== "none",
      [t.selected]: this.selected,
      [t.nonInteractive]: !this.interactive
    })} @click=${this.handleEmittingEvent} .role=${r} .tabIndex=${i ? -1 : 0} title=${this.label ?? a} ${b(this.containerRef)}>${this.renderSwatchImage()}${!this.internalColor && !this.hasImage && this.renderEmptyDisplay() || ""}${this.disabled && this.renderDisabledDisplay() || ""}<svg class=${c(l)} role=presentation xmlns=http://www.w3.org/2000/svg>${this.renderSwatch()}</svg></div>` });
  }
}
$("calcite-swatch", P);
export {
