import { a as $, L as P, d as p, s as h, x as d, D as y, c as w } from "./iframe.js";
import { n as F } from "./ref.js";
import { i as g } from "./keyed.js";
import { s as b, C as D } from "./dom.js";
import { u as k, I as M } from "./interactive.js";
import { c as T } from "./observers.js";
import { b as I, l as v, c as L, i as C, e as O, u as R } from "./utils3.js";
import { d as H, c as N } from "./sortableComponent.js";
import { S as x } from "./resources11.js";
...
class K extends P {
  ...
  disconnectedCallback() {
    super.disconnectedCallback(), this.disconnectObserver(), H(this);
  }
  ...
  handleCalciteInternalListItemSelectMultiple(t) {
    ...
    if (i.selectMultiple && s) {
      ...
      l.slice(c, m + 1).forEach((u) => u.selected = s.selected);
    } else
  ...
  setParentList() {
    this.parentListEl = this.el.parentElement?.closest(v);
  }
  ...
  handleFilterActionsStartSlotChange(t) {
    this.hasFilterActionsStart = b(t);
  }
  handleFilterActionsEndSlotChange(t) {
    this.hasFilterActionsEnd = b(t);
  }
  handleFilterNoResultsSlotChange(t) {
    this.hasFilterNoResults = b(t);
  }
  setActiveListItem() {
  ...
  allParentListItemsExpanded(t) {
    const e = t.parentElement?.closest(I);
    if (e) {
      if (!e.expanded)
        return !1;
    } else return !0;
  ...
  updateGroupItems() {
    const { el: t, group: e, scale: i } = this, l = D(t), s = e ? Array.from(l.querySelectorAll(`${v}[group="${e}"]`)).filter((a) => !a.disabled && a.dragEnabled) : [];
  ...
  handleMove(t) {
    const { moveTo: e } = t.detail, i = t.target, l = i?.parentElement, s = e.element, a = Array.from(l.children).filter(C).indexOf(i), r = 0;
    if (l) {
      if (l.canPull?.({
        toEl: s,
        fromEl: l,
  ...
  handleReorder(t) {
    ...
    if (!l)
      return;
    i.sortHandleOpen = !1;
    const s = Array.from(l.children).filter(C), n = s.length - 1, a = s.indexOf(i);
    let r = a;
    switch (e) {
      case "top":
        r = 0;
  ...
  render() {
    const { loading: t, label: e, disabled: i, dataForFilter: l, filterEnabled: s, filterPlaceholder: n, filterText: a, filterLabel: r, hasFilterActionsStart: c, hasFilterActionsEnd: m, effectiveFilterProps: u } = this;
    return M({ disabled: this.disabled, children: d`<div class=${h(f.container)}>${this.dragEnabled ? d`<span aria-live=assertive class=${h(f.assistiveText)}>${this.assistiveText}</span>` : null}${this.renderItemAriaLive()}${t ? d`<calcite-scrim class=${h(f.scrim)} .loading=${t}></calcite-scrim>` : null}<div .ariaBusy=${t} .ariaLabel=${e || ""} class=${h(f.table)} @keydown=${this.handleListKeydown} role=treegrid>${s || c || m ? d`<div class=${h(f.sticky)} role=rowgroup><div role=row><div role=columnheader><calcite-stack class=${h(f.stack)}><slot name=${E.filterActionsStart} @slotchange=${this.handleFilterActionsStartSlotChange} slot=${x.actionsStart}></slot><calcite-filter .ariaLabel=${n} .disabled=${i} .filterProps=${u} .items=${l} .label=${r} @calciteFilterChange=${this.handleFilterChange} .placeholder=${n} .scale=${this.scale} .value=${a} ${F(this.setFilterEl)}></calcite-filter><slot name=${E.filterActionsEnd} @slotchange=${this.handleFilterActionsEndSlotChange} slot=${x.actionsEnd}></slot></calcite-stack></div></div></div>` : null}<div class=${h(f.tableContainer)} role=rowgroup><slot @slotchange=${this.handleDefaultSlotChange} ${F(this.setDefaultSlotEl)}></slot></div></div><div aria-live=polite data-test-id=no-results-container .hidden=${!this.showNoResultsContainer}><slot name=${E.filterNoResults} @slotchange=${this.handleFilterNoResultsSlotChange}></slot></div></div>` });
  }
  renderItemAriaLive() {
    ...
    return A.numberFormatOptions = {
      locale: l,
      numberingSystem: s
    }, i ? null : d`<div aria-live=polite class=${h(f.assistiveText)}>${this.hasActiveFilter ? g("aria-filter-enabled", d`<div>${t.filterEnabled}</div>`) : null}${g("aria-item-count", d`<div>${t.total.replace("{count}", A.localize(e.length.toString()))}</div>`)}${e.length ? g("aria-item-list", d`<ol>${e.map((n) => d`<li>${n.label}</li>`)}</ol>`) : null}</div>`;
  }
}
w("calcite-list", K);
export {
