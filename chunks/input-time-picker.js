import { N as G, d as x, a as J, L as Q, h as X, E as d, s as u, x as g, c as Y } from "./iframe.js";
...
import { t as pe, u as fe } from "./useT9n.js";
import { i as m } from "./locale.js";
import { f as P, d as ge, e as be, t as B, i as ve, p as Se, j as $e, g as U, a as C, b as we, c as D, k as ke, m as K } from "./time.js";
import { c as Pe } from "./text.js";
...
class xe extends G {
  ...
  nudgeFractionalSecond(e) {
    ...
    let r, o, l, c;
    e === "up" && (r = isNaN(a) ? 0 : s + t, o = parseFloat(r.toFixed(i)), l = O(o), c = o < 1 && f(l) > 0 ? P(l, i) : "".padStart(i, "0")), e === "down" && (r = isNaN(a) || a === 0 ? 1 - t : s - t, o = parseFloat(r.toFixed(i)), l = O(o), c = o < 1 && f(l) > 0 && Math.sign(l) === 1 ? P(l, i) : "".padStart(i, "0")), this.setValuePart("fractionalSecond", c);
  }
  setHourFormat() {
    const { hourFormat: e, messages: t } = this.component, i = t._lang;
    this.hourFormat = e === "user" ? ge(i) : e;
  }
  setMeridiemOrder() {
    const { messages: e } = this.component, t = e._lang;
    this.meridiemOrder = be(t);
  }
  toggleMeridiem(e) {
    let t;
  ...
  setValue(e, t = !1) {
    ...
    if (ve(e)) {
      const { hour: S, minute: y, second: b, fractionalSecond: $ } = Se(c, s), {
        hour: w,
        hourSuffix: p,
        minute: h,
        minuteSuffix: z,
      ...
      this.hour = S, this.minute = y, this.second = b, this.fractionalSecond = $, this.localizedHour = w, this.localizedHourSuffix = p, this.localizedMinute = h, this.localizedMinuteSuffix = z, this.localizedSecond = E, this.localizedDecimalSeparator = F, this.localizedFractionalSecond = M, this.localizedSecondSuffix = V, k && (this.meridiem = U(this.hour), this.localizedMeridiem = k);
    } else
      this.hour = null, this.minute = null, this.second = null, this.fractionalSecond = null, this.meridiem = null, this.localizedHour = null, this.localizedHourSuffix = C("hour", o, a), this.localizedMinute = null, this.localizedMinuteSuffix = C("minute", o, a), this.localizedSecond = null, this.localizedDecimalSeparator = we(o, a), this.localizedFractionalSecond = null, this.localizedSecondSuffix = C("second", o, a), this.localizedMeridiem = null;
  ...
  setValuePart(e, t) {
    ...
    if (e === "meridiem") {
      ...
      const p = this[e];
      this[e] = typeof t == "number" ? P(t) : t, this[`localized${Pe(e)}`] = D({
        value: this[e],
        part: e,
        locale: o,
        numberingSystem: s,
...
class ze extends Q {
  constructor() {
    super(), this.messages = fe(), this.time = ye(this), this.disabled = !1, this.focusTrapDisabled = !1, this.hourFormat = "user", this.open = !1, this.overlayPositioning = "absolute", this.placement = "auto", this.readOnly = !1, this.required = !1, this.scale = "m", this.status = "idle", this.step = 60, this.validity = {
      valid: !1,
      badInput: !1,
      customError: !1,
      patternMismatch: !1,
      ...
      typeMismatch: !1,
      valueMissing: !1
    }, this.calciteInputTimePickerBeforeClose = x({ cancelable: !1 }), this.calciteInputTimePickerBeforeOpen = x({ cancelable: !1 }), this.calciteInputTimePickerChange = x(), this.calciteInputTimePickerClose = x({ cancelable: !1 }), this.calciteInputTimePickerOpen = x({ cancelable: !1 }), this.listen("blur", this.blurHandler), this.listen("keydown", this.keyDownHandler), this.listen("calciteTimeChange", this.timeChangeHandler);
  }
  static {
  ...
  render() {
    const { messages: e, readOnly: t, scale: i } = this, { fractionalSecond: a, handleHourKeyDownEvent: s, handleMinuteKeyDownEvent: r, handleSecondKeyDownEvent: o, handleFractionalSecondKeyDownEvent: l, hour: c, hourFormat: S, localizedDecimalSeparator: y, localizedFractionalSecond: b, localizedHour: $, localizedHourSuffix: w, localizedMinute: p, localizedMinuteSuffix: h, localizedSecond: z, localizedSecondSuffix: E, meridiemOrder: V, minute: F, second: M } = this.time, k = "--", _ = m(a), q = m(c), j = m(F), W = m(M), T = f(this.step) > 0, A = S === "12", L = this.step < 60, N = V === 0 || he(this.el) === "rtl", H = !this.disabled && !this.readOnly;
    return se({ disabled: this.disabled, children: g`<div aria-label=${re(this) ?? d} class=${u({
      [n.container]: !0,
      [n.readOnly]: t
    })} role=combobox ${v(this.setContainerEl)}><calcite-icon class=${u(n.clockIcon)} icon=clock .scale=${i === "l" ? "m" : "s"}></calcite-icon><div class=${u(n.inputContainer)} dir=ltr>${A && N && this.renderMeridiem("start") || ""}<span aria-label=${this.messages.hour ?? d} aria-valuemax=23 aria-valuemin=1 aria-valuenow=${q && parseInt(c) || "0"} aria-valuetext=${c ?? d} class=${u({
      [n.empty]: !$,
      [n.hour]: !0,
      [n.input]: !0
    })} @focus=${this.timePartFocusHandler} @keydown=${H ? s : void 0} role=spinbutton tabindex=0 ${v(this.setHourEl)}>${$ || k}</span><span class=${u(n.hourSuffix)}>${w}</span><span aria-label=${this.messages.minute ?? d} aria-valuemax=12 aria-valuemin=1 aria-valuenow=${j && parseInt(F) || "0"} aria-valuetext=${F ?? d} class=${u({
      [n.empty]: !p,
      [n.input]: !0,
      [n.minute]: !0
    })} @focus=${this.timePartFocusHandler} @keydown=${H ? r : void 0} role=spinbutton tabindex=0 ${v(this.setMinuteEl)}>${p || k}</span>${L && g`<span class=${u(n.minuteSuffix)}>${h}</span>` || ""}${L && g`<span aria-label=${this.messages.second ?? d} aria-valuemax=59 aria-valuemin=0 aria-valuenow=${W && parseInt(M) || "0"} aria-valuetext=${M ?? d} class=${u({
      [n.empty]: !z,
      [n.input]: !0,
      [n.second]: !0
    })} @focus=${this.timePartFocusHandler} @keydown=${H ? o : void 0} role=spinbutton tabindex=0 ${v(this.setSecondEl)}>${z || k}</span>` || ""}${T && g`<span class=${u(n.decimalSeparator)}>${y}</span>` || ""}${T && g`<span aria-label=${this.messages.fractionalSecond ?? d} aria-valuemax=999 aria-valuemin=1 aria-valuenow=${_ && parseInt(a) || "0"} aria-valuetext=${b ?? d} class=${u({
      [n.empty]: !b,
      [n.fractionalSecond]: !0,
      [n.input]: !0
    })} @focus=${this.timePartFocusHandler} @keydown=${H ? l : void 0} role=spinbutton tabindex=0 ${v(this.setFractionalSecondEl)}>${b || "".padStart(f(this.step), "-")}</span>` || ""}${E && g`<span class=${u(n.secondSuffix)}>${E}</span>` || ""}${A && !N && this.renderMeridiem("end") || ""}</div>${!this.readOnly && this.renderToggleIcon(this.open) || ""}</div><calcite-popover auto-close .focusTrapDisabled=${this.focusTrapDisabled} .focusTrapOptions=${{ initialFocus: !1 }} .label=${e.chooseTime} lang=${this.messages._lang ?? d} @calcitePopoverBeforeClose=${this.popoverBeforeCloseHandler} @calcitePopoverBeforeOpen=${this.popoverBeforeOpenHandler} @calcitePopoverClose=${this.popoverCloseHandler} @calcitePopoverOpen=${this.popoverOpenHandler} .overlayPositioning=${this.overlayPositioning} .placement=${this.placement} .referenceElement=${this.containerEl} trigger-disabled ${v(this.setCalcitePopoverEl)}><calcite-time-picker .hourFormat=${this.time.hourFormat} lang=${this.messages._lang ?? d} .messageOverrides=${this.messageOverrides} .numberingSystem=${this.numberingSystem} @calciteTimePickerChange=${this.timePickerChangeHandler} .scale=${this.scale} .step=${this.step} tabindex=${(this.open ? void 0 : -1) ?? d} .value=${this.value}></calcite-time-picker></calcite-popover>${ie({ component: this })}${this.validationMessage && this.status === "invalid" ? de({ icon: this.validationIcon, id: Me.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
  renderMeridiem(e) {
...
Y("calcite-input-time-picker", ze);
export {
