...
import { f as x, d as ge, e as ve, t as B, i as be, p as Se, j as $e, g as U, a as C, b as we, c as D, k as ke, m as K } from "./time.js";
...
class Pe extends G {
  ...
  setValue(e, t = !1) {
    ...
    if (be(e)) {
      const { hour: S, minute: y, second: v, fractionalSecond: $ } = Se(c, s), {
        hour: w,
        hourSuffix: p,
        minute: h,
        minuteSuffix: z,
  ...
  setValuePart(e, t) {
    ...
    if (e === "meridiem") {
      ...
      const p = this[e];
      this[e] = typeof t == "number" ? x(t) : t, this[`localized${xe(e)}`] = D({
        value: this[e],
        part: e,
        locale: o,
        numberingSystem: s,
...
class ze extends Q {
  ...
  render() {
    const { messages: e, readOnly: t, scale: i } = this, { fractionalSecond: a, handleHourKeyDownEvent: s, handleMinuteKeyDownEvent: r, handleSecondKeyDownEvent: o, handleFractionalSecondKeyDownEvent: l, hour: c, hourFormat: S, localizedDecimalSeparator: y, localizedFractionalSecond: v, localizedHour: $, localizedHourSuffix: w, localizedMinute: p, localizedMinuteSuffix: h, localizedSecond: z, localizedSecondSuffix: E, meridiemOrder: V, minute: F, second: M } = this.time, k = "--", _ = m(a), q = m(c), j = m(F), W = m(M), T = f(this.step) > 0, A = S === "12", L = this.step < 60, N = V === 0 || he(this.el) === "rtl", H = !this.disabled && !this.readOnly;
    return se({ disabled: this.disabled, children: g`<div aria-label=${re(this) ?? d} class=${u({
      [n.container]: !0,
      [n.readOnly]: t
    })} role=combobox ${b(this.setContainerEl)}><calcite-icon class=${u(n.clockIcon)} icon=clock .scale=${i === "l" ? "m" : "s"}></calcite-icon><div class=${u(n.inputContainer)} dir=ltr>${A && N && this.renderMeridiem("start") || ""}<span aria-label=${this.messages.hour ?? d} aria-valuemax=23 aria-valuemin=1 aria-valuenow=${q && parseInt(c) || "0"} aria-valuetext=${c ?? d} class=${u({
      [n.empty]: !$,
      [n.hour]: !0,
      [n.input]: !0
