import { N as J, d as x, a as Q, L as X, h as Y, s as u, E as d, x as g, c as Z } from "./iframe.js";
import { n as v } from "./ref.js";
import { c as ee, d as te, s as ie, H as ae } from "./form.js";
import { u as se, I as ne } from "./interactive.js";
import { c as oe, d as re, g as le } from "./label.js";
...
class ye extends J {
  ...
  nudgeFractionalSecond(e) {
    ...
    let r, o, l, c;
    e === "up" && (r = isNaN(a) ? 0 : s + t, o = parseFloat(r.toFixed(i)), l = C(o), c = o < 1 && f(l) > 0 ? P(l, i) : "".padStart(i, "0")), e === "down" && (r = isNaN(a) || a === 0 ? 1 - t : s - t, o = parseFloat(r.toFixed(i)), l = C(o), c = o < 1 && f(l) > 0 && Math.sign(l) === 1 ? P(l, i) : "".padStart(i, "0")), this.setValuePart("fractionalSecond", c);
  }
  setHourFormat() {
  ...
  setValue(e, t = !1) {
    const { messages: i, numberingSystem: a, step: s, value: r } = this.component, o = i._lang, l = this.hourFormat === "12", c = U(e, s);
    if (Se(e)) {
      const { hour: S, minute: y, second: b, fractionalSecond: $ } = $e(c, s), {
        hour: w,
        hourSuffix: p,
        minute: h,
        minuteSuffix: z,
      ...
      this.hour = null, this.minute = null, this.second = null, this.fractionalSecond = null, this.meridiem = null, this.localizedHour = null, this.localizedHourSuffix = O("hour", o, a), this.localizedMinute = null, this.localizedMinuteSuffix = O("minute", o, a), this.localizedSecond = null, this.localizedDecimalSeparator = ke(o, a), this.localizedFractionalSecond = null, this.localizedSecondSuffix = O("second", o, a), this.localizedMeridiem = null;
  ...
  setValuePart(e, t) {
    ...
    if (e === "meridiem") {
      ...
      const p = this[e];
      this[e] = typeof t == "number" ? P(t) : t, this[`localized${xe(e)}`] = D({
        value: this[e],
        part: e,
        locale: o,
        numberingSystem: s,
...
class ze extends X {
  ...
  connectedCallback() {
    super.connectedCallback(), oe(this), ee(this);
  }
  willUpdate(e) {
  ...
  render() {
    const { messages: e, readOnly: t, scale: i } = this, { fractionalSecond: a, handleHourKeyDownEvent: s, handleMinuteKeyDownEvent: r, handleSecondKeyDownEvent: o, handleFractionalSecondKeyDownEvent: l, hour: c, hourFormat: S, localizedDecimalSeparator: y, localizedFractionalSecond: b, localizedHour: $, localizedHourSuffix: w, localizedMinute: p, localizedMinuteSuffix: h, localizedSecond: z, localizedSecondSuffix: E, meridiemOrder: V, minute: F, second: M } = this.time, k = "--", q = m(a), j = m(c), W = m(F), G = m(M), A = f(this.step) > 0, L = S === "12", N = this.step < 60, B = V === 0 || me(this.el) === "rtl", H = !this.disabled && !this.readOnly;
    return ne({ disabled: this.disabled, children: g`<div aria-controls=${n.inputContainer} aria-labelledby=${T.inputContainer} class=${u({
      [n.container]: !0,
      [n.readOnly]: t
    })} role=combobox ${v(this.setContainerEl)}><calcite-icon class=${u(n.clockIcon)} icon=clock .scale=${i === "l" ? "m" : "s"}></calcite-icon><div aria-label=${le(this) ?? d} class=${u(n.inputContainer)} dir=ltr id=${T.inputContainer} role=group>${L && B && this.renderMeridiem("start") || ""}<span aria-label=${this.messages.hour ?? d} aria-valuemax=23 aria-valuemin=1 aria-valuenow=${j && parseInt(c) || "0"} aria-valuetext=${c ?? d} class=${u({
      [n.empty]: !$,
      [n.hour]: !0,
      [n.input]: !0
    })} @focus=${this.timePartFocusHandler} @keydown=${H ? s : void 0} role=spinbutton tabindex=0 ${v(this.setHourEl)}>${$ || k}</span><span class=${u(n.hourSuffix)}>${w}</span><span aria-label=${this.messages.minute ?? d} aria-valuemax=12 aria-valuemin=1 aria-valuenow=${W && parseInt(F) || "0"} aria-valuetext=${F ?? d} class=${u({
      [n.empty]: !p,
      [n.input]: !0,
      [n.minute]: !0
    })} @focus=${this.timePartFocusHandler} @keydown=${H ? r : void 0} role=spinbutton tabindex=0 ${v(this.setMinuteEl)}>${p || k}</span>${N && g`<span class=${u(n.minuteSuffix)}>${h}</span>` || ""}${N && g`<span aria-label=${this.messages.second ?? d} aria-valuemax=59 aria-valuemin=0 aria-valuenow=${G && parseInt(M) || "0"} aria-valuetext=${M ?? d} class=${u({
      [n.empty]: !z,
      [n.input]: !0,
      [n.second]: !0
    })} @focus=${this.timePartFocusHandler} @keydown=${H ? o : void 0} role=spinbutton tabindex=0 ${v(this.setSecondEl)}>${z || k}</span>` || ""}${A && g`<span class=${u(n.decimalSeparator)}>${y}</span>` || ""}${A && g`<span aria-label=${this.messages.fractionalSecond ?? d} aria-valuemax=999 aria-valuemin=1 aria-valuenow=${q && parseInt(a) || "0"} aria-valuetext=${b ?? d} class=${u({
      [n.empty]: !b,
      [n.fractionalSecond]: !0,
      [n.input]: !0
    })} @focus=${this.timePartFocusHandler} @keydown=${H ? l : void 0} role=spinbutton tabindex=0 ${v(this.setFractionalSecondEl)}>${b || "".padStart(f(this.step), "-")}</span>` || ""}${E && g`<span class=${u(n.secondSuffix)}>${E}</span>` || ""}${L && !B && this.renderMeridiem("end") || ""}</div>${!this.readOnly && this.renderToggleIcon(this.open) || ""}</div><calcite-popover auto-close .focusTrapDisabled=${this.focusTrapDisabled} .focusTrapOptions=${{ initialFocus: !1 }} .label=${e.chooseTime} lang=${this.messages._lang ?? d} @calcitePopoverBeforeClose=${this.popoverBeforeCloseHandler} @calcitePopoverBeforeOpen=${this.popoverBeforeOpenHandler} @calcitePopoverClose=${this.popoverCloseHandler} @calcitePopoverOpen=${this.popoverOpenHandler} .overlayPositioning=${this.overlayPositioning} .placement=${this.placement} .referenceElement=${this.containerEl} trigger-disabled ${v(this.setCalcitePopoverEl)}><calcite-time-picker .hourFormat=${this.time.hourFormat} lang=${this.messages._lang ?? d} .messageOverrides=${this.messageOverrides} .numberingSystem=${this.numberingSystem} @calciteTimePickerChange=${this.timePickerChangeHandler} .scale=${this.scale} .step=${this.step} tabindex=${(this.open ? void 0 : -1) ?? d} .value=${this.value}></calcite-time-picker></calcite-popover>${ae({ component: this })}${this.validationMessage && this.status === "invalid" ? ue({ icon: this.validationIcon, id: T.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
  renderMeridiem(e) {
  ...
  renderToggleIcon(e) {
    return g`<span class=${u(n.toggleIcon)} @click=${this.toggleIconClickHandler}><calcite-icon .icon=${e ? "chevron-up" : "chevron-down"} .scale=${de(this.scale)}></calcite-icon></span>`;
  }
}
