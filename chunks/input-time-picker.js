...
import { a as N, L as K, c as u, F as j, A as t, s as c, b as r, d as _ } from "./index.js";
...
class re extends K {
  ...
  render() {
    const { messages: e, readOnly: n, scale: a } = this, { fractionalSecond: o, handleHourKeyDownEvent: p, handleMinuteKeyDownEvent: f, handleSecondKeyDownEvent: g, handleFractionalSecondKeyDownEvent: O, hour: b, hourFormat: R, localizedDecimalSeparator: z, localizedFractionalSecond: k, localizedHour: P, localizedHourSuffix: E, localizedMinute: S, localizedMinuteSuffix: D, localizedSecond: H, localizedSecondSuffix: C, meridiemOrder: V, minute: $, second: x } = this.time, w = "--", M = m(o), B = m(b), q = m($), A = m(x), F = h(this.step) > 0, I = R === "12", L = this.step < 60, T = V === 0 || this.direction === "rtl", d = !this.disabled && !this.readOnly, U = this.clearable && !!this.value && d;
    return this.interactiveContainer({ disabled: this.disabled, children: r`${this.labelText && Y({ labelText: this.labelText, onClick: this.onLabelClick, required: this.required, tooltipText: this.messages.required }) || ""}<div aria-controls=${v.inputContainer} aria-expanded=${ne(this.open) ?? t} aria-labelledby=${v.inputContainer} class=${c({
      [i.container]: !0,
      [i.readOnly]: n
    ...
    })} dir=ltr id=${v.inputContainer} role=group>${I && T && this.renderMeridiem() || ""}<span aria-label=${this.messages.hour ?? t} aria-valuemax=23 aria-valuemin=1 aria-valuenow=${(B && parseInt(b, 10) || "0") ?? t} aria-valuetext=${b ?? t} class=${c({
      [i.empty]: !P,
      [i.hour]: !0,
      [i.input]: !0
    })} @focus=${this.timePartFocusHandler} @keydown=${d ? p : void 0} @mousedown=${this.mouseDownHandler} role=spinbutton tabindex=0 ${s(this.hourRef)}>${P || w}</span><span class=${c(i.hourSuffix)}>${E}</span><span aria-label=${this.messages.minute ?? t} aria-valuemax=12 aria-valuemin=1 aria-valuenow=${(q && parseInt($, 10) || "0") ?? t} aria-valuetext=${$ ?? t} class=${c({
      [i.empty]: !S,
      [i.input]: !0,
      [i.minute]: !0
    })} @focus=${this.timePartFocusHandler} @keydown=${d ? f : void 0} @mousedown=${this.mouseDownHandler} role=spinbutton tabindex=0 ${s(this.minuteRef)}>${S || w}</span><span class=${c(i.minuteSuffix)}>${D}</span>${L && r`<span aria-label=${this.messages.second ?? t} aria-valuemax=59 aria-valuemin=0 aria-valuenow=${(A && parseInt(x, 10) || "0") ?? t} aria-valuetext=${x ?? t} class=${c({
      [i.empty]: !H,
      [i.input]: !0,
      [i.second]: !0
...
_("calcite-input-time-picker", re);
export {
  re as InputTimePicker
};
