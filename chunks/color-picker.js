...
import { a as he, L as de, c as H, l as $, s as h, o as x, b as C, A as k, d as ue } from "./index.js";
import { c as pe } from "./repeat.js";
import { i as I } from "./keyed.js";
import { D as g, C as E, g as Ce, a as fe, S as D, n as O, h as m, p as j, c as U, b as R, d as P, e as W, f as A, O as b, R as N, H as f, i as n, o as _, t as ve, j as V, k as B, l as ge, m as z, q as X, I as q } from "./utils4.js";
import { t as G } from "./throttle.js";
import { e as L, n as S } from "./ref.js";
import { u as me } from "./index2.js";
import { h as Y } from "./dom.js";
...
class Te extends de {
  ...
  toValue(e, t = this.mode) {
    ...
    if (t.includes("-css")) {
      ...
      if ((t.endsWith("a") || t.endsWith("a-css")) && e.alpha() === 1) {
        const u = r.slice(0, 3), p = r.slice(4, -1);
        return `${u}a(${p}, ${e.alpha()})`;
      }
      return r;
    }
  ...
  drawThumb(e, t, i, a, o, s) {
    ...
    if (e.beginPath(), e.arc(i, a, t, 0, c), e.fillStyle = "#fff", e.fill(), e.strokeStyle = "rgba(0,0,0,0.3)", e.lineWidth = r, e.stroke(), s && o.alpha() < 1) {
      ...
      e.beginPath(), e.arc(i, a, t - 3, 0, c), e.fillStyle = u, e.fill();
    }
  ...
  drawHueSlider() {
    ...
    let d = 0;
    c.forEach((u) => {
      l.addColorStop(d, A(u).string()), d += r;
  ...
  drawOpacitySlider() {
    ...
    const c = e.createLinearGradient(0, s, l, 0), r = t.rgb().alpha(0), d = t.rgb().alpha(0.5), u = t.rgb().alpha(1);
    c.addColorStop(0, r.string()), c.addColorStop(0.5, d.string()), c.addColorStop(1, u.string()), this.drawSliderPath(e, i, l, o, s);
    const p = e.createPattern(this.getCheckeredBackgroundPattern(), "repeat");
    e.fillStyle = p, e.fill(), e.fillStyle = c, e.fill(), e.strokeStyle = "rgba(0,0,0,0.3)", e.lineWidth = 1, e.stroke(), this.drawActiveOpacitySliderColor();
  }
  ...
  drawActiveOpacitySliderColor() {
    ...
    if (!e)
      return;
    const t = e, { staticDimensions: { thumb: { radius: i } } } = this, a = this.effectiveSliderWidth, o = z(t.alpha()) / (b.max / a), s = i, l = this.getSliderBoundX(o, a, i);
    requestAnimationFrame(() => {
  ...
  render() {
    const { channelsDisabled: e, color: t, colorFieldScopeLeft: i, colorFieldScopeTop: a, staticDimensions: { thumb: { radius: o } }, fieldDisabled: s, hexDisabled: l, hueScopeLeft: c, messages: r, alphaChannel: d, opacityScopeLeft: u, savedColors: p, savedDisabled: w, scale: v, scopeOrientation: Q } = this, M = this.effectiveSliderWidth, T = t ? m(t, d) : void 0, ee = o, te = c ?? M * g.hue() / f.h, ie = o, oe = u ?? M * z(g.alpha()) / b.max, K = t === void 0, F = Q === "vertical", [ae, se] = this.getAdjustedScopePosition(i, a), [le, ne] = this.getAdjustedScopePosition(te, ee), [re, ce] = this.getAdjustedScopePosition(oe, ie);
    return this.interactiveContainer({ disabled: this.disabled, children: C`<div class=${h(n.container)}>${s ? null : C`<div class=${h(n.controlAndScope)}><canvas class=${h(n.colorField)} @pointerdown=${this.handleColorFieldPointerDown} ${S(this.initColorField)}></canvas><div .ariaLabel=${F ? r.value : r.saturation} .ariaValueMax=${F ? f.v : f.s} aria-valuemin=0 .ariaValueNow=${(F ? t?.saturationv() : t?.value()) || "0"} class=${h({ [n.scope]: !0, [n.colorFieldScope]: !0 })} @keydown=${this.handleColorFieldScopeKeyDown} role=slider style=${x({
      ...
      [n.splitSection]: !0
    })} .scale=${v === "l" ? "m" : "s"}><calcite-tab-nav slot=title-group>${this.renderChannelsTabTitle("rgb")}${this.renderChannelsTabTitle("hsv")}</calcite-tab-nav>${this.renderChannelsTab("rgb")}${this.renderChannelsTab("hsv")}</calcite-tabs>`}</div></div>`}${w ? null : C`<div class=${h({ [n.savedColorsSection]: !0, [n.section]: !0 })}><div class=${h(n.header)}><label>${r.saved}</label><div class=${h(n.savedColorsButtons)}><calcite-button appearance=transparent class=${h(n.deleteColor)} .disabled=${K} .iconStart=${q.minus} kind=neutral .label=${r.deleteColor} @click=${this.deleteColor} .scale=${v} type=button></calcite-button><calcite-button appearance=transparent class=${h(n.saveColor)} .disabled=${K} .iconStart=${q.plus} kind=neutral .label=${r.saveColor} @click=${this.saveColor} .scale=${v} type=button></calcite-button></div></div>${p.length > 0 ? C`<calcite-swatch-group class=${h(n.swatchGroup)} .label=${r.saved} .scale=${v} selection-mode=none>${pe(p, (y) => y, (y) => C`<calcite-swatch class=${h(n.savedColor)} .color=${y} .label=${y} @click=${this.handleSavedColorSelect} @keydown=${this.handleSavedColorKeyDown} .scale=${v} tabindex=0></calcite-swatch>`)}</calcite-swatch-group>` : null}</div>`}</div>` });
  }
  ...
  renderChannelsTab(e) {
    const { clearable: t, channelMode: i, channels: a, messages: o, alphaChannel: s } = this, l = e === i, r = e === "rgb" ? [o.red, o.green, o.blue] : [o.hue, o.saturation, o.value], d = this.direction, u = s ? a : a.slice(0, 3);
    return I(e, C`<calcite-tab class=${h(n.control)} .selected=${l}><div class=${h(n.channels)} dir=ltr>${u.map((p, w) => {
      const v = w === 3;
      return v && (p = // channels can only be undefined when clearable
      t && !p ? p : z(p)), this.renderChannel(p, w, r[w], d, v ? "%" : "");
    })}</div></calcite-tab>`);
  }
