import { b as re, L as ce, c as K, s as h, w as x, x as u, E as k, q as he } from "./index.js";
import { c as de } from "./repeat.js";
import { i as O } from "./keyed.js";
import { D as m, C as H, g as ue, a as pe, S as D, n as R, h as g, p as U, b, c as P, d as A, e as Ce, f as j, O as S, R as T, H as p, i as n, o as W, t as fe, j as _, k as N, l as ve, m as z, q as B, I as V } from "./utils4.js";
import { e as L, n as w } from "./ref.js";
import { e as X, a as me } from "./dom.js";
import { u as ge, I as be } from "./interactive.js";
import { i as Se } from "./key.js";
import { c as we, r as q, a as ye } from "./math.js";
...
class Te extends ce {
  constructor() {
    ...
    }, this.cancelable = xe()(this), this.drawColorControls = G((e = "all") => {
    ...
    }, Y), this.globalPointerMoveHandler = (e) => {
    ...
    }, Y), this.updateDynamicDimensions = (e) => {
      const t = {
        width: ue(e, this.staticDimensions, this.alphaChannel),
  ...
  async load() {
    this._valueWasSet || (this._value ??= R(g(m, this.alphaChannel))), this.handleAllowEmptyOrClearableChange();
    const { isClearable: e, color: t, format: i, value: a } = this, o = e && !a, l = U(a), s = o || i === "auto" && l || i === l, r = o ? null : s ? b(a) : t;
    s || this.showIncompatibleColorWarning(a, i), this.setMode(i, !1), this.internalColorSet(r, !1, "initial"), this.updateStaticDimensions(this.scale), this.updateDynamicDimensions(D[this.scale].minWidth);
    const c = `${P}${this.storageId}`;
  ...
  handleValueChange(e, t) {
    ...
    const r = i && !e ? null : b(e != null && typeof e == "object" && A(this.mode) ? Ce(e) : e), c = !j(r, this.color);
  ...
  handleHexInputChange(e) {
    ...
    const l = i && R(g(i, A(this.mode)));
    o !== l && this.internalColorSet(b(o));
  }
  ...
  handleChannelInput(e) {
    const t = e.currentTarget, i = Number(t.getAttribute("data-channel-index")), o = i === 3 ? S.max : this.channelMode === "rgb" ? T[Object.keys(T)[i]] : p[Object.keys(p)[i]];
    let l;
    if (!t.value)
  ...
  getChannelInputLimit(e) {
    return this.channelMode === "rgb" ? T[Object.keys(T)[e]] : p[Object.keys(p)[e]];
  }
  handleChannelChange(e) {
    ...
    a[i] = l ? W(s) : s, this.updateColorFromChannels(a);
  }
  ...
  handleCanvasControlPointerDown(e, t, i, a) {
    X(e) && (window.addEventListener("pointermove", this.globalPointerMoveHandler), window.addEventListener("pointerup", this.globalPointerUpHandler, {
      once: !0
  ...
  ensureCompatibleMode(e, t) {
    const { alphaChannel: i } = this, a = A(e);
    if (i && !a) {
      const o = fe(e);
  ...
  captureOpacitySliderValue(e) {
    const t = W(S.max / this.effectiveSliderWidth * e);
  ...
  drawActiveHueSliderColor() {
    ...
    if (!e)
      return;
    const t = e.hsv().saturationv(100).value(100), { staticDimensions: { thumb: { radius: i } } } = this, a = this.effectiveSliderWidth, o = t.hue() / (N / a), l = i, s = this.getSliderBoundX(o, a, i);
    requestAnimationFrame(() => {
      this.hueScopeLeft = s;
  ...
  drawHueSlider() {
    ...
    let d = 0;
    r.forEach((f) => {
      s.addColorStop(d, b(f).string()), d += c;
  ...
  drawActiveOpacitySliderColor() {
    ...
    if (!e)
      return;
    const t = e, { staticDimensions: { thumb: { radius: i } } } = this, a = this.effectiveSliderWidth, o = z(t.alpha()) / (S.max / a), l = i, s = this.getSliderBoundX(o, a, i);
    requestAnimationFrame(() => {
  ...
  getSliderBoundX(e, t, i) {
    const a = ye(e, t, i);
    return a === 0 ? e : a === -1 ? q(e, 0, t, i, i * 2) : q(e, 0, t, t - i * 2, t - i);
  }
  ...
  render() {
    const { channelsDisabled: e, color: t, colorFieldScopeLeft: i, colorFieldScopeTop: a, staticDimensions: { thumb: { radius: o } }, fieldDisabled: l, hexDisabled: s, hueScopeLeft: r, messages: c, alphaChannel: d, opacityScopeLeft: f, savedColors: C, savedDisabled: y, scale: v, scopeOrientation: J } = this, M = this.effectiveSliderWidth, F = t ? g(t, d) : null, Z = o, Q = r ?? M * m.hue() / p.h, ee = o, te = f ?? M * z(m.alpha()) / S.max, E = t === void 0, I = J === "vertical", [ie, ae] = this.getAdjustedScopePosition(i, a), [oe, le] = this.getAdjustedScopePosition(Q, Z), [se, ne] = this.getAdjustedScopePosition(te, ee);
    return be({ disabled: this.disabled, children: u`<div class=${h(n.container)}>${l ? null : u`<div class=${h(n.controlAndScope)}><canvas class=${h(n.colorField)} @pointerdown=${this.handleColorFieldPointerDown} ${w(this.initColorField)}></canvas><div .ariaLabel=${I ? c.value : c.saturation} .ariaValueMax=${I ? p.v : p.s} aria-valuemin=0 .ariaValueNow=${(I ? t?.saturationv() : t?.value()) || "0"} class=${h({ [n.scope]: !0, [n.colorFieldScope]: !0 })} @keydown=${this.handleColorFieldScopeKeyDown} role=slider style=${x({
      top: `${ae || 0}px`,
      left: `${ie || 0}px`
    })} tabindex=0 ${w(this.colorFieldScopeRef)}></div></div>`}<div class=${h(n.previewAndSliders)}><calcite-swatch class=${h(n.preview)} .color=${F} .label=${F} .scale=${this.alphaChannel ? "l" : this.scale}></calcite-swatch><div class=${h(n.sliders)}><div class=${h(n.controlAndScope)}><canvas class=${h({ [n.slider]: !0, [n.hueSlider]: !0 })} @pointerdown=${this.handleHueSliderPointerDown} ${w(this.initHueSlider)}></canvas><div .ariaLabel=${c.hue} .ariaValueMax=${p.h} aria-valuemin=0 .ariaValueNow=${t?.round().hue() || m.round().hue()} class=${h({ [n.scope]: !0, [n.hueScope]: !0 })} @keydown=${this.handleHueScopeKeyDown} role=slider style=${x({
      top: `${le}px`,
      left: `${oe}px`
    })} tabindex=0 ${w(this.hueScopeRef)}></div></div>${d ? u`<div class=${h(n.controlAndScope)}><canvas class=${h({ [n.slider]: !0, [n.opacitySlider]: !0 })} @pointerdown=${this.handleOpacitySliderPointerDown} ${w(this.initOpacitySlider)}></canvas><div .ariaLabel=${c.opacity} .ariaValueMax=${S.max} .ariaValueMin=${S.min} .ariaValueNow=${(t || m).round().alpha()} class=${h({ [n.scope]: !0, [n.opacityScope]: !0 })} @keydown=${this.handleOpacityScopeKeyDown} role=slider style=${x({
      top: `${ne}px`,
      left: `${se}px`
    })} tabindex=0 ${w(this.opacityScopeRef)}></div></div>` : null}</div></div>${s && e ? null : u`<div class=${h({
      [n.controlSection]: !0,
      [n.section]: !0
      ...
      [n.splitSection]: !0
    })} .scale=${v === "l" ? "m" : "s"}><calcite-tab-nav slot=title-group>${this.renderChannelsTabTitle("rgb")}${this.renderChannelsTabTitle("hsv")}</calcite-tab-nav>${this.renderChannelsTab("rgb")}${this.renderChannelsTab("hsv")}</calcite-tabs>`}</div></div>`}${y ? null : u`<div class=${h({ [n.savedColorsSection]: !0, [n.section]: !0 })}><div class=${h(n.header)}><label>${c.saved}</label><div class=${h(n.savedColorsButtons)}><calcite-button appearance=transparent class=${h(n.deleteColor)} .disabled=${E} .iconStart=${V.minus} kind=neutral .label=${c.deleteColor} @click=${this.deleteColor} .scale=${v} type=button></calcite-button><calcite-button appearance=transparent class=${h(n.saveColor)} .disabled=${E} .iconStart=${V.plus} kind=neutral .label=${c.saveColor} @click=${this.saveColor} .scale=${v} type=button></calcite-button></div></div>${C.length > 0 ? u`<calcite-swatch-group class=${h(n.swatchGroup)} .label=${c.saved} .scale=${v} selection-mode=none>${de(C, ($) => $, ($) => u`<calcite-swatch class=${h(n.savedColor)} .color=${$} .label=${$} @click=${this.handleSavedColorSelect} @keydown=${this.handleSavedColorKeyDown} .scale=${v} tabindex=0></calcite-swatch>`)}</calcite-swatch-group>` : null}</div>`}</div>` });
  }
  renderChannelsTabTitle(e) {
    const { channelMode: t, messages: i } = this, a = e === t, o = e === "rgb" ? i.rgb : i.hsv;
    return O(e, u`<calcite-tab-title class=${h(n.colorMode)} data-color-mode=${e ?? k} @calciteTabsActivate=${this.handleTabActivate} .selected=${a}>${o}</calcite-tab-title>`);
  }
  renderChannelsTab(e) {
    ...
    return O(e, u`<calcite-tab class=${h(n.control)} .selected=${s}><div class=${h(n.channels)} dir=ltr>${f.map((C, y) => {
      const v = y === 3;
      return v && (C = t && !C ? C : z(C)), this.renderChannel(C, y, c[y], d, v ? "%" : "");
    })}</div></calcite-tab>`);
  }
  renderChannel(e, t, i, a, o) {
    return O(t, u`<calcite-input-number class=${h(n.channel)} data-channel-index=${t ?? k} dir=${a ?? k} .label=${i} lang=${this.messages._lang ?? k} number-button-type=none .numberingSystem=${this.numberingSystem} @keydown=${this.handleKeyDown} @calciteInputNumberChange=${this.handleChannelChange} @calciteInputNumberInput=${this.handleChannelInput} @calciteInternalInputNumberBlur=${this.handleChannelBlur} @calciteInternalInputNumberFocus=${this.handleChannelFocus} .scale=${this.scale === "l" ? "m" : "s"} style=${x({
      marginLeft: t > 0 && !(this.scale === "s" && this.alphaChannel && t === 3) ? "-1px" : ""
    })} .suffixText=${o} .value=${e?.toString()}></calcite-input-number>`);
  }
}
he("calcite-color-picker", Te);
export {
  Te as ColorPicker
};
