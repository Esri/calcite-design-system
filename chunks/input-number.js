import { a as D, L as F, d as y, h as N, s, x as u, E as m, C as O, c as T } from "./iframe.js";
...
import { c as Q, g as x } from "./component.js";
import { i as g, n as r, B as X, p as tt, s as et, h as it } from "./locale.js";
import { V as rt } from "./Validation.js";
import { s as nt } from "./input.js";
...
class lt extends F {
  constructor() {
    super(), this.actionWrapperEl = V(), this.attributeWatch = j(["autofocus", "enterkeyhint", "inputmode"], this.handleGlobalAttributesChanged), this.inputWrapperEl = V(), this.onHiddenFormInputInput = (t) => {
      ...
      typeMismatch: !1,
      valueMissing: !1
    }, this.calciteInputNumberChange = y({ cancelable: !1 }), this.calciteInputNumberInput = y(), this.calciteInternalInputNumberBlur = y({ cancelable: !1 }), this.calciteInternalInputNumberFocus = y({ cancelable: !1 }), this.listen("click", this.clickHandler), this.listen("keydown", this.keyDownHandler);
  }
  static {
  ...
  willUpdate(t) {
    t.has("max") && (this.maxString = this.max?.toString() || null), t.has("min") && (this.minString = this.min?.toString() || null), t.has("icon") && (this.requestedIcon = $({}, this.icon, "number")), t.has("messages") && (r.numberFormatOptions = {
  ...
  incrementOrDecrementNumberValue(t, e, i, d) {
    ...
    if (a === "Infinity" || a === "-Infinity")
      return;
    const c = t === "up" ? 1 : -1, o = this.integer && this.step !== "any" ? Math.round(this.step) : this.step, p = o === "any" ? 1 : Math.abs(o || 1), h = new X(a !== "" ? a : "0").add(`${p * c}`), f = () => typeof i == "number" && !isNaN(i) && h.subtract(`${i}`).isNegative, b = () => typeof e == "number" && !isNaN(e) && !h.subtract(`${e}`).isNegative, l = f() ? `${i}` : b() ? `${e}` : h.toString();
    this.setNumberValue({
      committing: !0,
      nativeEvent: d,
      origin: "user",
  ...
  inputNumberKeyDownHandler(t) {
    ...
    r.numberFormatOptions = {
      ...
      numberingSystem: this.numberingSystem,
      useGrouping: this.groupSeparator
    }, !(t.key === r.decimal && !this.integer && (!this.value && !this.childNumberEl.value || this.value && this.childNumberEl.value.indexOf(r.decimal) === -1)) && (/[eE]/.test(t.key) && !this.integer && (!this.value && !this.childNumberEl.value || this.value && !/[eE]/.test(this.childNumberEl.value)) || t.key === "-" && (!this.value && !this.childNumberEl.value || this.value && this.childNumberEl.value.split("-").length <= 2) || t.preventDefault());
  }
  ...
  setNumberValue({ committing: t = !1, nativeEvent: e, origin: i, previousValue: d, value: a }) {
    ...
    const c = this.previousValue?.length > a.length || this.value?.length > a.length, o = this.integer ? a.replace(/[e.]/g, "") : a, p = o.charAt(o.length - 1) === ".", v = o.charAt(0) === "-", h = o.match(/^-?(0+)\d/), f = p && c ? o : et(o), b = a && !f ? g(this.previousValue) ? this.previousValue : "" : f;
    let l = r.localize(b);
    i !== "connected" && !p && (l = it(l, b, r)), p && c && (l = `${l}${r.decimal}`), h && (l = `${v ? l.charAt(0) : ""}${r.localize("0").repeat(h[1].length)}${v ? l.slice(1) : l}`), this.displayedValue = l, this.setPreviousNumberValue(d ?? this.value), this.previousValueOrigin = i, this.userChangedValue = i === "user" && this.value !== b;
    ...
    i === "direct" && (this.setInputNumberValue(l), this.setPreviousEmittedNumberValue(z)), e && (this.calciteInputNumberInput.emit().defaultPrevented ? (this.value = this.previousValue, this.displayedValue = r.localize(this.previousValue)) : t && this.emitChangeIfUserModified());
  }
  ...
  render() {
    const t = A(this.el), e = u`<div class=${s(n.loader)}><calcite-progress .label=${this.messages.loading} type=indeterminate></calcite-progress></div>`, i = u`<button .ariaLabel=${this.messages.clear} class=${s(n.clearButton)} .disabled=${this.disabled || this.readOnly} @click=${this.clearInputValue} tabindex=-1 type=button><calcite-icon icon=x .scale=${x(this.scale)}></calcite-icon></button>`, d = u`<calcite-icon class=${s(n.inputIcon)} .flipRtl=${this.iconFlipRtl} .icon=${this.requestedIcon} .scale=${x(this.scale)}></calcite-icon>`, a = this.numberButtonType === "horizontal", c = u`<button aria-hidden=true class=${s({
      [n.numberButtonItem]: !0,
      [n.buttonItemHorizontal]: a
    })} data-adjustment=up .disabled=${this.disabled || this.readOnly} @pointerdown=${this.nudgeButtonPointerDownHandler} @pointerout=${this.nudgeButtonPointerOutHandler} @pointerup=${this.nudgeButtonPointerUpHandler} tabindex=-1 type=button><calcite-icon icon=chevron-up .scale=${x(this.scale)}></calcite-icon></button>`, o = u`<button aria-hidden=true class=${s({
      [n.numberButtonItem]: !0,
      [n.buttonItemHorizontal]: a
    })} data-adjustment=down .disabled=${this.disabled || this.readOnly} @pointerdown=${this.nudgeButtonPointerDownHandler} @pointerout=${this.nudgeButtonPointerOutHandler} @pointerup=${this.nudgeButtonPointerUpHandler} tabindex=-1 type=button><calcite-icon icon=chevron-down .scale=${x(this.scale)}></calcite-icon></button>`, p = u`<div class=${s(n.numberButtonWrapper)}>${c}${o}</div>`, v = u`<div class=${s(n.prefix)}>${this.prefixText}</div>`, h = u`<div class=${s(n.suffix)}>${this.suffixText}</div>`, f = L("localized-input", u`<input aria-errormessage=${H.validationMessage} .ariaInvalid=${this.status === "invalid"} .ariaLabel=${J(this)} autocomplete=${this.autocomplete ?? m} .autofocus=${this.el.autofocus} value=${this.defaultValue ?? m} .disabled=${this.disabled} enterkeyhint=${this.el.enterKeyHint ?? m} inputmode=${this.el.inputMode || "decimal"} maxlength=${this.maxLength ?? m} minlength=${this.minLength ?? m} name=${m} @blur=${this.inputNumberBlurHandler} @focus=${this.inputNumberFocusHandler} @input=${this.inputNumberInputHandler} @keydown=${this.inputNumberKeyDownHandler} @keyup=${this.inputNumberKeyUpHandler} placeholder=${(this.placeholder || "") ?? m} .readOnly=${this.readOnly} type=text .value=${P(this.displayedValue ?? "")} ${I(this.setChildNumberElRef)}>`);
    return R({ disabled: this.disabled, children: u`<div class=${s({
      [n.inputWrapper]: !0,
      [O.rtl]: t === "rtl",
      [n.hasSuffix]: this.suffixText,
      [n.hasPrefix]: this.prefixText,
      [n.clearable]: this.isClearable
    })} ${I(this.inputWrapperEl)}>${this.numberButtonType === "horizontal" && !this.readOnly ? o : null}${this.prefixText ? v : null}<div class=${s(n.wrapper)}>${f}${this.isClearable ? i : null}${this.requestedIcon ? d : null}${this.loading ? e : null}</div><div class=${s(n.actionWrapper)} ${I(this.actionWrapperEl)}><slot name=${at.action}></slot></div>${this.numberButtonType === "vertical" && !this.readOnly ? p : null}${this.suffixText ? h : null}${this.numberButtonType === "horizontal" && !this.readOnly ? c : null}${_({ component: this })}</div>${this.validationMessage && this.status === "invalid" ? rt({ icon: this.validationIcon, id: H.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
T("calcite-input-number", lt);
export {
  lt as InputNumber
};
