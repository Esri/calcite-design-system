...
class H extends C {
  ...
  prevMonthKeydown(e) {
    y(e.key) && this.prevMonthClick(e);
  }
  nextMonthClick(e) {
    this.handleArrowClick(e, this.nextMonthDate);
  }
  nextMonthKeydown(e) {
    y(e.key) && this.nextMonthClick(e);
  }
  ...
  getInRangeDate({ localizedYear: e, offset: t = 0 }) {
    ...
    if (d && D && l === e.length) {
      const g = new Date(c);
      return g.setFullYear(d), h(g, a, n);
    }
  }
  ...
  renderMonthPicker() {
    ...
    return o`<calcite-select class=${r(s.monthPicker)} .label=${this.messages.monthMenu} @calciteSelectChange=${this.handleMonthChange} width=auto ${v(this.monthPickerEl)}>${t.map((a, n) => o`<calcite-option .disabled=${!this.isMonthInRange(n)} .selected=${n === e} .value=${a}>${a}</calcite-option>`)}</calcite-select>`;
  }
  renderYearInput() {
    ...
    return o`<span class=${r(s.yearContainer)}><input .ariaLabel=${this.messages.year} class=${r({
      year: !0
    })} inputmode=numeric maxlength=4 minlength=1 @change=${this.onYearChange} @input=${this.onYearInput} @keydown=${this.onYearKey} pattern=\\d* type=text .value=${S(t ?? "")} ${v(this.yearInputEl)}>${e && o`<span class=${r(s.suffix)}>${e}</span>` || ""}</span>`;
  }
  renderChevron(e) {
    const t = e === "right", a = p(t ? this.nextMonthDate : this.prevMonthDate, this.activeDate) || !f(this.activeDate, this.min, this.max);
    return o`<calcite-action alignment=center .ariaDisabled=${a} .ariaLabel=${t ? this.messages.nextMonth : this.messages.prevMonth} class=${r(s.chevron)} compact data-direction=${e ?? Y} .disabled=${a} .icon=${t ? M.chevronRight : M.chevronLeft} icon-flip-rtl @click=${t ? this.nextMonthClick : this.prevMonthClick} @keydown=${t ? this.nextMonthKeydown : this.prevMonthKeydown} role=button .scale=${this.scale === "l" ? "l" : "m"} .text=${t ? this.messages.nextMonth : this.messages.prevMonth} ${v((n) => t ? this.nextMonthAction = n : this.prevMonthAction = n)}></calcite-action>`;
  }
}
