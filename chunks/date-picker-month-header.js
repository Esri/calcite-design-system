import { j as $, L as C, n as z, x as o, s, E as Y, k as S } from "./iframe.js";
import { l as w } from "./live.js";
import { e as P, f as I, g as E } from "./global.js";
import { e as u, n as g } from "./ref.js";
import { b as h, n as x, p as k, f as A, e as O, j as y, i as f, h as p, k as F } from "./date.js";
...
class H extends C {
  ...
  handleMonthChange(e) {
    const t = e.target, { abbreviated: a, wide: i } = this.localeData.months, n = (this.monthStyle === "wide" ? i : a).indexOf(t.value);
  ...
  getInRangeDate({ localizedYear: e, offset: t = 0 }) {
    const { min: a, max: i, activeDate: c } = this, n = Number(m.delocalize(e)), l = n.toString().length, d = isNaN(n) ? !1 : n + t, D = d && (!a || a.getFullYear() <= d) && (!i || i.getFullYear() >= d);
    if (d && D && l === e.length) {
      const v = new Date(c);
      return v.setFullYear(d), h(v, a, i);
    }
  }
  /**
  ...
  setYear({ localizedYear: e, commit: t = !0, offset: a = 0 }) {
    const { yearInputEl: { value: i }, activeDate: c } = this, n = this.getInRangeDate({ localizedYear: e, offset: a });
    n && this.calciteInternalDatePickerMonthHeaderSelectChange.emit(n), t && (i.value = this.formatCalendarYear((n || c).getFullYear()));
  }
  ...
  setYearSelectMenuWidth() {
    ...
    e && requestAnimationFrame(() => {
      const t = getComputedStyle(e), a = `${t.fontStyle} ${t.fontVariant} ${t.fontWeight} ${t.fontSize}/${t.lineHeight} ${t.fontFamily}`, c = this.localeData.months[this.monthStyle][this.activeDate.getMonth()], n = Math.ceil(R(c, a));
  ...
  async handlePenultimateValidMonth(e) {
    const i = e.target.getAttribute("data-direction") === "left";
    let c;
    if (i && this.min) {
      const n = h(k(this.activeDate), this.min, this.max);
      c = p(n, this.min);
    } else if (this.max) {
      const n = h(x(this.activeDate), this.min, this.max);
      c = p(n, this.max);
    }
    c && (this.position ? this.yearInputEl.value.focus() : await (i ? this.nextMonthAction.setFocus() : this.prevMonthAction.setFocus()));
  }
  getPx(e) {
  ...
  renderContent() {
    ...
    const a = F(e.unitOrder), i = a.indexOf("y") < a.indexOf("m");
    ...
    })}>${this.renderMonthYearContainer(i)}</div>${!this.position && o`<div class=${s({ [r.chevronContainer]: !0 })}>${this.renderChevron("left")}</div>` || ""}<div class=${s({ [r.chevronContainer]: !0 })}>${this.position !== "start" && this.renderChevron("right") || ""}</div>`;
  }
  ...
  renderMonthPicker() {
    ...
    return o`<calcite-select class=${s(r.monthPicker)} .label=${this.messages.monthMenu} @calciteSelectChange=${this.handleMonthChange} width=auto ${g(this.monthPickerEl)}>${t.map((a, i) => o`<calcite-option .disabled=${!this.isMonthInRange(i)} .selected=${i === e} .value=${a}>${a}</calcite-option>`)}</calcite-select>`;
  }
  renderYearInput() {
  ...
  renderChevron(e) {
    ...
    return o`<calcite-action alignment=center .ariaDisabled=${a} .ariaLabel=${t ? this.messages.nextMonth : this.messages.prevMonth} class=${s(r.chevron)} compact data-direction=${e ?? Y} .disabled=${a} .icon=${t ? M.chevronRight : M.chevronLeft} icon-flip-rtl @click=${t ? this.nextMonthClick : this.prevMonthClick} @keydown=${t ? this.nextMonthKeydown : this.prevMonthKeydown} role=button .scale=${this.scale === "l" ? "l" : "m"} .text=${t ? this.messages.nextMonth : this.messages.prevMonth} ${g((i) => t ? this.nextMonthAction = i : this.prevMonthAction = i)}></calcite-action>`;
  }
}
