...
class H extends C {
  constructor() {
    super(...arguments), this.monthPickerRef = f(), this.nextMonthActionRef = f(), this.prevMonthActionRef = f(), this.yearInputRef = f(), this.scale = "m", this.calciteInternalDatePickerMonthHeaderSelectChange = z({ cancelable: !1 });
  }
  static {
  ...
  load() {
    this.parentDatePickerEl = F(this.el, "calcite-date-picker") ?? void 0;
  }
  willUpdate(e) {
  ...
  setNextPrevMonthDates() {
    this.activeDate && (this.nextMonthDate = h(x(this.activeDate), this.min, this.max), this.prevMonthDate = h(y(this.activeDate), this.min, this.max));
  }
  onYearKey(e) {
  ...
  prevMonthClick(e) {
    this.prevMonthDate && this.handleArrowClick(e, this.prevMonthDate);
  }
  prevMonthKeydown(e) {
    k(e.key) && this.prevMonthClick(e);
  }
  nextMonthClick(e) {
    this.nextMonthDate && this.handleArrowClick(e, this.nextMonthDate);
  }
  nextMonthKeydown(e) {
  ...
  handleMonthChange(e) {
    ...
    let r = b(this.activeDate, i);
    g(r, this.min, this.max) || (r = h(r, this.min, this.max)), this.calciteInternalDatePickerMonthHeaderSelectChange.emit(r), this.setYearSelectMenuWidth();
  }
  getInRangeDate({ localizedYear: e, offset: t = 0 }) {
    const { min: a, max: n, activeDate: c } = this, i = Number(m.delocalize(e)), r = i.toString().length, d = isNaN(i) ? !1 : i + t, D = d && (!a || a.getFullYear() <= d) && (!n || n.getFullYear() >= d);
    if (d && D && r === e.length) {
      const u = new Date(c);
  ...
  setYear({ localizedYear: e, commit: t = !0, offset: a = 0 }) {
    ...
    i && this.calciteInternalDatePickerMonthHeaderSelectChange.emit(i), t && n.value && (n.value.value = this.formatCalendarYear((i || c).getFullYear()));
  }
  setYearSelectWidthOffset() {
    this.yearSelectWidthOffset = E + 3 * parseInt(this.getYearSelectPadding(), 10), this.setYearSelectMenuWidth();
  }
  ...
  isMonthInRange(e) {
    const t = b(this.activeDate, e);
    return !this.min && !this.max || g(t, this.min, this.max) ? !0 : !!(this.max && p(t, this.max) || this.min && p(t, this.min));
  }
  async handlePenultimateValidMonth(e) {
    const n = e.target.getAttribute("data-direction") === "left";
    let c = !1;
    if (n && this.min) {
      const i = h(y(this.activeDate), this.min, this.max);
      c = i ? p(i, this.min) : !1;
    } else if (this.max) {
    ...
    if (c)
      if (this.position)
        ...
        const r = (n ? this.nextMonthActionRef : this.prevMonthActionRef).value;
        if (!r)
          return;
        r.disabled = !1, await r.setFocus();
      }
  }
  getPx(e) {
  ...
  render() {
    return l`<div class=${o(s.header)}>${this.renderContent()}</div>`;
  }
  renderContent() {
    ...
    return l`${this.position && l`<div class=${o({ [s.chevronContainer]: !0 })}>${this.position === "start" && this.renderChevron("left") || ""}</div>` || ""}<div class=${o({
      [s.monthYearContainer]: !0,
      [s.rangeCalendar]: !!this.position
    })}>${this.renderMonthYearContainer(n)}</div>${!this.position && l`<div class=${o({ [s.chevronContainer]: !0 })}>${this.renderChevron("left")}</div>` || ""}<div class=${o({ [s.chevronContainer]: !0 })}>${this.position !== "start" && this.renderChevron("right") || ""}</div>`;
  }
  ...
  renderMonthPicker() {
    ...
    return l`<calcite-select class=${o(s.monthPicker)} .label=${this.messages.monthMenu} @calciteSelectChange=${this.handleMonthChange} width=auto ${v(this.monthPickerRef)}>${t.map((a, n) => l`<calcite-option .disabled=${!this.isMonthInRange(n)} .selected=${n === e} .value=${a}>${a}</calcite-option>`)}</calcite-select>`;
  }
  renderYearInput() {
    const e = this.localeData.year?.suffix, t = this.formatCalendarYear(this.activeDate.getFullYear());
    return l`<span class=${o(s.yearContainer)}><input .ariaLabel=${this.messages.year} class=${o({ [s.year]: !0 })} inputmode=numeric maxlength=4 minlength=1 @change=${this.onYearChange} @input=${this.onYearInput} @keydown=${this.onYearKey} pattern=\\d* type=text .value=${S(t ?? "")} ${v(this.yearInputRef)}>${e && l`<span class=${o(s.suffix)}>${e}</span>` || ""}</span>`;
  }
  renderChevron(e) {
    const t = e === "right", a = p(t ? this.nextMonthDate : this.prevMonthDate, this.activeDate) || !g(this.activeDate, this.min, this.max);
    return l`<calcite-action alignment=center .ariaDisabled=${a} .ariaLabel=${t ? this.messages.nextMonth : this.messages.prevMonth} class=${o(s.chevron)} compact data-direction=${e ?? Y} .disabled=${a} .icon=${t ? M.chevronRight : M.chevronLeft} icon-flip-rtl @click=${t ? this.nextMonthClick : this.prevMonthClick} @keydown=${t ? this.nextMonthKeydown : this.prevMonthKeydown} role=button .scale=${this.scale} .text=${t ? this.messages.nextMonth : this.messages.prevMonth} ${v(t ? this.nextMonthActionRef : this.prevMonthActionRef)}></calcite-action>`;
  }
}
