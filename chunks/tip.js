import { a as c, L as d, d as h, s as a, x as n, c as m } from "./iframe.js";
...
class y extends d {
  ...
  renderHeader() {
    ...
    return i ? n`<header class=${a(e.header)}>${p({ class: e.heading, level: r, children: i })}</header>` : null;
  }
  renderDismissButton() {
    const { closeDisabled: i, hideTip: t } = this;
    return i ? null : n`<calcite-action class=${a(e.close)} .icon=${x.close} @click=${t} scale=l .text=${this.messages.close}></calcite-action>`;
  }
  renderImageFrame() {
    return g("thumbnail", n`<div class=${a(e.imageFrame)} .hidden=${!this.hasThumbnail}><slot name=${$.thumbnail} @slotchange=${this.handleThumbnailSlotChange}></slot></div>`);
  }
  renderInfoNode() {
    return n`<div class=${a({ [e.info]: !0, [e.infoNoThumbnail]: !this.hasThumbnail })}><slot></slot></div>`;
  }
  renderContent() {
    return n`<div class=${a(e.content)}>${this.renderImageFrame()}${this.renderInfoNode()}</div>`;
  }
  render() {
    return n`<article class=${a(e.container)}>${this.renderHeader()}${this.renderContent()}</article>${this.renderDismissButton()}`;
  }
}
m("calcite-tip", y);
export {
