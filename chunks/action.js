import { j as y, L as C, x as i, s as n, z as r, k as z } from "./iframe.js";
...
class B extends C {
  ...
  // #region Rendering
  renderTextContainer() {
    const { text: t, textEnabled: a } = this, e = {
      [o.textContainer]: !0,
      [o.textContainerVisible]: a
    };
    return t ? g("text-container", i`<div class=${n(e)}>${t}</div>`) : null;
  }
  renderIndicatorText() {
    const { indicator: t, messages: a, indicatorId: e, buttonId: c } = this;
    return i`<div aria-labelledby=${c ?? r} aria-live=polite class=${n(o.indicatorText)} id=${e ?? r} role=region>${t ? a.indicator : null}</div>`;
  }
  renderIconContainer() {
    const { loading: t, icon: a, scale: e, el: c, iconFlipRtl: d, indicator: b } = this, p = e === "l" ? "l" : "m", h = t ? i`<calcite-loader inline .label=${this.messages.loading} .scale=${p}></calcite-loader>` : null, l = a ? i`<calcite-icon class=${n({ [o.indicatorWithIcon]: b })} .flipRtl=${d} .icon=${a} .scale=${S(this.scale)}></calcite-icon>` : null, s = h || l, u = s || c.children?.length, v = i`<div class=${n({
      [o.slotContainer]: !0,
      [o.slotContainerHidden]: t
    })}><slot></slot></div>`;
    return u ? g("icon-container", i`<div aria-hidden=true class=${n(o.iconContainer)}>${s}${v}</div>`) : null;
  }
  renderButton() {
    const { active: t, compact: a, disabled: e, icon: c, loading: d, textEnabled: b, label: p, text: h, indicator: l, indicatorId: s, buttonId: u, messages: v } = this, m = p || h || "", f = l ? v.indicatorLabel.replace("{label}", m) : m, x = {
      [o.button]: !0,
      [o.buttonTextVisible]: b,
      [o.buttonCompact]: a
    }, k = i`${this.renderIconContainer()}${this.renderTextContainer()}${!c && l && g("indicator-no-icon", i`<div class=${n(o.indicatorWithoutIcon)}></div>`) || ""}`;
    return this.dragHandle ? i`<span aria-controls=${(l ? s : null) ?? r} .ariaBusy=${d} .ariaDisabled=${this.disabled ? this.disabled : null} .ariaLabel=${f} .ariaPressed=${t} class=${n(x)} id=${u ?? r} role=button tabindex=${(this.disabled ? null : 0) ?? r} ${$(this.buttonEl)}>${k}</span>` : i`<button aria-controls=${(l ? s : null) ?? r} .ariaBusy=${d} .ariaLabel=${f} .ariaPressed=${t} class=${n(x)} .disabled=${e} id=${u ?? r} ${$(this.buttonEl)}>${k}</button>`;
  }
  render() {
