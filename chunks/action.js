...
class O extends z {
  ...
  renderIndicatorText() {
    ...
    return e`<div aria-labelledby=${n ?? r} aria-live=polite class=${a($.screenReaderText)} role=region ${p(this.indicatorRef)}>${t ? o.indicator : null}</div>`;
  }
  renderIconContainer() {
    const { loading: t, icon: o, scale: n, el: l, iconFlipRtl: v, indicator: s } = this, d = n === "l" ? "l" : "m", u = t ? e`<calcite-loader inline .label=${this.messages.loading} .scale=${d}></calcite-loader>` : null, h = o ? e`<calcite-icon class=${a({ [i.indicatorWithIcon]: s })} .flipRtl=${v} .icon=${o} .scale=${I(this.scale)}></calcite-icon>` : null, c = u || h, b = c || l.children?.length, g = e`<div class=${a({
      [i.slotContainer]: !0,
  ...
  renderLabel() {
    ...
    return t ? e`<span class=${a($.screenReaderText)} ${p(this.labelElRef)}>${t}</span>` : null;
  }
  renderButton() {
    ...
    return this.dragHandle ? e`<span .ariaBusy=${k(l, void 0)} .ariaControlsElements=${x} .ariaDescribedByElements=${this.aria?.describedByElements} .ariaExpanded=${this.aria?.expanded} .ariaHasPopup=${this.aria?.hasPopup} .ariaLabelledByElements=${h} .ariaOwnsElements=${this.aria?.ownsElements} .ariaPressed=${this.aria?.pressed} class=${a(c)} id=${u ?? r} role=button tabindex=${(this.disabled ? void 0 : 0) ?? r} ${p(this.buttonRef)}>${b}</span>` : e`<button .ariaBusy=${k(l, void 0)} .ariaChecked=${this.aria?.checked} .ariaControlsElements=${x} .ariaDescribedByElements=${this.aria?.describedByElements} .ariaExpanded=${this.aria?.expanded} .ariaHasPopup=${this.aria?.hasPopup} .ariaLabelledByElements=${h} .ariaOwnsElements=${this.aria?.ownsElements} .ariaPressed=${this.aria?.pressed} class=${a(c)} .disabled=${o} id=${u ?? r} .role=${this.aria?.role} type=${this.type ?? r} ${p(this.buttonRef)}>${b}</button>`;
  }
  render() {
