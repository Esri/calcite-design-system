import { b as E, L as z, s as n, x as e, z as m, q as I } from "./index.js";
...
class q extends z {
  ...
  static {
    this.properties = { aria: 0, active: 7, activeDescendant: 7, alignment: 3, appearance: 3, compact: 7, disabled: 7, dragHandle: 7, form: 3, icon: [3, { type: String }], iconFlipRtl: 7, indicator: 7, label: 1, loading: 7, messageOverrides: 0, scale: 3, width: 3, text: 1, textEnabled: 7, type: 3 };
  }
  static {
    this.styles = W;
  }
  ...
  renderButton() {
    const { compact: t, disabled: a, icon: o, loading: c, textEnabled: b, label: h, text: p, indicator: r, indicatorEl: d, buttonId: l, messages: v } = this, u = h || p || "", x = r ? v.indicatorLabel.replace("{label}", u) : u, $ = {
      ...
      [i.buttonCompact]: t
    }, k = e`${this.renderIconContainer()}${this.renderTextContainer()}${!o && r && g("indicator-no-icon", e`<div class=${n(i.indicatorWithoutIcon)}></div>`) || ""}`, C = r && d ? [d] : [], y = [
      ...this.aria?.controlsElements ?? [],
      ...C
    ];
    return this.dragHandle ? e`<span .ariaBusy=${c} .ariaControlsElements=${y} .ariaDescribedByElements=${this.aria?.describedByElements} .ariaExpanded=${this.aria?.expanded} .ariaHasPopup=${this.aria?.hasPopup} .ariaLabel=${x} .ariaLabelledByElements=${this.aria?.labelledByElements} .ariaOwnsElements=${this.aria?.ownsElements} .ariaPressed=${this.aria?.pressed} class=${n($)} id=${l ?? m} role=button tabindex=${this.disabled ? null : 0} ${f(this.buttonRef)}>${k}</span>` : e`<button .ariaBusy=${c} .ariaControlsElements=${y} .ariaDescribedByElements=${this.aria?.describedByElements} .ariaExpanded=${this.aria?.expanded} .ariaHasPopup=${this.aria?.hasPopup} .ariaLabel=${x} .ariaLabelledByElements=${this.aria?.labelledByElements} .ariaOwnsElements=${this.aria?.ownsElements} .ariaPressed=${this.aria?.pressed} class=${n($)} .disabled=${a} id=${l ?? m} @click=${this.handleClick} ${f(this.buttonRef)}>${k}</button>`;
  }
  render() {
