import { b as C, L as z, s as n, x as e, z as m, q as I } from "./index.js";
import { i as g } from "./keyed.js";
import { e as T, n as f } from "./ref.js";
import { g as B } from "./guid.js";
import { u as S, I as L } from "./interactive.js";
...
import { f as P, s as j, r as N } from "./form.js";
...
class q extends z {
  ...
  handleClick() {
    const { type: t } = this;
    t === "submit" ? j(this) : t === "reset" && N(this);
  }
  ...
  renderIndicatorText() {
    ...
    return e`<div aria-labelledby=${o ?? m} aria-live=polite class=${n(i.indicatorText)} role=region ${f(this.storeIndicatorEl)}>${t ? a.indicator : null}</div>`;
  }
  renderIconContainer() {
    const { loading: t, icon: a, scale: o, el: c, iconFlipRtl: b, indicator: h } = this, p = o === "l" ? "l" : "m", r = t ? e`<calcite-loader inline .label=${this.messages.loading} .scale=${p}></calcite-loader>` : null, d = a ? e`<calcite-icon class=${n({ [i.indicatorWithIcon]: h })} .flipRtl=${b} .icon=${a} .scale=${H(this.scale)}></calcite-icon>` : null, l = r || d, v = l || c.children?.length, u = e`<div class=${n({
      [i.slotContainer]: !0,
      [i.slotContainerHidden]: t
    })}><slot></slot></div>`;
    return v ? g("icon-container", e`<div aria-hidden=true class=${n(i.iconContainer)}>${l}${u}</div>`) : null;
  }
  renderButton() {
    const { compact: t, disabled: a, icon: o, loading: c, textEnabled: b, label: h, text: p, indicator: r, indicatorEl: d, buttonId: l, messages: v } = this, u = h || p || "", x = r ? v.indicatorLabel.replace("{label}", u) : u, $ = {
      ...
      [i.buttonCompact]: t
    }, k = e`${this.renderIconContainer()}${this.renderTextContainer()}${!o && r && g("indicator-no-icon", e`<div class=${n(i.indicatorWithoutIcon)}></div>`) || ""}`, w = r && d ? [d] : [], y = [
      ...this.aria?.controlsElements ?? [],
      ...w
    ];
    return this.dragHandle ? e`<span .ariaBusy=${c} .ariaControlsElements=${y} .ariaDescribedByElements=${this.aria?.describedByElements} .ariaExpanded=${this.aria?.expanded} .ariaHasPopup=${this.aria?.hasPopup} .ariaLabel=${x} .ariaLabelledByElements=${this.aria?.labelledByElements} .ariaOwnsElements=${this.aria?.ownsElements} .ariaPressed=${this.aria?.pressed} class=${n($)} id=${l ?? m} role=button tabindex=${this.disabled ? null : 0} ${f(this.buttonEl)}>${k}</span>` : e`<button .ariaBusy=${c} .ariaControlsElements=${y} .ariaDescribedByElements=${this.aria?.describedByElements} .ariaExpanded=${this.aria?.expanded} .ariaHasPopup=${this.aria?.hasPopup} .ariaLabel=${x} .ariaLabelledByElements=${this.aria?.labelledByElements} .ariaOwnsElements=${this.aria?.ownsElements} .ariaPressed=${this.aria?.pressed} class=${n($)} .disabled=${a} id=${l ?? m} @click=${this.handleClick} ${f(this.buttonEl)}>${k}</button>`;
  }
  render() {
