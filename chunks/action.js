import { a as y, L as w, s as n, x as i, o as r, c as C } from "./iframe.js";
import { i as g } from "./keyed.js";
import { e as z, n as $ } from "./ref.js";
...
class B extends w {
  ...
  renderTextContainer() {
    ...
    return t ? g("text-container", i`<div class=${n(o)}>${t}</div>`) : null;
  }
  renderIndicatorText() {
    const { indicator: t, messages: a, indicatorId: o, buttonId: c } = this;
    return i`<div aria-labelledby=${c ?? r} aria-live=polite class=${n(e.indicatorText)} id=${o ?? r} role=region>${t ? a.indicator : null}</div>`;
  }
  renderIconContainer() {
    const { loading: t, icon: a, scale: o, el: c, iconFlipRtl: d, indicator: b } = this, h = o === "l" ? "l" : "m", v = t ? i`<calcite-loader inline .label=${this.messages.loading} .scale=${h}></calcite-loader>` : null, l = a ? i`<calcite-icon class=${n({ [e.indicatorWithIcon]: b })} .flipRtl=${d} .icon=${a} .scale=${S(this.scale)}></calcite-icon>` : null, s = v || l, u = s || c.children?.length, p = i`<div class=${n({
      [e.slotContainer]: !0,
      [e.slotContainerHidden]: t
    })}><slot></slot></div>`;
    return u ? g("icon-container", i`<div aria-hidden=true class=${n(e.iconContainer)}>${s}${p}</div>`) : null;
  }
  renderButton() {
    const { active: t, compact: a, disabled: o, icon: c, loading: d, textEnabled: b, label: h, text: v, indicator: l, indicatorId: s, buttonId: u, messages: p } = this, m = h || v || "", f = l ? p.indicatorLabel.replace("{label}", m) : m, x = {
      [e.button]: !0,
      [e.buttonTextVisible]: b,
      [e.buttonCompact]: a
    }, k = i`${this.renderIconContainer()}${this.renderTextContainer()}${!c && l && g("indicator-no-icon", i`<div class=${n(e.indicatorWithoutIcon)}></div>`) || ""}`;
    return this.dragHandle ? i`<span aria-controls=${(l ? s : null) ?? r} .ariaBusy=${d} .ariaDisabled=${this.disabled ? this.disabled : null} .ariaLabel=${f} .ariaPressed=${t} class=${n(x)} id=${u ?? r} role=button tabindex=${(this.disabled ? null : 0) ?? r} ${$(this.buttonEl)}>${k}</span>` : i`<button aria-controls=${(l ? s : null) ?? r} .ariaBusy=${d} .ariaLabel=${f} .ariaPressed=${t} class=${n(x)} .disabled=${o} id=${u ?? r} ${$(this.buttonEl)}>${k}</button>`;
  }
  render() {
    return T({ disabled: this.disabled, children: i`${this.renderButton()}<slot name=${O.tooltip} @slotchange=${this.handleTooltipSlotChange}></slot>${this.renderIndicatorText()}` });
  }
}
C("calcite-action", B);
export {
