import { b as w, L as v, c as i, s as m, E as l, x as r, q as T } from "./index.js";
...
class E extends v {
  ...
  getLabel() {
    ...
    if (!d)
      return e ?? "";
    let a = e ? t.repositionLabel.replace(h.label, e) : t.reposition;
    return a = a.replace(h.position, o ? o.toString() : ""), a.replace(h.total, s ? s.toString() : "");
  }
  handleBeforeOpen(e) {
  ...
  render() {
    const { disabled: e, flipPlacements: t, open: o, overlayPositioning: s, placement: d, scale: a, widthScale: $, hasNoItems: b } = this, c = this.getLabel(), S = e || b;
    return H({ disabled: e, children: r`<calcite-dropdown class=${m(u.dropdown)} .disabled=${S} .flipPlacements=${t} @calciteDropdownBeforeClose=${this.handleBeforeClose} @calciteDropdownBeforeOpen=${this.handleBeforeOpen} @calciteDropdownClose=${this.handleClose} @calciteDropdownOpen=${this.handleOpen} .overlayPositioning=${s} .placement=${d} .scale=${a} .widthScale=${$} ${I(this.setDropdownEl)}><calcite-action .active=${o} appearance=transparent .aria=${{ expanded: o }} class=${m(u.handle)} .dragHandle=${!0} .icon=${e ? g.blank : g.drag} .label=${c} .scale=${a} slot=${B.trigger} .text=${c} title=${c ?? l}></calcite-action>${this.renderReorderGroup()}${this.renderMoveToGroup()}${this.renderAddToGroup()}</calcite-dropdown>` });
  }
  renderAddToItem(e) {
    return n(e.id, r`<calcite-dropdown-item data-id=${e.id ?? l} .label=${e.label} @calciteDropdownItemSelect=${this.handleAddTo}>${e.label}</calcite-dropdown-item>`);
  }
  renderMoveToItem(e) {
    return n(e.id, r`<calcite-dropdown-item data-id=${e.id ?? l} .label=${e.label} @calciteDropdownItemSelect=${this.handleMoveTo}>${e.label}</calcite-dropdown-item>`);
  }
  renderReorderGroup() {
    return this.hasReorderItems ? n("reorder", r`<calcite-dropdown-group .groupTitle=${this.messages.reorder} id=${p.reorder} .scale=${this.scale} selection-mode=none>${this.renderTop()}${this.renderUp()}${this.renderDown()}${this.renderBottom()}</calcite-dropdown-group>`) : null;
  }
  renderAddToGroup() {
    const { messages: e, addToItems: t, scale: o } = this;
    return t.length ? n("add-to-items", r`<calcite-dropdown-group .groupTitle=${e.addTo} id=${p.add} .scale=${o} selection-mode=none>${t.map((s) => this.renderAddToItem(s))}</calcite-dropdown-group>`) : null;
  }
  renderMoveToGroup() {
    const { messages: e, moveToItems: t, scale: o } = this;
    return t.length ? n("move-to-items", r`<calcite-dropdown-group .groupTitle=${e.moveTo} id=${p.move} .scale=${o} selection-mode=none>${t.map((s) => this.renderMoveToItem(s))}</calcite-dropdown-group>`) : null;
  }
  renderDropdownItem(e, t) {
    return n(f[e], r`<calcite-dropdown-item data-value=${f[e] ?? l} .label=${t} @calciteDropdownItemSelect=${this.handleReorder}>${t}</calcite-dropdown-item>`);
  }
  renderTop() {
