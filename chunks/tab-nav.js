import { b as I, L as m, c as T, s as h, C, x as f, q as x } from "./index.js";
import { n as S } from "./ref.js";
import { i as y } from "./keyed.js";
import { a as k, b as O, d as B } from "./core.js";
import { a as E, F as u, b as F } from "./dom.js";
...
class z extends m {
  ...
  willUpdate(t) {
    ...
    this.layout = e?.layout, this.bordered = e?.bordered, this.effectiveDir = E(this.el);
  }
  loaded() {
  ...
  get enabledTabTitles() {
    return u(this.el, "calcite-tab-title:not([disabled])").filter((t) => !t.closed);
  }
  get scrollerButtonWidth() {
    const { scale: t } = this;
    return parseInt(t === "s" ? k : t === "m" ? O : B);
  }
  get tabTitles() {
    return u(this.el, "calcite-tab-title");
  }
  ...
  storeTabTitleWrapperRef(t) {
    t && (this.tabTitleContainerEl = t, this.intersectionObserver = g("intersection", () => this.updateScrollingState(), {
      root: t,
      threshold: [0, 0.5, 1]
    }));
  }
  ...
  handleTabFocus(t, e, i) {
    const a = F(this.enabledTabTitles, e, i);
  ...
  render() {
    return this.el.role = "tablist", f`<div class=${h({
      [o.container]: !0,
      ...
      [o.position(this.position)]: !0,
      [C.rtl]: this.effectiveDir === "rtl"
    })}>${this.renderScrollButton("start")}<div class=${h({
      [o.tabTitleSlotWrapper]: !0
    })} @scroll=${this.onTabTitleScroll} @wheel=${this.onTabTitleWheel} ${S(this.storeTabTitleWrapperRef)}><slot @slotchange=${this.onSlotChange}></slot></div>${this.renderScrollButton("end")}</div>`;
  }
  renderScrollButton(t) {
    ...
    return y(t, f`<div class=${h({
      [o.scrollButtonContainer]: !0,
      [o.scrollBackwardContainerButton]: !s,
      [o.scrollForwardContainerButton]: s
    })} .hidden=${s && !n || !s && !a}><calcite-button .appearance=${e ? "outline-fill" : "transparent"} .ariaLabel=${s ? i.nextTabTitles : i.previousTabTitles} class=${h({
      [o.scrollButton]: !0
    })} icon-flip-rtl=both .iconStart=${s ? v.chevronRight : v.chevronLeft} kind=neutral @click=${s ? this.scrollToNextTabTitles : this.scrollToPreviousTabTitles} .scale=${l} tabindex=-1></calcite-button></div>`);
  }
}
x("calcite-tab-nav", z);
export {
