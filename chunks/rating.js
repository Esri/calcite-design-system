import { x as s, s as g, j as b, L as f, n as y, o as $, z as r, q as k, k as w } from "./iframe.js";
...
class q extends f {
  ...
  willUpdate() {
    this.starsMap = Array.from({ length: this.max }, (e, t) => {
      const a = t + 1, i = !this.hoverValue && this.average && !this.value && a <= this.average, d = a === this.value, n = this.average && this.average + 1 - a, l = a <= this.hoverValue, c = `${this.guid}-${a}`, o = !this.hoverValue && !this.value && !l && n > 0 && n < 1, h = this.value >= a, m = this.getTabIndex(a);
      return {
        average: i,
        checked: d,
        fraction: n,
  ...
  render() {
    ...
    return M({ disabled: this.disabled, children: s`<span class="wrapper"><fieldset class="fieldset" .disabled=${this.disabled}><legend class="visually-hidden">${this.messages.rating}</legend>${this.starsMap.map(({ average: t, checked: a, fraction: i, hovered: d, id: n, partial: l, selected: c, value: o, tabIndex: h }) => s`<label class=${g({
      star: !0,
      selected: c,
      hovered: d,
      average: t,
      partial: l
    })} data-value=${o ?? r} for=${n ?? r} @click=${this.handleLabelClick} @focus=${this.handleLabelFocus} @keydown=${this.handleLabelKeyDown} @pointerdown=${this.handleLabelPointerDown} @pointerover=${this.handleLabelPointerOver} tabindex=${h ?? r}><input aria-errormessage=${v.validationMessage} .ariaInvalid=${this.status === "invalid"} .checked=${a} class="visually-hidden" .disabled=${this.disabled || this.readOnly} id=${n ?? r} name=${this.guid ?? r} @change=${this.handleInputChange} tabindex=-1 type=radio .value=${x(o ?? "")}>${p({ full: c || t, scale: this.scale })}${l && s`<div class="fraction" style=${k({ width: `${i * 100}%` })}>${p({ full: !0, partial: !0, scale: this.scale })}</div>` || ""}<span class="visually-hidden">${this.messages.stars.replace("{num}", `${o}`)}</span></label>`)}${(this.count || this.average) && this.showChip ? s`<calcite-chip .label=${e} .scale=${this.scale} .value=${e}>${!!this.average && s`<span class="number--average">${this.average.toString()}</span>` || ""}${!!this.count && s`<span class="number--count">(${e})</span>` || ""}</calcite-chip>` : null}</fieldset>${I({ component: this })}${this.validationMessage && this.status === "invalid" ? D({ icon: this.validationIcon, id: v.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}</span>` });
  }
}
w("calcite-rating", q);
export {
