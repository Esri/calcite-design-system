import { s as g, x as s, h as b, L as f, k as y, n as $, y as r, o as k, j as w } from "./iframe.js";
...
class S extends f {
  ...
  render() {
    ...
    return M({ disabled: this.disabled, children: s`<span class="wrapper"><fieldset class="fieldset" .disabled=${this.disabled}><legend class="visually-hidden">${this.messages.rating}</legend>${this.starsMap.map(({ average: t, checked: a, fraction: i, hovered: c, id: n, partial: l, selected: h, value: o, tabIndex: d }) => s`<label class=${g({
      star: !0,
      selected: h,
      hovered: c,
      average: t,
      partial: l
    })} data-value=${o ?? r} for=${n ?? r} @click=${this.handleLabelClick} @focus=${this.handleLabelFocus} @keydown=${this.handleLabelKeyDown} @pointerdown=${this.handleLabelPointerDown} @pointerover=${this.handleLabelPointerOver} tabindex=${d ?? r}><input aria-errormessage=${v.validationMessage} .ariaInvalid=${this.status === "invalid"} .checked=${a} class="visually-hidden" .disabled=${this.disabled || this.readOnly} id=${n ?? r} name=${this.guid ?? r} @change=${this.handleInputChange} tabindex=-1 type=radio .value=${x(o ?? "")}>${p({ full: h || t || c, scale: this.scale })}${l && s`<div class="fraction" style=${k({ width: `${i * 100}%` })}>${p({ full: !0, partial: !0, scale: this.scale })}</div>` || ""}<span class="visually-hidden">${this.messages.stars.replace("{num}", `${o}`)}</span></label>`)}${(this.count || this.average) && this.showChip ? s`<calcite-chip .label=${e} .scale=${this.scale} .value=${e}>${!!this.average && s`<span class="number--average">${this.average.toString()}</span>` || ""}${!!this.count && s`<span class="number--count">(${e})</span>` || ""}</calcite-chip>` : null}</fieldset>${I({ component: this })}${this.validationMessage && this.status === "invalid" ? D({ icon: this.validationIcon, id: v.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}</span>` });
  }
}
w("calcite-rating", S);
export {
