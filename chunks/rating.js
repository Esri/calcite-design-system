...
class S extends f {
  ...
  render() {
    ...
    return M({ disabled: this.disabled, children: s`<span class="wrapper"><fieldset class="fieldset" .disabled=${this.disabled}><legend class="visually-hidden">${this.messages.rating}</legend>${this.starsMap.map(({ average: t, checked: a, fraction: i, hovered: c, id: n, partial: l, selected: d, value: o, tabIndex: h }) => s`<label class=${g({
      ...
      average: t,
      partial: l
    })} data-value=${o ?? r} for=${n ?? r} @click=${this.handleLabelClick} @focus=${this.handleLabelFocus} @keydown=${this.handleLabelKeyDown} @pointerdown=${this.handleLabelPointerDown} @pointerover=${this.handleLabelPointerOver} tabindex=${h ?? r}><input aria-errormessage=${p.validationMessage} .ariaInvalid=${this.status === "invalid"} .checked=${a} class="visually-hidden" .disabled=${this.disabled || this.readOnly} id=${n ?? r} name=${this.guid ?? r} @change=${this.handleInputChange} tabindex=-1 type=radio .value=${x(o ?? "")}>${v({ full: d || t || c, scale: this.scale })}${l && s`<div class="fraction" style=${k({ width: `${i * 100}%` })}>${v({ full: !0, partial: !0, scale: this.scale })}</div>` || ""}<span class="visually-hidden">${this.messages.stars.replace("{num}", `${o}`)}</span></label>`)}${(this.count || this.average) && this.showChip ? s`<calcite-chip .label=${e} .scale=${this.scale} .value=${e}>${!!this.average && s`<span class="number--average">${this.average.toString()}</span>` || ""}${!!this.count && s`<span class="number--count">(${e})</span>` || ""}</calcite-chip>` : null}</fieldset>${I({ component: this })}${this.validationMessage && this.status === "invalid" ? D({ icon: this.validationIcon, id: p.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}</span>` });
  }
}
w("calcite-rating", S);
export {
