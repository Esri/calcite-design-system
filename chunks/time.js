...
v3.3.0-next.19 */
const N = ["12", "24"], U = 5;
function S({ locale: e, numberingSystem: t, includeSeconds: r = !0, fractionalSecondDigits: i, hour12: n }) {
  const o = {
    hour: "2-digit",
    minute: "2-digit",
    timeZone: "UTC",
...
function w({ locale: e, meridiem: t, parts: r }) {
  const i = ["he", "bs", "mk"];
  let n;
  if (r) {
    ...
    if (i.includes(e)) {
      ...
      const o = S({ locale: e, hour12: !0 }), s = 6, c = 18, l = new Date(Date.UTC(0, 0, 0, t === "AM" ? s : c, 0));
...
function k({ value: e, part: t, locale: r, numberingSystem: i = "latn", hour12: n }) {
  if (!O(e, t))
    return;
  ...
  const o = parseInt(e), s = new Date(Date.UTC(0, 0, 0, t === "hour" ? o : t === "meridiem" ? e === "AM" ? 0 : 12 : 0, t === "minute" ? o : 0, t === "second" ? o : 0));
  if (!s)
    return;
  const l = S({ hour12: n, locale: r, numberingSystem: i }).formatToParts(s);
  return f(t, l, r);
}
function W({ hour12: e, includeSeconds: t = !0, locale: r, numberingSystem: i = "latn", parts: n = !1, step: o, value: s }) {
  if (!T(s))
    return null;
  const { hour: c, minute: l, second: m = "0", fractionalSecond: d } = z(s, o), u = new Date(Date.UTC(0, 0, 0, parseInt(c), parseInt(l), parseInt(m), d && A(d))), M = S({
...
function Z({ hour12: e, locale: t, numberingSystem: r = "latn", step: i = 60, value: n }) {
  if (!T(n))
    return null;
  const { hour: o, minute: s, second: c = "0", fractionalSecond: l } = z(n, i), m = new Date(Date.UTC(0, 0, 0, parseInt(o), parseInt(s), parseInt(c), l && A(l)));
  if (m) {
    const u = S({
