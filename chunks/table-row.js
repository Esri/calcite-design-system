import { b as w, L as p, c as d, x as s, s as m, I as f, q as C } from "./index.js";
import { i as a } from "./keyed.js";
import { e as R, n as b } from "./ref.js";
...
const k = {
  ...
  circleF: "circle-f",
  circle: "circle"
}, S = w`@charset "UTF-8";:host{--calcite-internal-color-focus: var( --calcite-color-focus, var(--calcite-ui-focus-color, var(--calcite-color-brand)) )}:host([disabled]){cursor:default;-webkit-user-select:none;user-select:none;opacity:var(--calcite-opacity-disabled)}:host([disabled]) *,:host([disabled]) ::slotted(*){pointer-events:none}:host{display:contents}:host([hidden]){display:none}[hidden]{display:none}:host([disabled]) tr{pointer-events:none;opacity:var(--calcite-opacity-disabled)}:host([disabled]) ::slotted([calcite-hydrated][disabled]),:host([disabled]) [calcite-hydrated][disabled]{opacity:1}.interaction-container{display:contents}calcite-table-cell{--calcite-internal-table-cell-background-color: var( --calcite-table-row-background-color, var(--calcite-color-foreground-1) )}:host([selected]) calcite-table-cell{--calcite-internal-table-cell-background-color: var( --calcite-table-row-background-color-selected, var(--calcite-color-foreground-1) )}tr{border-block-end:var(--calcite-border-width-sm) solid var(--calcite-table-row-border-color, transparent);background-color:var(--calcite-table-row-background-color, var(--calcite-color-foreground-1))}tr.last-visible-row{border-block-end:0}@-moz-document url-prefix(){tr{box-shadow:inset 0 -1px 0 0 var(--calcite-table-row-border-color)}tr.last-visible-row{box-shadow:inset 0 -1px 0 0 transparent}}:host([item-hidden]){display:none}`;
class A extends p {
  constructor() {
    super(), this.rowCells = [], this.tableRowSlotEl = R(), this.userTriggered = !1, this._selected = !1, this.clickHandler = () => {
  ...
  renderSelectionIcon() {
    ...
    return s`<calcite-icon .icon=${e} .scale=${$(this.scale)}></calcite-icon>`;
  }
  ...
  render() {
    return T({ disabled: this.disabled, children: s`<tr .ariaRowIndex=${this.positionAll + 1} .ariaSelected=${this.selected} class=${m({ [k.lastVisibleRow]: this.lastVisibleRow })} @keydown=${this.keyDownHandler} ${b((e) => {
      e && (this.tableRowEl = e, f(s`${this.numbered && this.renderNumberedCell() || ""}${this.selectionMode !== "none" && this.renderSelectableCell() || ""}<slot ${b(this.tableRowSlotEl)}></slot>`, e));
    })}></tr>` });
  }
}
C("calcite-table-row", A);
export {
  A as TableRow
};
