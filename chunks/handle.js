...
class C extends d {
  constructor() {
    super(...arguments), this.handleButton = f(), this.messages = y({ blocking: !0 }), this.focusSetter = S()(this), this.blurUnselectDisabled = !1, this.disabled = !1, this.selected = !1, this.calciteHandleChange = s({ cancelable: !1 }), this.calciteHandleNudge = s({ cancelable: !1 }), this.calciteInternalAssistiveTextChange = s({ cancelable: !1 });
  }
  static {
  ...
  async setFocus(e) {
    return this.focusSetter(() => this.handleButton.value, e);
  }
  willUpdate(e) {
  ...
  render() {
    return v({ disabled: this.disabled, children: b`<span .ariaChecked=${this.disabled ? null : this.selected} .ariaDisabled=${this.disabled ? this.disabled : null} .ariaLabel=${this.disabled ? null : this.getAriaText("label")} class=${h({ [r.handle]: !0, [r.handleSelected]: !this.disabled && this.selected })} @blur=${this.handleBlur} @keydown=${this.handleKeyDown} role=radio tabindex=${this.disabled ? null : 0} title=${this.getTooltip() ?? u} ${g(this.handleButton)}><calcite-icon .icon=${k.drag} scale=s></calcite-icon></span>` });
  }
}
p("calcite-handle", C);
export {
