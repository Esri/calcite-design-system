...
import { a as g, L as v, D as E, c as h, s as p, b, d as B } from "./index.js";
import { d as I } from "./debounce.js";
import { c as x } from "./observers.js";
import { g as S, d as O } from "./dom.js";
...
}, m = "calcite-block-group", k = "calcite-block", P = M("calcite-block-group"), H = g`:host([disabled]){cursor:default;-webkit-user-select:none;user-select:none;opacity:var(--calcite-opacity-disabled)}:host([disabled]) *,:host([disabled]) ::slotted(*){pointer-events:none}:host{display:block}.container{position:relative}.assistive-text{position:absolute;inline-size:var(--calcite-border-width-sm);block-size:var(--calcite-border-width-sm);padding:0;margin:calc(-1 * var(--calcite-border-width-sm));overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border-width:0}:host([hidden]){display:none}[hidden]{display:none}:host([disabled]) ::slotted([calcite-hydrated][disabled]),:host([disabled]) [calcite-hydrated][disabled]{opacity:1}.interaction-container{display:contents}:host([slot=children]) slot::slotted(calcite-block){border-block-end-width:0}`;
function T(d) {
  d.forEach((e) => {
...
class $ extends v {
  ...
  updateBlockItems() {
    ...
    const { dragEnabled: e, el: t, sortDisabled: s, sortHandleMenuItems: l } = this, a = Array.from(this.el.querySelectorAll(k)), i = t, r = Array.from(i.children).filter(c);
    a.forEach((o) => {
      o.closest(m) === t && (o.moveToItems = l.filter((n) => this.validateSortMenuItem({
        type: "move",
        fromEl: i,
        toEl: n.element,
        dragEl: o,
      ...
      })), o.addToItems = this.sortHandleMenuItems.filter((n) => this.validateSortMenuItem({
        type: "add",
        fromEl: i,
        toEl: n.element,
        dragEl: o,
  ...
  handleDefaultSlotChange(e) {
    const t = [];
    this.blockAndGroups = O(e).filter((s) => c(s) ? (t.push(s), !0) : P(s)), T(t), this.updateBlockAndGroupScale();
  }
  ...
  validateSortMenuItem({ fromEl: e, toEl: t, dragEl: s, newIndex: l, oldIndex: a, type: i }) {
  ...
  handleAdd(e) {
    ...
    const n = s.cloneNode();
    a.prepend(n), this.updateBlockItemsDebounced(), this.connectObserver();
    const f = {
      dragEl: s,
      fromEl: l,
      toEl: a,
  ...
  handleMove(e) {
    ...
    this.calciteBlockGroupOrderChange.emit(n), a.emitOrderChangeEvent(n);
  }
  handleReorder(e) {
    ...
    if (!l)
      return;
    s.sortHandleOpen = !1;
    const a = Array.from(l.children).filter(c), i = a.length - 1, r = a.indexOf(s);
    let o = r;
    switch (t) {
      case "top":
        o = 0;
    ...
    this.disconnectObserver();
    const n = t === "up" || t === "top" ? a[o] : a[o].nextSibling;
    l.insertBefore(s, n), this.updateBlockItemsDebounced(), this.connectObserver(), this.calciteBlockGroupOrderChange.emit({
      dragEl: s,
      fromEl: l,
      toEl: l,
      newIndex: o,
  ...
  render() {
    ...
    return this.interactiveContainer({ disabled: this.disabled, children: b`<div class=${p(u.container)}>${this.dragEnabled ? b`<span aria-live=assertive class=${p(u.assistiveText)}>${this.assistiveText}</span>` : null}${e ? b`<calcite-scrim class=${p(u.scrim)} .loading=${e}></calcite-scrim>` : null}<div .ariaBusy=${w(e, void 0)} .ariaLabel=${t || ""} class=${p(u.groupContainer)} role=group><slot @slotchange=${this.handleDefaultSlotChange}></slot></div></div>` });
  }
}
B("calcite-block-group", $);
export {
  $ as BlockGroup
};
