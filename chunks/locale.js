...
class i {
  ...
  format(t) {
    const { integers: s, decimals: n } = this.getIntegersAndDecimals(), r = `${this.isNegative ? t.minusSign : ""}${t.numberFormatter.format(BigInt(s))}`, a = n.length ? `${t.decimal}${n.split("").map((p) => t.numberFormatter.format(Number(p))).join("")}` : "";
    return `${r}${a}`;
  }
  add(t) {
...
const v = /^([-0])0+(?=\d)/, D = /(?!^\.)\.$/, T = /(?!^-)-/g, x = /^-\b0\b\.?0*$/, L = /0*$/, _ = /* @__PURE__ */ new Set(["e", "E", "-", ",", ".", ...b]), J = (e) => {
  const t = Array.from(e).filter((s) => _.has(s)).join("");
  return l(t, (s) => {
    const n = s.replace(T, "").replace(D, "").replace(v, "$1");
    return $(n) ? x.test(n) ? n : M(n) : s;
  });
};
function M(e) {
...
function y(e) {
  return e ? d.includes(e) ? e : (e = e.toLowerCase(), e.includes("-") && (e = e.replace(/(\w+)-(\w+)/, (t, s, n) => `${s}-${n.toUpperCase()}`), d.includes(e) || (e = e.split("-")[0])), e === "nb" || e === "nn" ? "no" : e === "zh" ? "zh-CN" : d.includes(e) ? e : (console.warn(`Translations for the "${e}" locale are not available and will fall back to the default, English (en).`), h)) : h;
}
function q(e) {
  switch (e) {
    case "it-CH":
...
class Z {
  constructor() {
    this.delocalize = (t) => (
      ...
      this._numberFormatOptions ? l(t, (s) => s.replace(new RegExp(`[${this._minusSign}]`, "g"), "-").replace(new RegExp(`[${this._group}]`, "g"), "").replace(new RegExp(`[${this._decimal}]`, "g"), ".").replace(new RegExp(`[${this._digits.join("")}]`, "g"), this._getDigitIndex)) : t
    ), this.localize = (t) => this._numberFormatOptions ? l(t, (s) => $(s.trim()) ? new i(s.trim()).format(this).replace(new RegExp(`[${this._actualGroup}]`, "g"), this._group) : s) : t;
  }
  get group() {
    return this._group;
  }
...
function Y(e, t) {
  e = y(e), c || (c = /* @__PURE__ */ new Map()), I !== e && (c.clear(), I = e);
  const s = z(t), n = c.get(s);
  if (n)
    return n;
