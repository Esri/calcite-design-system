import { a as h, L as d, d as a, s as i, x as t, c as v } from "./iframe.js";
import { e as r, n as o } from "./ref.js";
import { s as g } from "./dom.js";
import { c as b, g as n } from "./component.js";
...
class y extends d {
  ...
  async setFocus() {
    await b(this), !this.disabled && this.interactive ? this.containerEl.value?.focus() : !this.disabled && this.closable && this.closeButtonEl.value?.focus();
  }
  async load() {
    this.updateHasText();
  }
  ...
  renderChipImage() {
    return t`<div class=${i(c.imageContainer)}><slot name=${k.image} @slotchange=${this.handleSlotImageChange}></slot></div>`;
  }
  renderSelectionIcon() {
  ...
  renderCloseButton() {
    return t`<button .ariaLabel=${this.messages.dismissLabel} class=${i(c.close)} @click=${this.close} @keydown=${this.closeButtonKeyDownHandler} .tabIndex=${this.disabled ? -1 : 0} ${o(this.closeButtonEl)}><calcite-icon .icon=${l.close} .scale=${n(this.scale)}></calcite-icon></button>`;
  }
  renderIcon() {
  ...
  render() {
    ...
    })} @click=${this.handleEmittingEvent} .role=${p} .tabIndex=${s ? -1 : 0} ${o(this.containerEl)}>${this.selectionMode !== "none" && this.renderSelectionIcon() || ""}${this.renderChipImage()}${this.icon && this.renderIcon() || ""}<span class=${i(c.title)}><slot @slotchange=${this.handleDefaultSlotChange}></slot></span>${this.closable && this.renderCloseButton() || ""}</div>` });
  }
}
v("calcite-chip", y);
export {
