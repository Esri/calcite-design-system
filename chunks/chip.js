...
import { c as g, g as n } from "./component.js";
...
class y extends d {
  ...
  render() {
    ...
    })} @click=${this.handleEmittingEvent} .role=${p} .tabIndex=${r ? -1 : 0} ${o(this.containerEl)}>${this.selectionMode !== "none" && this.renderSelectionIcon() || ""}${this.renderChipImage()}${this.icon && this.renderIcon() || ""}<span class=${i(c.title)}><slot @slotchange=${this.handleDefaultSlotChange}></slot></span>${this.closable && this.renderCloseButton() || ""}</div>` });
  }
}
v("calcite-chip", y);
export {
