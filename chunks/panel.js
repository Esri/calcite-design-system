import { b as w, L as z, c as u, s as o, x as n, E as x, q as E } from "./index.js";
import { i as p } from "./keyed.js";
import { e as F, n as S } from "./ref.js";
...
class _ extends z {
  ...
  renderHeaderContent() {
    const { heading: e, headingLevel: a, description: i, hasHeaderContent: l, icon: r, scale: g } = this, f = r ? n`<calcite-icon class=${o(t.icon)} .flipRtl=${this.iconFlipRtl} .icon=${r} .scale=${T(g)}></calcite-icon>` : null, d = e ? O({ class: t.heading, level: a, children: e }) : null, h = i ? n`<span class=${o(t.description)}>${i}</span>` : null;
  ...
  renderHeaderSlottedContent() {
    return p("slotted-header-content", n`<div class=${o({ [t.headerContent]: !0, [t.headerSlottedContent]: !0 })} .hidden=${!this.hasHeaderContent}><slot name=${c.headerContent} @slotchange=${this.handleHeaderContentSlotChange}></slot></div>`);
  }
  renderHeaderStartActions() {
    const { hasStartActions: e } = this;
    return p("header-actions-start", n`<div class=${o({ [t.headerActionsStart]: !0, [t.headerActions]: !0 })} .hidden=${!e}><slot name=${c.headerActionsStart} @slotchange=${this.handleHeaderActionsStartSlotChange}></slot></div>`);
  }
  renderHeaderActionsEnd() {
    const { hasEndActions: e, messages: a, closable: i, collapsed: l, collapseDirection: r, collapsible: g, hasMenuItems: f } = this, { collapse: d, expand: h, close: v } = a, b = [m.expand, m.collapse];
    r === "up" && b.reverse();
    const $ = g ? n`<calcite-action .aria=${{ expanded: !l }} .icon=${l ? b[0] : b[1]} id=${A.collapse} .label=${d} @click=${this.collapse} .scale=${this.scale} .text=${d} title=${(l ? h : d) ?? x}></calcite-action>` : null, C = i ? n`<calcite-action .ariaLabel=${v} .icon=${m.close} id=${A.close} @click=${this.closeClickHandler} .scale=${this.scale} .text=${v} title=${v ?? x}></calcite-action>` : null, H = n`<slot name=${c.headerActionsEnd} @slotchange=${this.handleHeaderActionsEndSlotChange}></slot>`, k = e || $ || C || f;
  ...
  renderMenu() {
    ...
    return p("menu", n`<calcite-action-menu .flipPlacements=${l ?? ["top", "bottom"]} .hidden=${!e} .label=${a.options} .open=${i} .overlayPositioning=${this.overlayPositioning} .placement=${r}><calcite-action class=${o(t.menuAction)} .icon=${m.menu} .scale=${this.scale} slot=${M.trigger} .text=${a.options}></calcite-action><slot name=${c.headerMenuActions} @slotchange=${this.handleHeaderMenuActionsSlotChange}></slot></calcite-action-menu>`);
  }
  renderHeaderNode() {
  ...
  renderFooterNode() {
    ...
    return n`<footer class=${o(t.footer)} .hidden=${!r}><div class=${o(t.footerContent)} .hidden=${!i}><slot name=${c.footer} @slotchange=${this.handleFooterSlotChange}></slot></div><div class=${o(t.footerStart)} .hidden=${i || !a}><slot name=${c.footerStart} @slotchange=${this.handleFooterStartSlotChange}></slot></div><div class=${o(t.footerEnd)} .hidden=${i || !e}><slot name=${c.footerEnd} @slotchange=${this.handleFooterEndSlotChange}></slot></div><div class=${o(t.footerActions)} .hidden=${i || !l}>${p("footer-actions-slot", n`<slot name=${c.footerActions} @slotchange=${this.handleFooterActionsSlotChange}></slot>`)}</div></footer>`;
  }
  renderContent() {
  ...
  render() {
    const { disabled: e, loading: a, closed: i } = this, l = n`<article .ariaBusy=${a} class=${o(t.container)} .hidden=${i} ${S(this.containerRef)}>${this.renderHeaderNode()}${this.renderContent()}${this.renderContentBottom()}${this.renderFooterNode()}${p("alerts", n`<slot name=${c.alerts} @slotchange=${this.handleAlertsSlotChange}></slot>`)}</article>`;
