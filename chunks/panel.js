...
import { C as t, S as c, I as u, a as A } from "./resources6.js";
...
class R extends z {
  ...
  renderHeaderActionsEnd() {
    ...
    r === "up" && v.reverse();
    const $ = g ? n`<calcite-action .ariaExpanded=${!l} .ariaLabel=${d} .icon=${l ? v[0] : v[1]} id=${A.collapse} @click=${this.collapse} .scale=${this.scale} .text=${d} title=${(l ? h : d) ?? x}></calcite-action>` : null, C = i ? n`<calcite-action .ariaLabel=${m} .icon=${u.close} id=${A.close} @click=${this.handleUserClose} .scale=${this.scale} .text=${m} title=${m ?? x}></calcite-action>` : null, H = n`<slot name=${c.headerActionsEnd} @slotchange=${this.handleHeaderActionsEndSlotChange}></slot>`, k = e || $ || C || f;
  ...
  renderMenu() {
    ...
    return p("menu", n`<calcite-action-menu .flipPlacements=${l ?? ["top", "bottom"]} .hidden=${!e} .label=${a.options} .open=${i} .overlayPositioning=${this.overlayPositioning} .placement=${r}><calcite-action class=${o(t.menuAction)} .icon=${u.menu} .scale=${this.scale} slot=${M.trigger} .text=${a.options}></calcite-action><slot name=${c.headerMenuActions} @slotchange=${this.handleHeaderMenuActionsSlotChange}></slot></calcite-action-menu>`);
  }
  renderHeaderNode() {
  ...
  render() {
    const { disabled: e, loading: a, isClosed: i } = this, l = n`<article .ariaBusy=${a} class=${o(t.container)} .hidden=${i} ${S(this.setContainerRef)}>${this.renderHeaderNode()}${this.renderContent()}${this.renderContentBottom()}${this.renderFooterNode()}${p("alerts", n`<slot name=${c.alerts} @slotchange=${this.handleAlertsSlotChange}></slot>`)}</article>`;
