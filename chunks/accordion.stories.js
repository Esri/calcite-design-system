...
const {
  ...
  argTypes: {
    ...
    appearance: {
      options: g.values.filter((i) => i !== "outline" && i !== "outline-fill"),
      control: {
        type: "select"
      }
    },
    selectionMode: {
      options: _.values.filter((i) => i !== "none" && i !== "children" && i !== "multichildren" && i !== "ancestors"),
      control: {
        type: "select"
      }
    },
...
`, f = [{
...
}], w = f.map((i) => e`<calcite-accordion-item
        icon-start="${i.icon}"
        icon-end="${i.icon}"
        scale="l"
        heading="${i.heading}"
  ...
  <calcite-accordion scale="l" style="width: 600px"> ${w} </calcite-accordion>
`, l = () => e`
...
const S = ["simple", "withActions", "headingLevelAllScales", "darkModeRTL_TestOnly", "transparentAppearance_TestOnly", "withIconStartAndEnd_TestOnly", "longHeading_MediumIconForLargeAccordionItem_TestOnly", "slottedItemsStretched"];
export {
