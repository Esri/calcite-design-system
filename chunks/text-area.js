...
class G extends z {
  constructor() {
    super(...arguments), this.attributeWatch = S(["autofocus", "spellcheck"], this.handleGlobalAttributesChanged), this.footerEl = L(), this.guid = R(), this.resizeObserver = M("resize", async () => {
      ...
      const { textAreaHeight: e, textAreaWidth: t, elHeight: r, elWidth: l, footerHeight: n, footerWidth: s, validationMessageHeight: c } = this.getHeightAndWidthOfElements();
      if (s > 0 && s !== t && (this.footerEl.value.style.width = `${t}px`), this.resize === "none")
        return;
      const { width: p, height: b } = getComputedStyle(this.el);
      l !== t && p !== "auto" && this.updateSizeToAuto("width"), r !== e + n + c && b !== "auto" && this.updateSizeToAuto("height");
  ...
  setTextAreaHeight() {
    const { textAreaHeight: e, elHeight: t, footerHeight: r, validationMessageHeight: l } = this.getHeightAndWidthOfElements();
    r > 0 && e + r + l != t && (this.textAreaEl.style.height = `${t - r}px`);
  }
  getHeightAndWidthOfElements() {
    const { height: e, width: t } = this.textAreaEl ? this.textAreaEl.getBoundingClientRect() : g, { height: r, width: l } = this.el.getBoundingClientRect(), { height: n, width: s } = this.footerEl.value ? this.footerEl.value.getBoundingClientRect() : g, { height: c } = this.validationMessageEl ? this.validationMessageEl.getBoundingClientRect() : g;
    return {
      textAreaHeight: e,
      textAreaWidth: t,
      elHeight: r,
  ...
  render() {
    ...
    return j({ disabled: this.disabled, children: d`<div class=${i(a.wrapper)}><textarea aria-describedby=${this.guid ?? o} aria-errormessage=${v.validationMessage} .ariaInvalid=${this.status === "invalid" || this.isCharacterLimitExceeded()} .ariaLabel=${T(this)} .autofocus=${this.el.autofocus} class=${i({
      [a.textArea]: !0,
      ...
      [a.textAreaOnly]: !e
    })} .cols=${this.columns} .disabled=${this.disabled} maxlength=${(this.limitText ? this.maxLength : void 0) ?? o} name=${this.name ?? o} @change=${this.handleChange} @input=${this.handleInput} placeholder=${this.placeholder ?? o} .readOnly=${this.readOnly} .required=${this.required} .rows=${this.rows} spellcheck=${this.el.spellcheck ?? o} .value=${k(this.value ?? "")} wrap=${this.wrap ?? o} ${u(this.setTextAreaEl)}></textarea><span class=${i({ [a.content]: !0 })}><slot @slotchange=${this.contentSlotChangeHandler}></slot></span><footer class=${i({
      [a.footer]: !0,
      [a.readOnly]: this.readOnly,
      [a.hide]: !e
    })} ${u(this.footerEl)}><div class=${i({
      [a.container]: !0,
      [a.footerEndSlotOnly]: !this.startSlotHasElements && this.endSlotHasElements
    })}><slot name=${f.footerStart} @slotchange=${(t) => this.startSlotHasElements = x(t)}></slot><slot name=${f.footerEnd} @slotchange=${(t) => this.endSlotHasElements = x(t)}></slot></div>${this.renderCharacterLimit()}</footer>${O({ component: this })}${this.isCharacterLimitExceeded() && d`<span aria-live=polite class=${i(a.assistiveText)} id=${this.guid ?? o}>${this.replacePlaceholdersInMessages()}</span>` || ""}${this.validationMessage && this.status === "invalid" ? V({ icon: this.validationIcon, id: v.validationMessage, message: this.validationMessage, ref: this.setValidationRef, scale: this.scale, status: this.status }) : null}</div>` });
  }
