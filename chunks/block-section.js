...
import { a as x, L as u, c as g, l as b, s as o, b as n, I as m, d as f } from "./index.js";
...
class E extends u {
  ...
  handleHeaderKeyDown(t) {
    S(t.key) && (this.toggleSection(), t.preventDefault(), t.stopPropagation());
  }
  toggleSection() {
  ...
  renderIcon(t) {
    ...
    if ((t === "start" ? a : l) === void 0)
      return null;
    const s = c === "both" || c === "start", d = c === "both" || c === "end", r = t === "start";
    return k(r ? a : l, n`<calcite-icon class=${o(r ? e.iconStart : e.iconEnd)} .flipRtl=${r ? s : d} .icon=${r ? a : l} .scale=${h(this.scale)}></calcite-icon>`);
  }
  render() {
    const { messages: t, expanded: c, text: a, toggleDisplay: l } = this, v = c ? p.menuExpanded : p.menuCollapsed, s = c ? t.collapse : t.expand, d = l === "switch" ? n`<div class=${o({
      [e.toggleContainer]: !0
    })}><div aria-controls=${i.content} .ariaExpanded=${c} class=${o({
      [e.toggle]: !0
    })} id=${i.toggle} @click=${this.toggleSection} @keydown=${this.handleHeaderKeyDown} role=button tabindex=0 title=${s ?? m}>${this.renderIcon("start")}<div class=${o(e.toggleSwitchContent)}><span class=${o(e.toggleSwitchText)}>${a}</span></div>${this.renderIcon("end")}${this.renderStatusIcon()}<calcite-switch .checked=${c} class=${o(e.switch)} inert .label=${s} .scale=${this.scale}></calcite-switch></div></div>` : n`<div class=${o({
      [e.toggleContainer]: !0
    })}><button aria-controls=${i.content} .ariaExpanded=${c} class=${o({
      [e.toggle]: !0
    })} id=${i.toggle} @click=${this.toggleSection}>${this.renderIcon("start")}<span class=${o(e.sectionHeaderText)}>${a}</span>${this.renderIcon("end")}${this.renderStatusIcon()}<calcite-icon class=${o(e.chevronIcon)} .icon=${v} .scale=${h(this.scale)}></calcite-icon></button></div>`;
    return n`${d}<section aria-labelledby=${i.toggle} class=${o({ [e.content]: this.defaultSlotHasElements })} .hidden=${!c} id=${i.content}><slot @slotchange=${this.handleDefaultSlot}></slot></section>`;
  }
}
f("calcite-block-section", E);
export {
  E as BlockSection
};
