...
class I extends p {
  ...
  renderIcon(t) {
    const { iconFlipRtl: o, iconStart: i, iconEnd: n } = this;
    if ((t === "start" ? i : n) === void 0)
      return null;
    const r = o === "both" || o === "start", d = o === "both" || o === "end", s = t === "start";
    return f(s ? i : n, l`<calcite-icon class=${c(s ? e.iconStart : e.iconEnd)} .flipRtl=${s ? r : d} .icon=${s ? i : n} scale=s></calcite-icon>`);
  }
  render() {
    const { messages: t, expanded: o, text: i, toggleDisplay: n } = this, h = o ? g.menuExpanded : g.menuCollapsed, r = o ? t.collapse : t.expand, d = n === "switch" ? l`<div class=${c({
      [e.toggleContainer]: !0
    })}><div aria-controls=${a.content} .ariaExpanded=${o} class=${c({
      [e.toggle]: !0,
      [e.toggleSwitch]: !0
    })} id=${a.toggle} @click=${this.toggleSection} @keydown=${this.handleHeaderKeyDown} role=button tabindex=0 title=${r ?? x}>${this.renderIcon("start")}<div class=${c(e.toggleSwitchContent)}><span class=${c(e.toggleSwitchText)}>${i}</span></div>${this.renderIcon("end")}${this.renderStatusIcon()}<calcite-switch .checked=${o} class=${c(e.switch)} inert .label=${r} scale=s></calcite-switch></div></div>` : l`<div class=${c({
      [e.toggleContainer]: !0
    })}><button aria-controls=${a.content} .ariaExpanded=${o} class=${c({
      [e.sectionHeader]: !0,
      [e.toggle]: !0
    })} id=${a.toggle} @click=${this.toggleSection}>${this.renderIcon("start")}<span class=${c(e.sectionHeaderText)}>${i}</span>${this.renderIcon("end")}${this.renderStatusIcon()}<calcite-icon class=${c(e.chevronIcon)} .icon=${h} scale=s></calcite-icon></button></div>`;
    return l`${d}<section aria-labelledby=${a.toggle} class=${c(e.content)} .hidden=${!o} id=${a.content}><slot></slot></section>`;
  }
}
