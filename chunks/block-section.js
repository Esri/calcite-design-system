...
class I extends x {
  ...
  renderIcon(e) {
    const { iconFlipRtl: o, iconStart: a, iconEnd: i } = this;
    if ((e === "start" ? a : i) === void 0)
      return null;
    const s = o === "both" || o === "start", d = o === "both" || o === "end", r = e === "start";
    return f(r ? a : i, n`<calcite-icon class=${c(r ? t.iconStart : t.iconEnd)} .flipRtl=${r ? s : d} .icon=${r ? a : i} .scale=${h(this.scale)}></calcite-icon>`);
  }
  render() {
    const { messages: e, expanded: o, text: a, toggleDisplay: i } = this, v = o ? p.menuExpanded : p.menuCollapsed, s = o ? e.collapse : e.expand, d = i === "switch" ? n`<div class=${c({
      [t.toggleContainer]: !0
    })}><div aria-controls=${l.content} .ariaExpanded=${o} class=${c({
      [t.toggle]: !0
    })} id=${l.toggle} @click=${this.toggleSection} @keydown=${this.handleHeaderKeyDown} role=button tabindex=0 title=${s ?? b}>${this.renderIcon("start")}<div class=${c(t.toggleSwitchContent)}><span class=${c(t.toggleSwitchText)}>${a}</span></div>${this.renderIcon("end")}${this.renderStatusIcon()}<calcite-switch .checked=${o} class=${c(t.switch)} inert .label=${s} .scale=${this.scale}></calcite-switch></div></div>` : n`<div class=${c({
      [t.toggleContainer]: !0
    })}><button aria-controls=${l.content} .ariaExpanded=${o} class=${c({
      [t.toggle]: !0
    })} id=${l.toggle} @click=${this.toggleSection}>${this.renderIcon("start")}<span class=${c(t.sectionHeaderText)}>${a}</span>${this.renderIcon("end")}${this.renderStatusIcon()}<calcite-icon class=${c(t.chevronIcon)} .icon=${v} .scale=${h(this.scale)}></calcite-icon></button></div>`;
    return n`${d}<section aria-labelledby=${l.toggle} class=${c({ [t.content]: this.defaultSlotHasElements })} .hidden=${!o} id=${l.content}><slot @slotchange=${this.handleDefaultSlot}></slot></section>`;
  }
}
