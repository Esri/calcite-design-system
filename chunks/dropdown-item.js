...
class R extends x {
  ...
  render() {
    const { href: e, selectionMode: i, label: r, iconFlipRtl: c } = this, d = n`<calcite-icon class=${o(t.iconStart)} .flipRtl=${c === "start" || c === "both"} .icon=${this.iconStart} .scale=${l(this.scale)}></calcite-icon>`, a = n`<span class=${o(t.itemContent)}><slot></slot></span>`, h = n`<calcite-icon class=${o(t.iconEnd)} .flipRtl=${c === "end" || c === "both"} .icon=${this.iconEnd} .scale=${l(this.scale)}></calcite-icon>`, p = this.iconStart && this.iconEnd ? [d, a, h] : this.iconStart ? [d, a] : this.iconEnd ? [a, h] : a, v = e ? n`<a .ariaLabel=${r} class=${o(t.link)} href=${e ?? s} rel=${this.rel ?? s} tabindex=-1 target=${this.target ?? s} ${$(this.childLinkRef)}>${p}</a>` : p, f = e ? null : i === "single" ? "menuitemradio" : i === "multiple" ? "menuitemcheckbox" : "menuitem", m = i !== "none", b = y(m && this.selected, m ? "false" : null), { disabled: g } = this;
    return this.el.ariaChecked = b, this.el.ariaLabel = e ? "" : r ?? null, this.el.role = f, k(this.el, "tabIndex", -1), this.interactiveContainer({ disabled: g, children: n`<div class=${o({
      [t.container]: !0,
      [t.containerNone]: i === "none"
    })}>${i !== "none" ? n`<calcite-icon class=${o(t.icon)} .icon=${S.check} .scale=${l(this.scale)}></calcite-icon>` : null}${v}</div>` });
  }
}
