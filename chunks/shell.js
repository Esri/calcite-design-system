...
import { s as d, d as c, h as b } from "./dom.js";
...
class y extends $ {
  ...
  getDefaultSlotMinSize(e) {
    return this.defaultSlotRef.value?.assignedElements({ flatten: !0 }).reduce((t, n) => {
      ...
      return e === "inline" ? t + b(a.borderInlineStartWidth) + b(a.borderInlineEndWidth) : t + b(a.borderBlockStartWidth) + b(a.borderBlockEndWidth);
    }, 0) ?? 0;
  }
  ...
  renderContent() {
    const { panelIsResizing: e } = this, t = s("default-slot", l`<slot ${S(this.defaultSlotRef)}></slot>`), n = e ? l`<div class=${h(o.contentNonInteractive)}>${t}</div>` : t, a = s("panel-bottom-slot", l`<slot name=${r.panelBottom} @slotchange=${this.handlePanelBottomChange} ${S(this.panelBottomSlotRef)}></slot>`), i = s("panel-top-slot", l`<slot name=${r.panelTop} @slotchange=${this.handlePanelTopChange} ${S(this.panelTopSlotRef)}></slot>`), p = "content-container";
  ...
  renderMain() {
    ...
    })}><slot name=${r.panelStart} @slotchange=${this.handlePanelStartChange} ${S(this.panelStartSlotRef)}></slot>${this.renderContent()}<slot name=${r.panelEnd} @slotchange=${this.handlePanelEndChange} ${S(this.panelEndSlotRef)}></slot></div>`;
  }
