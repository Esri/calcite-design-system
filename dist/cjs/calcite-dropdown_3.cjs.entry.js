...
const CalciteDropdownItem = class {
    ...
    render() {
        ...
        const iconStartEl = (core.h("calcite-icon", { class: "dropdown-item-icon-start", icon: this.iconStart, scale: iconScale }));
        const iconEndEl = (core.h("calcite-icon", { class: "dropdown-item-icon-end", icon: this.iconEnd, scale: iconScale }));
        const slottedContent = this.iconStart && this.iconEnd ? ([iconStartEl, core.h("slot", null), iconEndEl]) : this.iconStart ? ([iconStartEl, core.h("slot", null)]) : this.iconEnd ? ([core.h("slot", null), iconEndEl]) : (core.h("slot", null));
        const contentEl = !this.href ? (slottedContent) : (core.h("a", { href: this.href, title: this.linkTitle }, slottedContent));
        return (core.h(core.Host, { theme: theme, dir: dir, scale: scale, tabindex: "0", role: "menuitem", "aria-selected": this.active.toString(), isLink: this.href }, contentEl));
    }
    //--------------------------------------------------------------------------
    //
    //  Event Listeners
