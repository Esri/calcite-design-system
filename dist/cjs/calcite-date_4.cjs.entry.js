...
const CalciteDatePicker = class {
    ...
    render() {
        ...
        return (index.h(index.Host, { role: "application", dir: dir }, index.h("slot", null), !this.noCalendarInput && (index.h("div", { class: "date-input-wrapper", role: "application" }, index.h("calcite-icon", { icon: "calendar", class: "calendar-icon", scale: "s" }), index.h("input", { type: "text", placeholder: this.localeData.placeholder, value: formattedDate, class: "date-input", onFocus: () => (this.showCalendar = true), onInput: (e) => this.input(e.target.value), onBlur: (e) => this.blur(e.target) }))), index.h("div", { class: "calendar-picker-wrapper" }, index.h("calcite-date-month-header", { activeDate: activeDate, selectedDate: date || new Date(), prevMonthLabel: this.prevMonthLabel, nextMonthLabel: this.nextMonthLabel, locale: this.locale, min: min, max: max, onCalciteActiveDateChange: (e) => {
                this.activeDate = new Date(e.detail);
            }, dir: dir }), index.h("calcite-date-month", { min: min, max: max, selectedDate: date, activeDate: activeDate, locale: this.locale, onCalciteDateSelect: (e) => {
    ...
    render() {
        ...
        const days = [
            ...prevMonDays.map((day) => {
                ...
                return (index.h("calcite-date-day", { day: day, disabled: !inRange(date, this.min, this.max), selected: sameDate(date, this.selectedDate), onCalciteDaySelect: () => this.calciteDateSelect.emit(date), locale: this.locale }));
            }),
            ...curMonDays.map((day) => {
                const date = new Date(year, month, day);
                const active = sameDate(date, this.activeDate);
                return (index.h("calcite-date-day", { day: day, disabled: !inRange(date, this.min, this.max), selected: sameDate(date, this.selectedDate), active: active, onCalciteDaySelect: () => this.calciteDateSelect.emit(date), locale: this.locale, ref: (el) => {
...
const CalciteDateMonthHeader = class {
    ...
    render() {
        ...
        const dir = dom.getElementDir(this.el);
        return (index.h(index.Host, { dir: dir }, index.h("div", { class: "month-year", "aria-hidden": "true" }, index.h("button", { class: "left-icon", "aria-label": this.prevMonthLabel, onClick: () => this.selectPrevMonth() }, index.h("calcite-icon", { icon: "chevron-left", scale: "s", mirrored: true, dir: dir })), index.h("div", { class: "month-year-text" }, index.h("span", { class: "month", role: "heading" }, localizedMonth), index.h("input", { class: "year", type: "text", inputmode: "numeric", maxlength: "4", minlength: "4", pattern: "\\d*", value: `${localizedYear.slice(-4)}`, onKeyDown: (event) => this.onYearKey(event), onChange: (event) => this.setYear(event.target.value), ref: (el) => (this.yearInput = el) })), index.h("button", { class: "right-icon", "aria-label": this.nextMonthLabel, onClick: () => this.selectNextMonth() }, index.h("calcite-icon", { icon: "chevron-right", scale: "s", mirrored: true, dir: dir })))));
    }
    //--------------------------------------------------------------------------
    //
    //  Private Methods
