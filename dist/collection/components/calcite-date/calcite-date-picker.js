...
export class CalciteDatePicker {
    ...
    render() {
        ...
        return (h(Host, { role: "application", expanded: this.showCalendar, onBlur: () => this.closeCalendar() },
            h("div", { class: `date-input-wrapper ${this.showCalendar ? "expanded" : ""}`, role: "application" },
                ...
                h("input", { type: "text", placeholder: "dd/mm/yyyy", value: this.value, class: "date-input", onFocus: () => this.expandCalendar(), onChange: (e) => { this.setDate(e.target); } })),
            this.showCalendar && (h("div", { class: "calendar-picker-wrapper" },
                ...
                h("calcite-date-month", { month: this.getMonth(), year: this.getYear(), min: this.min ? new Date(this.min) : null, max: this.max ? new Date(this.max) : null, selectedDate: selectedDate, activeDate: this.activeDate, startOfWeek: this.startOfWeek, locale: this.locale, onCalciteDateSelect: evt => this.setDate(evt.target), onCalciteActiveDateChange: evt => this.setActiveDate(evt.target) }))),
            h("slot", null)));
    }
