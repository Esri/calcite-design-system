...
describe("sticky header", () => {
  ...
  it("keeps focus behavior correct when a new first body row is slotted after initialization", async () => {
    ...
    const scrollMetrics = await page.$eval(
      "calcite-table",
      async (table, tableContainerClass) => {
        const scrollContainer = table.shadowRoot.querySelector<HTMLElement>(`.${tableContainerClass}`);
    ...
    await page.$eval("calcite-table", async (table) => {
      const newRow = document.createElement("calcite-table-row");
      newRow.id = "row-inserted";
      newRow.innerHTML = `
        <calcite-table-cell id="cell-inserted-a">inserted</calcite-table-cell>
        <calcite-table-cell id="cell-inserted-b">inserted</calcite-table-cell>
      `;

      const firstBodyRow = table.querySelector("calcite-table-row:not([slot])");
      firstBodyRow?.before(newRow);
    ...
    expect(await getFocusedElementProp(page, "id")).toBe("cell-inserted-a");
  ...
  it("keeps paginated sticky-header focus behavior correct when rows are inserted after initialization", async () => {
    ...
    await page.setContent(
      ...
      >
        <calcite-table-row slot="${SLOTS.tableHeader}">
          <calcite-table-header id="head-paged-1a" heading="Heading" description="Description"></calcite-table-header>
          <calcite-table-header id="head-paged-1b" heading="Heading" description="Description"></calcite-table-header>
        </calcite-table-row>
        ${Array.from(
          { length: 12 },
          (_, index) => html`
            <calcite-table-row id="row-paged-${index + 1}">
              <calcite-table-cell id="cell-paged-${index + 1}a">cell</calcite-table-cell>
              <calcite-table-cell id="cell-paged-${index + 1}b">cell</calcite-table-cell>
            </calcite-table-row>
          `,
        ).join("\n")}
      </calcite-table>`,
    ...
    await page.$eval("calcite-table", async (table) => {
      const newRow = document.createElement("calcite-table-row");
      newRow.id = "row-paged-inserted";
      newRow.innerHTML = `
        <calcite-table-cell id="cell-paged-inserted-a">inserted</calcite-table-cell>
        <calcite-table-cell id="cell-paged-inserted-b">inserted</calcite-table-cell>
      `;
    ...
    expect(await getFocusedElementProp(page, "id")).toBe("cell-paged-6a");
