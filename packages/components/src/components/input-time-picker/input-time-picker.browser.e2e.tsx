...
describe("labelable", () => {
  it("is labelable when wrapped in a label", async () => {
    await mount(
      <calcite-label id="label">
        Label <calcite-input-time-picker id="picker" />
      </calcite-label>,
    );

    await page.getBySelector("#label").click();
    await expect.element(page.getBySelector("#picker")).toHaveFocus();
  });

  it("is labelable with a label set as a sibling", async () => {
    await mount(
      <>
        <calcite-label for="picker" id="label">
          Label
        </calcite-label>
        <calcite-input-time-picker id="picker" />
      </>,
    );
  ...
  it("focuses only the first labelable in a wrapping label", async () => {
    await mount(
      <calcite-label id="label">
        Label
        <calcite-input-time-picker id="first-picker" />
        <calcite-input-time-picker id="second-picker" />
      </calcite-label>,
    );

    await page.getBySelector("#label").click();
    await expect.element(page.getBySelector("#first-picker")).toHaveFocus();
    await expect.element(page.getBySelector("#second-picker")).not.toHaveFocus();
  });

  for (const order of ["label-first", "picker-first"] as const) {
    it(`is labelable when a wrapping label is created ${order}`, async () => {
      await mount(<div id="container" />);
      const container = document.querySelector("#container")!;
      const label = document.createElement("calcite-label");
      label.id = "label";
      label.textContent = "Label";
      const picker = document.createElement("calcite-input-time-picker");
      picker.id = "picker";
    ...
    it(`is labelable when a sibling label is created ${order}`, async () => {
      await mount(<div id="container" />);
      const container = document.querySelector("#container")!;
      const label = document.createElement("calcite-label");
      label.id = "label";
      label.textContent = "Label";
      label.setAttribute("for", "picker");
      const picker = document.createElement("calcite-input-time-picker");
      picker.id = "picker";
      container.append(...(order === "label-first" ? [label, picker] : [picker, label]));
  ...
  it("is labelable when label for is set after initialization", async () => {
    await mount(
      <>
        <calcite-label id="label">Label</calcite-label>
        <calcite-input-time-picker id="picker" />
      </>,
    );
