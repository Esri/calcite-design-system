...
type OutputReferenceToken = Parameters<Exclude<OutputReferences, boolean>>[0];
...
export const hasOutputReferenceExtension = (token: OutputReferenceToken): boolean =>
  !!(
    token.original?.$extensions?.["calcite.outputReference"] || token.original?.extensions?.["calcite.outputReference"]
  );

export const hasReplacementReferenceExtension = (token: OutputReferenceToken): boolean =>
