...
const formattedVariables = jest.fn(({ dictionary, format, outputReferences }) => `${dictionary.allTokens.map(createPropertyFormatter({outputReferences, dictionary, format })).join(';\n')}`);
