      ...
      JSXIdentifier(node) {
        ...
        if (openingElement.type === "JSXOpeningElement") {
          ...
          if (refAttribute) {
            ...
            if (attributes.indexOf(refAttribute) !== attributes.length - 1) {
              context.report({
                node,
                ...
                fix(fixer) {
                  const fixerActions = [
                    ...
                    fixer.insertTextAfterRange(otherAttrs[otherAttrs.length - 1].range, disableRuleLine),
                  ];

                  return eslintDisableComments.length
                    ? [...eslintDisableComments.map((comment) => fixer.remove(comment as typeof node)), ...fixerActions]
                    : fixerActions;
                },
              });
            } else {
              context.report({
                ...
                message: `"ref" prop should be placed last in JSX to ensure the node attrs/props are in sync.`,
                fix(fixer) {
                  return [fixer.insertTextBefore(refAttribute as typeof node, disableRuleLine)];
                },
              });
            }
          }
