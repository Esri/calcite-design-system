...
for (const v of ["", "hidden"]) {
  tipContent[v] = html`
    ...
    <calcite-tip heading="Square Nature" ${v}>
...
export const simpleHidden = (args: TipManagerStoryArgs): string => html`
  <calcite-tip-manager ${boolean("closed", args.closed)}> ${tipContent["hidden"]} </calcite-tip-manager>
`;
