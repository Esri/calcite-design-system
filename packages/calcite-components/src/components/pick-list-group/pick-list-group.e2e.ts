...
describe("calcite-pick-list-group", () => {
  ...
  it("should render", async () => {
    const page = await newE2EPage(`<calcite-pick-list-group></calcite-pick-list-group>`);
  ...
  it("should render a header if one is provided", async () => {
    ...
    const page = await newE2EPage(`<calcite-pick-list-group group-title=${headingText}></calcite-pick-list-group>`);
  ...
  it("should indent children if a parent slot is passed", async () => {
    const page = await newE2EPage(`<calcite-pick-list filter-enabled>
      <calcite-pick-list-group>
