...
describe("calcite-slider", () => {
  ...
  describe.each(["horizontal", "vertical"])("%s layout", (layout) => {
    it("sets aria attributes properly for single value", async () => {
      ...
      expect(button).toEqualAttribute("aria-orientation", `${layout}`);
    });
    ...
    it("sets aria attributes properly for range values", async () => {
      ...
      expect(minButton).toEqualAttribute("aria-orientation", `${layout}`);
      expect(maxButton).toEqualAttribute("aria-orientation", `${layout}`);
    });
    ...
    describe("slider taking the precision of the provided step", () => {
      it("takes the precision of the decimal step when controlled through keyboard", async () => {
        ...
        await page.setContent(html`
          <calcite-slider value="30" min="0" max="100" step="1.12" layout=${layout}> </calcite-slider>
        `);
        const slider = await page.find("calcite-slider");
        const handle = await page.find("calcite-slider >>> .thumb");
        await page.waitForChanges();
    ...
    describe("mouse interaction", () => {
      ...
      it("single handle: clicking and dragging the track changes and emits the value", async () => {
        ...
        if (layout === "horizontal") {
          ...
          await page.mouse.move(trackX, trackY + 99);
          await page.mouse.move(trackX, trackY + 98);
          await page.mouse.move(trackX, trackY + 97);
          await page.mouse.move(trackX, trackY + 96);
          await page.mouse.move(trackX, trackY + 95);
        }
        await page.waitForChanges();
        await page.mouse.up();
      ...
      it("range: clicking the track to the left of the min handle changes minValue on mousedown, emits on mouseup", async () => {
        const page = await newE2EPage({
          html: `<calcite-slider min-value="50" max-value="75" snap style="width:${sliderWidthFor1To1PixelValueTrack}" layout=${layout}></calcite-slider>`,
        });
        await page.waitForChanges();
      ...
      it("range: clicking and dragging the track to the left of the min handle changes minValue and emits", async () => {
        ...
        if (layout === "horizontal") {
          ...
          await page.mouse.move(trackX, trackY + 79);
          await page.mouse.down();
          await page.mouse.move(trackX, trackY + 78);
          await page.mouse.move(trackX, trackY + 77);
          await page.mouse.move(trackX, trackY + 76);
          await page.mouse.move(trackX, trackY + 75);
        }
        await page.waitForChanges();
        await page.mouse.up();
      ...
      it("range: clicking the track to the right of the max handle changes maxValue on mousedown, emits on mouseup", async () => {
        const page = await newE2EPage({
          html: `<calcite-slider min-value="25" max-value="50" snap style="width:${sliderWidthFor1To1PixelValueTrack}; margin-top: 100px" layout=${layout}></calcite-slider>`,
        });
        await page.waitForChanges();
      ...
      it("range: clicking and dragging the track to the right of the max handle changes maxValue on mousedown, emits on mouseup", async () => {
        ...
        if (layout === "horizontal") {
          ...
          await page.mouse.move(trackX, trackY + 29);
          await page.mouse.down();
          await page.mouse.move(trackX, trackY + 28);
          await page.mouse.move(trackX, trackY + 27);
          await page.mouse.move(trackX, trackY + 26);
          await page.mouse.move(trackX, trackY + 25);
        }
        await page.mouse.up();
        await page.waitForChanges();
      ...
      it("range: clicking and dragging the range changes minValue and maxValue on mousedown, emits on mouseup", async () => {
        ...
        if (layout === "horizontal") {
          ...
          await page.mouse.move(trackX, trackY + 75);
          await page.mouse.down();
          await page.mouse.move(trackX, trackY + 74);
          await page.mouse.move(trackX, trackY + 73);
          await page.mouse.move(trackX, trackY + 72);
          await page.mouse.move(trackX, trackY + 71);
          await page.mouse.move(trackX, trackY + 70);
          await page.mouse.move(trackX, trackY + 69);
        }
        await page.mouse.up();
        await page.waitForChanges();
    ...
    describe("histogram", () => {
      it("creates calcite-graph with color stops", async () => {
        const page = await newE2EPage({ html: `<calcite-slider layout=${layout}></calcite-slider>` });
    ...
    describe("when range thumbs overlap", () => {
      ...
      describe("at min edge", () => {
        ...
        it("mirrored: click/tap should grab the max value thumb", async () => {
          await setUpTest(
            `${commonSliderAttrs} min-value="${expectedValue}" max-value="${expectedValue}" mirrored layout=${layout}`,
          );
    ...
    describe("when a range has 0 for both minValue and maxValue", () => {
      ...
      it("should position the minValue thumb beside the maxValue thumb", async () => {
        const page = await newE2EPage({ html: nonMirroredSlider.replace("{l}", layout) });
      ...
      it("should position the minValue thumb beside the maxValue thumb when mirrored", async () => {
        const page = await newE2EPage({ html: mirroredSlider.replace("{l}", layout) });
        const minValueThumb = await page.find("calcite-slider >>> .thumb--minValue");
        const maxValueThumb = await page.find("calcite-slider >>> .thumb--value");
        const minHandleLeft = await (await minValueThumb.getComputedStyle()).left;
        const maxHandleRight = await (await maxValueThumb.getComputedStyle()).right;
    ...
    describe("snap + step interaction", () => {
      ...
      async function dragThumbToMax(layout = "horizontal"): Promise<void> {
        ...
        const thumbWidth = layout === "horizontal" ? thumbRect.width : thumbRect.height;
      ...
      it("honors snap value with step", async () => {
        page = await newE2EPage({
          html: `<calcite-slider max="10" min="1" snap step="2" ticks="2" layout=${layout}></calcite-slider>`,
        });
        const slider = await page.find("calcite-slider");

        expect(await slider.getProperty("value")).toBe(1);

        await dragThumbToMax(layout);
        expect(await slider.getProperty("value")).toBe(9);
      });

      it("honors snap value with step (fractional)", async () => {
        page = await newE2EPage({
          html: `<calcite-slider max="10" min="1.5" snap step="1" ticks="1" layout=${layout}></calcite-slider>`,
        });
        const slider = await page.find("calcite-slider");
      ...
      it("snaps to max limit beyond upper bound", async () => {
        page = await newE2EPage({
          html: `<calcite-slider max="10.5" min="0" snap step="1" ticks="1" value="10.5" layout=${layout} style="margin-top: 369px"></calcite-slider>`,
        });
        const slider = await page.find("calcite-slider");
      ...
      it("snaps to max limit at upper bound", async () => {
        page = await newE2EPage({
          html: `<calcite-slider max="10.4" min="0" snap step="1" ticks="1" value="10.4" layout=${layout}></calcite-slider>`,
        });
        const slider = await page.find("calcite-slider");
