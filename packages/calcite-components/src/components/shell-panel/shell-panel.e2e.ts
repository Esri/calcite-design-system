...
describe("calcite-shell-panel", () => {
  ...
  it("has a slot", async () => {
    const page = await newE2EPage("<calcite-shell-panel></calcite-shell-panel>");
  ...
  it("should have detached class when detached", async () => {
    const page = await newE2EPage("<calcite-shell-panel><div>content</div></calcite-shell-panel>");
  ...
  it("should update width based on the requested CSS variable override", async () => {
    ...
    const content2 = await page.find(`calcite-shell-panel >>> .${CSS.content}`);
