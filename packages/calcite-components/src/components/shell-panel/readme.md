...
| --------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ | ------------ |
| `actionBarPosition`   | `action-bar-position`   | Specifies the action bar's position. Will be flipped when the element direction is right-to-left (`"rtl"`).                                                                                                                                                                                                                  | `"end" \| "start"`                                     | `"start"`    |
...
| `detachedHeightScale` | `detached-height-scale` | <span style="color:red">**[DEPRECATED]**</span> Use `heightScale` instead.<br/><br/>When `displayMode` is `content-detached`, specifies the maximum height of the component.                                                                                                                                                 | `"l" \| "m" \| "s"`                                    | `undefined`  |
| `displayMode`         | `display-mode`          | Specifies the display mode of the component, where: `"dock"` displays at full height adjacent to center content, `"overlay"` displays at full height on top of center content, and `"content-detached"` does not display at full height with content separately detached from `calcite-action-bar` on top of center content. | `"content-detached" \| "dock" \| "float" \| "overlay"` | `"dock"`     |
...
| `resizable`           | `resizable`             | When `true` and `displayMode` is not `content-detached`, the component's content area is resizable.                                                                                                                                                                                                                          | `boolean`                                              | `false`      |
