...
@Component({
  tag: "calcite-date-picker-month-range",
  styleUrl: "date-picker-month-range.scss",
  shadow: true,
})
export class DatePickerMonthRange {
  //--------------------------------------------------------------------------
  //
  //  Properties
  //
    ...
    return (
      <Host onFocusOut={this.disableActiveFocus} onKeyDown={this.keyDownHandler}>
        <div class="calendar" role="grid">
          {this.renderMonth(adjustedWeekDays, weeks)}
          {this.renderMonth(adjustedWeekDays, nextMonthWeeks)}
        </div>
      </Host>
    );
  }
  ...
  @Element() el: HTMLCalciteDatePickerMonthRangeElement;
  ...
  private renderMonth(days: string[], weeks: Day[][]): VNode {
    return (
      <div class="month">
