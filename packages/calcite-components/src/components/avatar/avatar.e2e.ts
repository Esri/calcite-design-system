...
describe("calcite-avatar", () => {
  ...
  it("renders thumbnail when provided", async () => {
    const page = await newE2EPage(`<calcite-avatar thumbnail="${placeholderUrl}"></calcite-avatar>`);
  ...
  it("renders initials when no thumbnail is provided", async () => {
    const page = await newE2EPage("<calcite-avatar full-name='My Dude'></calcite-avatar>");
  ...
  it("computes a background fill color based on user id", async () => {
    const page = await newE2EPage("<calcite-avatar user-id='25684463a00c449585dbb32a065f6b74'></calcite-avatar>");
  ...
  it("computes a background fill if id is not a valid hex", async () => {
    const page = await newE2EPage(
      "<calcite-avatar user-id='for sure not a hex' username='THaverford'></calcite-avatar>",
    );
    const initials = await page.find("calcite-avatar >>> .initials");
  ...
  it("renders default icon when no information is passed", async () => {
    const page = await newE2EPage("<calcite-avatar></calcite-avatar>");
