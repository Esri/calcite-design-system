...
export class DatePickerMonth extends LitElement {
  ...
  override render(): JsxNode {
    ...
    const numDaysToDisplay = NUM_WEEKS_TO_DISPLAY * DAYS_PER_WEEK;
    if (numDaysDisplayed < numDaysToDisplay) {
      const initialDay = nextMonDays.length ? nextMonDays[nextMonDays.length - 1] : 0;
      for (let i = 1; i <= numDaysToDisplay - numDaysDisplayed; i++) {
