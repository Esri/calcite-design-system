...
import { assertRequiredOption, formatDescription, getSummary } from "./utils/commands.ts";
...
export const registerCommand = (command: Command) =>
  void command
    ...
    .action(notifyTeamsAction);

async function notifyTeamsAction({ webhook, title, message, action_text, action_url }: Partial<NotifyTeams>) {
 ...
 * succeeded.
 */
export async function notifyTeams({ webhook, title, message, action_text, action_url }: NotifyTeams) {
  const teamsCard = {
    type: "message",
    attachments: [
      {
